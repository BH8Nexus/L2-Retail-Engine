...
public class SelectorThread<T extends MMOClient> extends Thread
{
	...
	protected boolean tryReadPacket2(SelectionKey key, MMOConnection<T> con, ByteBuffer buf)
	{
		...
		// check if we have enough bytes to read the header and not the empty package body
		if (buf.remaining() > _sc.HEADER_SIZE)
		{
			...
			// check if there is enough bytes to read the body
			if (size <= buf.remaining())
			{
				...
				// finished reading from the buffer, clean
				if (!buf.hasRemaining())
				{
					freeBuffer(buf, con);
					return false;
			...
			// there is not enough data to read the package body, reset the position
			buf.position(pos);
		}
