...
public final class StatsUtils
{
	...
	public static CharSequence getMemUsageHtml()
	{
		double maxMem = memMXbean.getHeapMemoryUsage().getMax(); // maxMemory is the upper limit the jvm can use
		double allocatedMem = memMXbean.getHeapMemoryUsage().getCommitted(); //totalMemory the size of the current allocation pool
		double usedMem = memMXbean.getHeapMemoryUsage().getUsed(); // freeMemory the unused memory in the allocation pool
		double nonAllocatedMem = maxMem - allocatedMem; //non allocated memory till jvm limit
		double cachedMem = allocatedMem - usedMem; // really used memory
		double useableMem = maxMem - usedMem; //allocated, but non-used and non-allocated memory
		...
		list.append("AllowedMemory: ").append(toNumInUnits((long) maxMem)).append("<br1>");
		list.append("&nbsp; *Allocated: ").append(toNumInUnits((long) allocatedMem)).append(" ").append(((double) Math.round(allocatedMem / maxMem * 1000000) / 10000)).append("%").append("<br1>");
		list.append("&nbsp; *Non-Allocated: ").append(toNumInUnits((long) nonAllocatedMem)).append(" ").append((double) Math.round(nonAllocatedMem / maxMem * 1000000) / 10000).append("%").append("<br1>");
		list.append("AllocatedMemory: ").append(toNumInUnits((long) allocatedMem)).append("").append("<br1>");
		list.append("&nbsp; *Used: ").append(toNumInUnits((long) usedMem)).append(" ").append((double) Math.round(usedMem / maxMem * 1000000) / 10000).append("%").append("<br1>");
		list.append("&nbsp; *Unused (cached): ").append(toNumInUnits((long) cachedMem)).append(" ").append((double) Math.round(cachedMem / maxMem * 1000000) / 10000).append("%").append("<br1>");
		list.append("UseableMemory: ").append(toNumInUnits((long) useableMem)).append(" ").append((double) Math.round(useableMem / maxMem * 1000000) / 10000).append("%").append("<br1>");
	...
	public static CharSequence getThreadStatsHtml()
	{
		StringBuilder list = new StringBuilder();
		...
		list.append("Live: .................... ").append(threadCount).append(" threads").append("<br1>");
		list.append("&nbsp; *Non-Daemon: ......... ").append(nonDaemonCount).append(" threads").append("<br1>");
		list.append("&nbsp; *Daemon: ............. ").append(daemonCount).append(" threads").append("<br1>");
		list.append("Peak: .................... ").append(peakCount).append(" threads").append("<br1>");
		list.append("Total started: ........... ").append(totalCount).append(" threads").append("<br1>");
	...
	public static CharSequence getGCStatsHtml()
	{
		StringBuilder list = new StringBuilder();

		for(GarbageCollectorMXBean gcBean : ManagementFactory.getGarbageCollectorMXBeans())
		{
			list.append("GarbageCollector (").append(gcBean.getName()).append(")<br1>");
			list.append("getCollectionCount: ..... ").append(gcBean.getCollectionCount()).append("<br1>");
			list.append("getCollectionTime: ...... ").append(gcBean.getCollectionTime()).append(" ms").append("<br1>");
		}
