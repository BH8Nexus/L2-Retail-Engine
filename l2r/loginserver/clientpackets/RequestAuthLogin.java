...
import l2r.loginserver.serverpackets.AccountKicked.AccountKickedReason;
...
public class RequestAuthLogin extends L2LoginClientPacket
{
	...
	protected void runImpl() throws Exception
	{
		...
		if (!passwordCorrect)
		{
			// Check if the password is encrypted by one of the obsolete but supported algorithms
		...
		if (Config.BANNED_IP.contains(client.getIpAddress()))
		{
		...
		if (Config.ENABLE_ACCOUNT_EMAIL_ACTIVATION)
		{
			...
			{
				client.close(AccountKickedReason.REASON_ACCOUNT_MUST_BE_VERIFIED);
				return;
			}
		}
		...
		account.setLoginserverIP(client.getConnection().getSocket().getLocalAddress().getHostAddress());
