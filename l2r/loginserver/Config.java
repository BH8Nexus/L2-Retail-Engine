...
import l2r.commons.util.Regexp;
import l2r.commons.util.Rnd;
import l2r.loginserver.crypt.PasswordHash;
import l2r.loginserver.crypt.ScrambledKeyPair;
...
//	public static String DATABASE_PASSWORD;
	...
	public static boolean ENABLE_ACCOUNT_EMAIL_ACTIVATION;
	...
	public static List<String> BANNED_IP = new ArrayList<>();
	public static boolean EnableProxieIps;
	public static boolean HideMainServer;
	public static Map<Integer, String> PROXIES = new HashMap<>();
	public static Map<Integer, String> LATINPROXIES = new HashMap<>();
	
	public static String proxiesToLoad;
	public static String latinProxiesToLoad;
	...
	public final static void load()
	{
		loadConfiguration();
		loadServerNames();
		// System.out.println(proxiesToLoad);
		loadProxies(proxiesToLoad);
		loadLatinProxies(latinProxiesToLoad);
	}
	
	private static void loadProxies(String value)
	{
		if (!value.isEmpty())
		{
			...
			if (pairs.length != 0)
			{
				...
				{
					String[] ids = set.split("@");
					PROXIES.put(Integer.parseInt(ids[0]), ids[1]);
				}
			}
		}
	}
	
	private static void loadLatinProxies(String value)
	{
		if (!value.isEmpty())
		{
			String[] pairs = value.split(";");
			if (pairs.length != 0)
			{
				...
				{
					String[] ids = set.split("@");
					LATINPROXIES.put(Integer.parseInt(ids[0]), ids[1]);
				}
			}
		}
	}
	...
	public final static void loadServerNames()
	{
		...
		try
		{
			...
			Document document = reader.read(SERVER_NAMES_FILE);
...
//		DATABASE_PASSWORD = serverSettings.getProperty("Password", "");
		...
		ENABLE_ACCOUNT_EMAIL_ACTIVATION = serverSettings.getProperty("accountEmailActivation", true);
		...
		EnableProxieIps = serverSettings.getProperty("NewProxiesEnabled", true);
		HideMainServer = serverSettings.getProperty("HideMainServer", true);
		proxiesToLoad = serverSettings.getProperty("proxies", "");
		latinProxiesToLoad = serverSettings.getProperty("latinProxies", "");
		DONT_SEND_SERVER_LIST_IF_ONE_SERVER = serverSettings.getProperty("DONT_SEND_SERVER_LIST_IF_ONE_SERVER", false);
	}
	
	public final static void loadBannedIps()
	{
		try
		{
			for (String ip : FileUtils.readFileToString(new File("config/banned_ip.cfg"), "UTF-8").split("\n"))
			{
				if (Regexp.isMatchingRegexp(ip, "\\d{1,4}\\.\\d{1,4}\\.\\d{1,4}\\.\\d{1,4}"))
				{
					BANNED_IP.add(ip);
				}
			}
			_log.info("Loaded " + BANNED_IP.size() + " blocked IPs");
		}
		catch (Exception e)
		{
			_log.error("Error while loading config/banned-ips.txt!");
		}
	}
