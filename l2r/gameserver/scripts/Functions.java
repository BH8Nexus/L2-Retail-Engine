...
public class Functions
{
	...
	/**
	 * Î â€™Î¡â€¹Î Â·Î¡â€¹Î Â²Î Â°Î ÂµÎ¡â€š Î ÎŒÎ ÂµÎ¡â€šÎ ÎŽÎ Î„ Î¡ï¿½ Î Â·Î Â°Î Î„Î ÂµÎ¡â‚¬Î Â¶Î ÎŠÎ ÎŽÎ Î‰
	 * @param object - Î ÎŽÎ¡â€š Î¡â€¡Î¡ï¿½Î ÂµÎ Â³Î ÎŽ Î ÎˆÎ ÎŒÎ ÂµÎ Â½Î Îˆ Î Â²Î¡â€¹Î Â·Î¡â€¹Î Â²Î Â°Î¡â€šÎ¡ï¿½
	 * @param sClass<?> - Î Â²Î¡â€¹Î Â·Î¡â€¹Î Â²Î Â°Î ÂµÎ ÎŒÎ¡â€¹Î Î‰ Î ÎŠÎ Â»Î Â°Î¡ï¿½Î¡ï¿½
	 * @param methodName - Î Â²Î¡â€¹Î Â·Î¡â€¹Î Â²Î Â°Î ÂµÎ ÎŒÎ¡â€¹Î Î‰ Î ÎŒÎ ÂµÎ¡â€šÎ ÎŽÎ Î„
	 * @param args - Î ÎŒÎ Â°Î¡ï¿½Î¡ï¿½Î ÎˆÎ Â² Î Â°Î¡â‚¬Î Â³Î¡Æ’Î ÎŒÎ ÂµÎ¡â€šÎ ÎŽÎ Â²
	 * @param variables - Î¡ï¿½Î Î�Î ÎˆÎ¡ï¿½Î ÎŽÎ ÎŠ Î Â²Î¡â€¹Î¡ï¿½Î¡â€šÎ Â°Î Â²Î Â»Î¡ï¿½Î ÂµÎ ÎŒÎ¡â€¹Î¡â€¦ Î Î�Î ÂµÎ¡â‚¬Î ÂµÎ ÎŒÎ ÂµÎ Â½Î Â½Î¡â€¹Î¡â€¦
	 * @param delay - Î Â·Î Â°Î Î„Î ÂµÎ¡â‚¬Î Â¶Î ÎŠÎ Â° Î Â² Î ÎŒÎ ÎˆÎ Â»Î Â»Î ÎˆÎ¡ï¿½Î ÂµÎ ÎŠÎ¡Æ’Î Â½Î Î„Î Â°Î¡â€¦
	 */
	...
	/**
	 * Î â€™Î¡â€¹Î Â·Î¡â€¹Î Â²Î Â°Î¡â€šÎ¡ï¿½ Î¡â€šÎ ÎŽÎ Â»Î¡ï¿½Î ÎŠÎ ÎŽ Î ÎˆÎ Â· Î¡ï¿½Î ÎŠÎ¡â‚¬Î ÎˆÎ Î�Î¡â€šÎ ÎŽÎ Â²
	 */
	...
	/**
	 * Î Î…Î¡â€šÎ Â°Î¡â€šÎ ÎˆÎ¡â€¡Î ÂµÎ¡ï¿½Î ÎŠÎ ÎˆÎ Î‰ Î ÎŒÎ ÂµÎ¡â€šÎ ÎŽÎ Î„, Î Î„Î Â»Î¡ï¿½ Î Â²Î¡â€¹Î Â·Î ÎŽÎ Â²Î Â° Î ÎˆÎ Â· Î Â»Î¡ï¿½Î Â±Î¡â€¹Î¡â€¦ Î ÎŒÎ ÂµÎ¡ï¿½Î¡â€š
	 */
	public static void show(String text, Player self, NpcInstance npc, Object... arg)
	{
		...
		// Î Î�Î¡â‚¬Î ÎˆÎ Â²Î ÎŽÎ Î„Î ÎˆÎ ÎŒ Î Â½Î Â°Î¡ï¿½Î¡Æ’ html-Î ÎŠÎ¡Æ’ Î Â² Î Â½Î¡Æ’Î Â¶Î Â½Î¡â€¹Î Î‰ Î Â²Î ÎˆÎ Î„
	...
	// Î â€˜Î ÂµÎ Â»Î¡â€¹Î Î‰ Î¡â€¡Î Â°Î¡â€š
	...
	// Shout (Î Â¶Î ÂµÎ Â»Î¡â€šÎ¡â€¹Î Î‰) Î¡â€¡Î Â°Î¡â€š
	...
	public static void npcShout(NpcInstance npc, NpcString npcString, String... params)
	{
		...
		NpcSay cs = new NpcSay(npc, ChatType.NPC_CHAT, npcString, params);
	...
	/*
	 * public static void addItemToParty(Player player, int item_id, long count) { if ((player == null) || (count < 1)) { return; } if (player.getParty() != null) { if (player.getParty().getLootDistribution() == Party.ITEM_LOOTER) { addItem(player, item_id, count); } else if
	 * ((player.getParty().getLootDistribution() == Party.ITEM_ORDER) || (player.getParty().getLootDistribution() == Party.ITEM_RANDOM_SPOIL)) { ItemInstance item = new ItemInstance(item_id); item.setCount(count); player.getParty().distributeItem(player, item, null); } else { ItemInstance item = new
	 * ItemInstance(item_id); item.setCount(count); player.getParty().distributeItem(player, item, null); } } } /**
	 * @see ItemFunctions
	 */
	/*
	 * public static void addItem(Playable playable, int itemId, long count) { // ItemFunctions.addItem(playable, itemId, count, true); if (!playable.isPet()) { playable.getPlayer().getInventory().addItem(itemId, count, true); } else { ((PetInstance) playable).getInventory().addItem(itemId, count,
	 * true); } } /**
	 * @see ItemFunctions
	 */
	/*
	 * public static void addItem(Playable playable, int itemId, long count, boolean mess) { // ItemFunctions.addItem(playable, itemId, count, mess); if (!playable.isPet()) { playable.getPlayer().getInventory().addItem(itemId, count, mess); } else { ((PetInstance)
	 * playable).getInventory().addItem(itemId, count, mess); } } /**
	 * @see ItemFunctions
	 */
	/*
	 * public static long getItemCount(Playable playable, int itemId) { return ItemFunctions.getItemCount(playable, itemId); } /**
	 * @see ItemFunctions
	 */
	/*
	 * public static long removeItem(Playable playable, int itemId, long count) { return ItemFunctions.removeItem(playable, itemId, count, true); }
	 */
	/**
	 * @see ItemFunctions
	 */
	/*
	 * public static long removeItem(Playable playable, int itemId, long count) { if ((playable == null) || (count < 1)) { return 0; } Player player = playable.getPlayer(); player.getInventory().writeLock(); try { ItemTemplate t = ItemHolder.getInstance().getTemplate(itemId); if (t == null) { return
	 * 0; } if (t.isStackable()) { if (!player.getInventory().destroyItemByItemId(itemId, count)) { // TODO audit return 0; } } else { if (player.getInventory().getCountOf(itemId) < count) { return 0; } for (long i = 0; i < count; i++) { if (!player.getInventory().destroyItemByItemId(itemId, 1L)) {
	 * // TODO audit return 0; } } } } finally { player.getInventory().writeUnlock(); } player.sendPacket(SystemMessage2.removeItems(itemId, count)); return count; /* if (ItemFunctions.getItemCount(playable, itemId) < count) { return 0; } boolean removed = false; if (!playable.isPet()) { removed =
	 * playable.getPlayer().getInventory().removeItem(itemId, count, true); } else { removed = ((PetInstance) playable).getInventory().removeItem(itemId, count, true); } return removed ? count : 0;
	 */
	// }
    ...
    public static boolean haveItem(final Playable playable, final int itemId, final long count) {
	...
	public static void sendAuctionMail(Player receiver, String title, String body, Map<Integer, Long> items)
	{
		...
		{
			ItemInstance item = new ItemInstance(itm.getKey());
	...
	public static void openColiseumDoors()
	{
		for (int i = 24190001; i <= 24190004; i++)
		{
			ReflectionUtils.getDoor(i).openMe();
		}
	}
	
	public static void closeColiseumDoors()
	{
		for (int i = 24190001; i <= 24190004; i++)
		{
