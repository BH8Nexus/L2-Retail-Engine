...
public class Scripts
{
	...
	public void init()
	{
		for (Class<?> clazz : _classes.values())
		{
			...
			if (Config.DONTLOADEVENTS && clazz.getName().startsWith("events"))
			{
				continue;
			}
	...
	/**
	 * Load all scripts from data/scripts/<b>target</b>. If <b>target</b> is an empty string, it will load all scripts.
	...
	private boolean load(List<Class<?>> classes, String target)
	{
		...
		else
		{
			...
			IOFileFilter filter = new DirectoryFileFilter()
			{
				 ...
				 * 
				 */
				private static final long serialVersionUID = 3630726004293712995L;
				...
				public boolean accept(File file)
				{
					...
					if (Config.DONTLOADEVENTS && file.isDirectory() && file.getPath().contains("events/"))
					{
						return false;
					}
