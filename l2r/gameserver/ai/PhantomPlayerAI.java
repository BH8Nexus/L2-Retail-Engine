...
public class PhantomPlayerAI extends PlayerAI implements OnAiEventListener, OnMagicHitListener, OnKillListener
{
	...
	// protected final NavigableSet<Task> _tasks = new ConcurrentSkipListSet<>();
	protected ConcurrentSkipListSet<Task> _tasks = new ConcurrentSkipListSet<>(task_comparator);
	...
	protected boolean thinkActive()
	{
		try
		{
			...
			if (getMood().equalsIgnoreCase("farming"))
			{
				...
				/*
				 * disable level difference. int maxLevelDiff = 10; if (actor.getLevel() > 80) maxLevelDiff = 3; else if (actor.getLevel() > 50) maxLevelDiff = 5; && (actor.getLevel() - ((Creature) actor.getTarget()).getLevel() > 4 || ((Creature) actor.getTarget()).getLevel() - actor.getLevel() >
				 * maxLevelDiff)
				 */
				...
				if (actor.getTarget() == null)
				{
					...
					/*
					 * Location randomLocation = getRandomLocation(300, 500, 200); if (randomLocation == null) randomLocation = getRandomLocation(200, 300, 200); if (randomLocation == null) randomLocation = getRandomLocation(100, 200, 200); if (randomLocation != null) { addTaskMove(randomLocation,
					 * 0, true); return true; } else // Fuck this... SOE out.
					 */
					{
						castSOE(-1);
						return true;
			...
			if (actor.getClassId().isMage())
			{
				...
				int actorSpeed = (int) actor.getSpeed(actor.getTemplate().getBaseRunSpd());
				int targetSpeed = (int) target.getSpeed(target.getTemplate().getBaseRunSpd());
				...
				/*
				 * if (speedDiff < 0 && distance <= 600 && target.getEffectList().getEffectByStackType("RunSpeedDown") == null) { // Find the best slow skill int slowChance = 0; for (Skill skill : actor.getAllAvailableSkillsArray(SkillType.DEBUFF)) { for (EffectTemplate et :
				 * skill.getEffectTemplates()) { if ("RunSpeedDown".equalsIgnoreCase(et._stackType)) { int slow = (int) Formulas.calcSkillSuccessChance(actor, target, skill); if (slowChance < slow) { slowChance = slow; skillCast = skill; } } } } if (skillCast != null) { if (slowChance >= 25) //
				 * Else... meh, not worthy to cast it. { say("Going to slow with " + skillCast.getName() + "(" + slowChance + ")"); setIntention(CtrlIntention.AI_INTENTION_CAST, skillCast, target); return true; } else say("Skipping slow with " + skillCast.getName() + "(" + slowChance +
				 * ") chance is too low."); } skillCast = null; // Skill not used, make it null again. }
				 */
		...
		try
		{
			...
			OUT: for (Creature tgt : list)
			{
				...
				else
				{
					...
					else if (!GeoEngine.canSeeTarget(getActor(), tgt, false))
					{
						lowPriorityTargets.add(tgt);
					}
				}
	...
	private class RoamingTask implements Runnable
	{
		...
		public void run()
		{
			...
			else if (ai.getMood().contains("check npcs"))
			{
				...
				if (((loc == null) && (target == null)) || Rnd.chance(25))
				{
					rnd = Rnd.get(100); // New mood time...
				}
			}
		...
		public void run()
		{
			...
			try
			{
				...
				if (ai.isInTown())
				{
					...
					List<Creature> npcList = new ArrayList<>();
					Location tpLoc = null;
	...
	protected void upgradeClass()
	{
			...
			(((getActor().getLevel() >= 20) && (getActor().getClassId().getLevel() == 1)) || ((getActor().getLevel() >= 40) && (getActor().getClassId().getLevel() == 2)) || ((getActor().getLevel() >= 75) && (getActor().getClassId().getLevel() == 3))))
		{
			...
			List<ClassId> classesToChoseFrom = new ArrayList<>(2);
	...
	protected void equipWeapon(WeaponType weaponType)
	{
		...
		for (ItemInstance item : getActor().getInventory().getItems())
		{
			if (item.isWeapon() && (((WeaponTemplate) item.getTemplate()).getItemType() == weaponType))
			{
				...
				else if (topWeapon.getTemplate().getReferencePrice() < item.getTemplate().getReferencePrice())
				{
					topWeapon = item;
				}
			}
...
//					}
	...
	public static Map<String, Location> TOWN_LOCATIONS = new HashMap<>();
	static
	{
	...
	public static Map<Integer, Location> HUNTING_GROUNDS = new HashMap<>();
	static
	{
		/*
		 * HUNTING_GROUNDS.put(81, new Location(177672, -180088, 2016)); // Mithril Mines HUNTING_GROUNDS.put(81, new Location(177496, -174664, 1344)); HUNTING_GROUNDS.put(81, new Location(175096, -184888, -1896)); HUNTING_GROUNDS.put(83, new Location(83368, 93896, -3360)); // Oren Lizardman
		 * HUNTING_GROUNDS.put(83, new Location(90856, 89880, -3432)); HUNTING_GROUNDS.put(83, new Location(91976, 82088, -3552)); HUNTING_GROUNDS.put(83, new Location(82552, 75304, -3632)); HUNTING_GROUNDS.put(new int[]{7,9}, new Location(-97448, 234392, -3520)); // Obelisk of Victory
		 * HUNTING_GROUNDS.put(new int[]{13,14}, new Location(-103960, 226472, -3648)); HUNTING_GROUNDS.put(new int[]{13,14}, new Location(-100552, 213288, -3072)); HUNTING_GROUNDS.put(new int[]{18,22}, new Location(173288, -211560, -3664)); HUNTING_GROUNDS.put(new int[]{}, new Location(135752,
		 * -172584, -1776)); // Coal Mines HUNTING_GROUNDS.put(new int[]{}, new Location(143496, -170360, -1776)); HUNTING_GROUNDS.put(new int[]{15,17}, new Location(146632, -175016, -1520)); HUNTING_GROUNDS.put(new int[]{7,9}, new Location(-26712, -120216, -2128)); // Immortal Pleatou North
		 * HUNTING_GROUNDS.put(new int[]{18,23}, new Location(6888, -117304, -1760)); // Cave of Trials HUNTING_GROUNDS.put(new int[]{18,23}, new Location(-424, -108184, -2784)); HUNTING_GROUNDS.put(new int[]{}, new Location(-17944, 49240, -3696)); // Swampland HUNTING_GROUNDS.put(new int[]{}, new
		 * Location(-63768, 78104, -3328)); // Spider Nest HUNTING_GROUNDS.put(new int[]{8,9}, new Location(-11800, 21672, -3640)); // Near Dark Omen Cata HUNTING_GROUNDS.put(new int[]{}, new Location(26744, 64520, -3632)); // Above Elven Fortress HUNTING_GROUNDS.put(new int[]{}, new Location(22872,
		 * 57048, -3360)); HUNTING_GROUNDS.put(new int[]{14,15}, new Location(10424, 55608, -3440)); HUNTING_GROUNDS.put(new int[]{17,18}, new Location(4392, 59720, -3488)); HUNTING_GROUNDS.put(new int[]{}, new Location(-3592, 81784, -3512)); // Neutral Zone HUNTING_GROUNDS.put(new int[]{}, new
		 * Location(-89352, 37352, -2256)); // Stronghold III HUNTING_GROUNDS.put(new int[]{}, new Location(-95288, 109528, -3568)); // Orc Barracks HUNTING_GROUNDS.put(new int[]{}, new Location(-97144, 112824, -3616)); HUNTING_GROUNDS.put(new int[]{}, new Location(-93624, 115336, -3376));
		 * HUNTING_GROUNDS.put(new int[]{}, new Location(-89752, 115096, -3408)); HUNTING_GROUNDS.put(new int[]{}, new Location(-49848, 111112, -3568)); // Ruins of Agony HUNTING_GROUNDS.put(new int[]{}, new Location(-45016, 113160, -3808)); HUNTING_GROUNDS.put(new int[]{}, new Location(-21000,
		 * 139944, -3896)); // Ruins of Despair HUNTING_GROUNDS.put(new int[]{}, new Location(-18232, 144344, -3840)); HUNTING_GROUNDS.put(new int[]{}, new Location(-14392, 144200, -3616)); HUNTING_GROUNDS.put(new int[]{}, new Location(-17352, 188360, -4208)); // Wasteland HUNTING_GROUNDS.put(new
		 * int[]{}, new Location(-12904, 189752, -4160)); HUNTING_GROUNDS.put(new int[]{}, new Location(-14856, 179656, -4192)); HUNTING_GROUNDS.put(new int[]{}, new Location(55320, 141592, -2912)); // Execution Grounds HUNTING_GROUNDS.put(new int[]{}, new Location(55288, 147544, -2856));
		 * HUNTING_GROUNDS.put(new int[]{}, new Location(53800, 150776, -2480)); HUNTING_GROUNDS.put(new int[]{}, new Location(94536, 103416, -2944)); // Hardins Academy HUNTING_GROUNDS.put(new int[]{}, new Location(85432, 98936, -3616)); HUNTING_GROUNDS.put(new int[]{}, new Location(88536, 14728,
		 * -5264)); // Ivory Tower HUNTING_GROUNDS.put(new int[]{}, new Location(82408, 17832, -5264)); HUNTING_GROUNDS.put(new int[]{45,48}, new Location(93672, 27384, -3648)); // Near Ivory HUNTING_GROUNDS.put(new int[]{45,49}, new Location(81832, 3064, -2896)); // Outlaw Forest
		 * HUNTING_GROUNDS.put(new int[]{}, new Location(79640, -1192, -3664)); // Outlaw Forest high lvl HUNTING_GROUNDS.put(new int[]{}, new Location(80936, -6968, -2896)); HUNTING_GROUNDS.put(new int[]{}, new Location(84664, -4136, -3232)); HUNTING_GROUNDS.put(new int[]{}, new Location(92360,
		 * -16504, -2032)); HUNTING_GROUNDS.put(new int[]{}, new Location(99128, -17992, -2896)); HUNTING_GROUNDS.put(new int[]{}, new Location(113352, 36264, -3504)); // Enchanted Valley HUNTING_GROUNDS.put(new int[]{}, new Location(120184, 45560, -3760)); HUNTING_GROUNDS.put(new int[]{}, new
		 * Location(171192, 24200, -3392)); // Cemetary HUNTING_GROUNDS.put(new int[]{}, new Location(179128, 23416, -3168)); HUNTING_GROUNDS.put(new int[]{}, new Location(181032, 16440, -3168)); HUNTING_GROUNDS.put(new int[]{}, new Location(141000, 92264, -3520)); // Forest of Mirrors
		 * HUNTING_GROUNDS.put(new int[]{}, new Location(171688, 41256, -4920)); // Forsaken Plains HUNTING_GROUNDS.put(new int[]{}, new Location(190328, 16488, -3672)); // Forbidden Gateway HUNTING_GROUNDS.put(new int[]{}, new Location(188264, 8376, -2736)); HUNTING_GROUNDS.put(new int[]{}, new
		 * Location(182888, 27176, -3712)); HUNTING_GROUNDS.put(new int[]{65,69}, new Location(141464, -5912, -4784)); // Blazing Swamp HUNTING_GROUNDS.put(new int[]{}, new Location(146828, -12859, -4464)); HUNTING_GROUNDS.put(new int[]{}, new Location(142728, -22184, -3152));
		 * HUNTING_GROUNDS.put(new int[]{}, new Location(143256, -25784, -2080)); HUNTING_GROUNDS.put(new int[]{}, new Location(80136, -40728, -3216)); // Swamp of Screams HUNTING_GROUNDS.put(new int[]{}, new Location(89064, -52808, -2600)); // Swamp of Screams high HUNTING_GROUNDS.put(new int[]{},
		 * new Location(55304, -53032, -3320)); // Cursed Village HUNTING_GROUNDS.put(new int[]{}, new Location(89432, -115272, -3312)); // Archaic Laboratory HUNTING_GROUNDS.put(new int[]{}, new Location(93032, -120376, -4512)); // Pavel Ruins HUNTING_GROUNDS.put(new int[]{}, new Location(171976,
		 * -49352, -3552)); // Wall of Agros HUNTING_GROUNDS.put(new int[]{}, new Location(176664, 61672, -4368)); // Giants Cave HUNTING_GROUNDS.put(new int[]{}, new Location(192008, 58632, -4808)); HUNTING_GROUNDS.put(new int[]{}, new Location(142650, -108944, -3568)); // Hot Springs
		 * HUNTING_GROUNDS.put(new int[]{}, new Location(153320, 190280, -3696)); // Isle of Prayer HUNTING_GROUNDS.put(new int[]{}, new Location(75416, -123272, -3264)); // Den of Evil HUNTING_GROUNDS.put(new int[]{}, new Location(563776, -81816, -2928)); // Beast Farm
		 */
