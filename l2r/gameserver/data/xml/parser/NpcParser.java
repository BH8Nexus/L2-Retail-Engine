...
public final class NpcParser extends AbstractDirParser<NpcHolder> {
	...
	protected void readData(org.dom4j.Element rootElement) throws Exception {
		for (Iterator<org.dom4j.Element> npcIterator = rootElement.elementIterator(); npcIterator.hasNext(); ) {
			...
			for (Iterator<org.dom4j.Element> secondIterator = npcElement.elementIterator(); secondIterator.hasNext(); ) {
				...
				if (nodeName.equalsIgnoreCase("faction")) {
				...
				} else if (!Config.DONTLOADNPCDROP && nodeName.equalsIgnoreCase("rewardlist")) {
					...
					for (Iterator<org.dom4j.Element> nextIterator = secondElement.elementIterator(); nextIterator.hasNext(); ) {
						...
						// Used to determine the race skill 4416
						if (id == 4416) {
	...
	private RewardData parseReward(org.dom4j.Element rewardElement, int id) {
		...
		double chance_lifeStone = chance * Config.RATE_CHANCE_DROP_LIFES;
		double chance_enchant = chance * Config.RATE_CHANCE_DROP_ENCHANT;
		double chance_AttributeStone = chance * Config.RATE_CHANCE_DROP_BUTESTONE;

		if (chance_AttributeStone > RewardList.MAX_CHANCE)
		{
			chance_AttributeStone = RewardList.MAX_CHANCE;
		}
		if (chance_enchant > RewardList.MAX_CHANCE)
		{
			chance_enchant = RewardList.MAX_CHANCE;
		}
		if (chance_lifeStone > RewardList.MAX_CHANCE)
		{
			chance_lifeStone = RewardList.MAX_CHANCE;
		}
		...
		if (id == 1) {
			...
			else if (data.getItem().isLifeStone()) {
				data.setChance(chance_lifeStone);
			} else if (data.getItem().isEnchantScroll()) {
				data.setChance(chance_enchant);
			} else if (data.getItem().isAttributable() && data.getItem().isAttributeCrystal() && data.getItem().isAttributeEnergy() && data.getItem().isAttributeJewel()) {
				data.setChance(chance_AttributeStone);
			} else
