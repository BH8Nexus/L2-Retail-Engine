...
public final class CubicParser extends AbstractFileParser<CubicHolder>
{
	...
	{
		return new File(Config.DATAPACK_ROOT, "data/xml/cubics.xml");
	}
	...
	protected void readData(Element rootElement) throws Exception
	{
		for (Iterator<Element> iterator = rootElement.elementIterator(); iterator.hasNext();)
		{
			...
			int mAtk = Integer.parseInt(cubicElement.attributeValue("mAtk"));
			if (Config.CUBIC_MATK_MULT.containsKey(id))
			{
				mAtk *= Config.CUBIC_MATK_MULT.get(id);
			}
			else
			{
				mAtk *= Config.CUBIC_MATK_MULT.get(-1);
			}
			
			CubicTemplate template = new CubicTemplate(id, level, delay, mAtk);
