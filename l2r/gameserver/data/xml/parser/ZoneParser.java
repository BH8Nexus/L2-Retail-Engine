...
public class ZoneParser extends AbstractDirParser<ZoneHolder>
{
	...
	protected void readData(Element rootElement) throws Exception
	{
		for (Iterator<Element> iterator = rootElement.elementIterator(); iterator.hasNext();)
		{
			...
			if ("zone".equals(zoneElement.getName()) || "dynamicZone".equals(zoneElement.getName()))
			{
				...
				if ("dynamicZone".equals(zoneElement.getName()))
				{
					try
					{
						getHolder().addDynamicZoneTemplate(Integer.parseInt(zoneElement.attribute("name").getValue()), template);
					}
					catch (NumberFormatException nfe)
					{
						_log.warn("Dynamic Zones accept only Integer values as name: ", nfe);
					}
				}
				else
				{
