...
public final class SkillAcquireParser extends AbstractDirParser<SkillAcquireHolder>
{
	...
	{
		return new File(Config.DATAPACK_ROOT, "data/xml/stats/skill_tree/");
	}
	...
	protected void readData(Element rootElement) throws Exception
	{
		...
		for (Iterator<Element> iterator = rootElement.elementIterator("customClassSkillTree"); iterator.hasNext();)
		{
			TIntObjectHashMap<List<SkillLearn>> map = new TIntObjectHashMap<>();
			Element nxt = iterator.next();
			for (Iterator<Element> classIterator = nxt.elementIterator("class"); classIterator.hasNext();)
	...
	private List<SkillLearn> parseSkillLearn(Element tree)
	{
		...
		for (Iterator<Element> iterator = tree.elementIterator("skill"); iterator.hasNext();)
		{
			...
			boolean isCustomSkill = (element.attributeValue("custom") != null) && Boolean.parseBoolean(element.attributeValue("custom"));
			
			if (isCustomSkill && !Config.CUSTOM_CLASS_TRANSFER_SKILLS)
			{
				continue;
			}
