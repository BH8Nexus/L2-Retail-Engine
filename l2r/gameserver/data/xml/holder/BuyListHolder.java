...
public class BuyListHolder
{
	...
	private BuyListHolder()
	{
		...
		try
		{
			final File filelists = new File(Config.DATAPACK_ROOT, "data/xml/merchant_filelists.xml");
			final DocumentBuilderFactory factory1 = DocumentBuilderFactory.newInstance();
			factory1.setValidating(false);
			factory1.setIgnoringComments(true);
			final FileInputStream fis = new FileInputStream(filelists);
			final Document doc1 = factory1.newDocumentBuilder().parse(fis);
				...
				if ("list".equalsIgnoreCase(n1.getNodeName()))
				{
					for (Node d1 = n1.getFirstChild(); d1 != null; d1 = d1.getNextSibling())
					{
						if ("file".equalsIgnoreCase(d1.getNodeName()))
						{
							...
							final File file = new File(Config.DATAPACK_ROOT, "data/xml/" + filename);
							...
							final Document doc2 = factory2.newDocumentBuilder().parse(fis2);
											...
											for (int n = 0; n < size; n++)
											{
												...
												for (Node i = d2.getFirstChild(); i != null; i = i.getNextSibling())
												{
													if ("item".equalsIgnoreCase(i.getNodeName()))
													{
														...
														final long price = i.getAttributes().getNamedItem("price") != null ? Long.parseLong(i.getAttributes().getNamedItem("price").getNodeValue()) : Math.round(template.getReferencePrice() * markup);
	...
	public boolean checkItem(ItemTemplate template)
	{
		...
		{
			for (final int i : Config.ALT_SHOP_UNALLOWED_ITEMS)
			{
	...
	public static class NpcTradeList
	{
		...
		public synchronized List<TradeItem> getItems()
		{
			...
			for (final TradeItem ti : tradeList)
			{
				// Is not it time to update the number of items in the limited trade list?
		...
		public synchronized void updateItems(List<TradeItem> buyList)
		{
			for (TradeItem ti : buyList)
			{
				...
				{
					if (ti.getCount() > ic.getCurrentValue())
					{
						int time = (int) (System.currentTimeMillis() / 60000);
						
						if ((ic.getLastRechargeTime() + ic.getRechargeTime()) <= time)
						{
							ic.setLastRechargeTime(time);
