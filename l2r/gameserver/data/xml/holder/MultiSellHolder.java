...
public class MultiSellHolder
{
	...
	public static class MultiSellListContainer
	{
		...
		private boolean bind_items_to_char = false;
		...
		public void setBindItems(boolean bool)
		{
			bind_items_to_char = bool;
		}
		
		public boolean isBindToChar()
		{
			return bind_items_to_char;
		}
			...
			if ("list".equalsIgnoreCase(n.getNodeName()))
			{
				for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
				{
					...
					else if ("config".equalsIgnoreCase(d.getNodeName()))
					{
						...
						list.setBindItems(XMLUtil.getAttributeBooleanValue(d, "bindtocharacter", false));
					}
				}
			}
		}
	...
	public void SeparateAndSend(int listId, Player player, double taxRate)
	{
		for (int i : Config.ALT_DISABLED_MULTISELL)
		{
			if (i == listId)
			{
				player.sendMessage(new CustomMessage("l2r.gameserver.data.xml.holder.MultiSellHolder.Disabled", player));
				return;
			}
		}
	...
	private MultiSellListContainer generateMultiSell(MultiSellListContainer container, Player player, double taxRate)
	{
		...
		boolean bindtoChar = container.isBindToChar();
		
		list.setShowAll(showall);
		list.setKeepEnchant(enchant);
		list.setNoTax(notax);
		list.setNoKey(nokey);
		list.setBindItems(bindtoChar);
		...
		for (MultiSellEntry origEntry : container.getEntries())
		{
			...
			else
			{
				...
				for (MultiSellIngredient ingredient : ingridients)
				{
					...
					if ((template == null) && (ingredient.getItemId() > 0))
					{
						_log.warn("Null item template for itemId: " + ingredient.getItemId());
						continue;
					}
					...
					if ((ingredient.getItemId() <= 0) || nokey || ((template != null) && template.isEquipment()))
					{
						...
						else if (ingredient.getItemId() == ItemTemplate.ITEM_ID_DIVISION_POINTS)
						{
							if (!itms.contains(ingredient.getItemId()) && (player.getDivisionPoints() >= ingredient.getItemCount()))
							{
								itms.add(ingredient.getItemId());
							}
							continue;
						...
						for (final ItemInstance item : items)
						{
							if ((item.getItemId() == ingredient.getItemId()) && item.canBeExchanged(player))
							{
								...
								{
									if (enchant && (template != null) && template.canBeEnchanted(true))
									{
