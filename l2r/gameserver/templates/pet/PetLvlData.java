package l2r.gameserver.templates.pet;

import l2r.gameserver.templates.StatsSet;

public class PetLvlData
{
	...
	private final int _accCombat;
	private final int _evasion;
	private final int _criticalRate;
	
	public PetLvlData(StatsSet set)
	{
		...
		_pAtkOnRide = set.getDouble("p_atk_on_ride", 0.0D);
		_mAtkOnRide = set.getDouble("m_attack_on_ride", 0.0D);
		_maxHpOnRide = set.getInteger("max_hp_on_ride", 0);
		_maxMpOnRide = set.getInteger("max_mp_on_ride", 0);
		_accCombat = set.getInteger("acc", 135);
		_evasion = set.getInteger("eva", 130);
		_criticalRate = set.getInteger("crit", 44);
	}
