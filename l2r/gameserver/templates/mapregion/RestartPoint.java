...
public class RestartPoint
{
	...
	public String getNameLoc()
	{
		...
		if(getName().equalsIgnoreCase("[hunter_town]"))
			return "Hunter town";
		if(getName().equalsIgnoreCase("[ironcastle_inner_oasis]"))
			return "Hellbound";
		if(getName().equalsIgnoreCase("[primeval_island]"))
			return "Primeval Island";
		if(getName().equalsIgnoreCase("[union_base_of_kserth]") || getName().equalsIgnoreCase("[inner_annihilation01]") || getName().equalsIgnoreCase("[inner_annihilation02]"))
			return "Gracia Area";
		return getName();
	}
