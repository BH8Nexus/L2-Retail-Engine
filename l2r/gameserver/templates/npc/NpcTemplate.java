package l2r.gameserver.templates.npc;
...
public final class NpcTemplate extends CharTemplate
{
	...
	// not used - public final String sex;
	public final int level;
	public final long rewardExp;
	public final int rewardSp;
	public final double rewardRp;
	...
	public boolean noInterractionDistance;
	private final StatsSet _AIParams;
	
	/** fixed skills */
	private int race = 0;
	private final int _castleId;
	private final int _CustomclanHallId;
	...
	private final List<MinionData> _minions = Collections.emptyList();
	...
	public NpcTemplate(StatsSet set)
	{
		...
		rewardRp = set.getDouble("rewardRp");
		...
		_CustomclanHallId = set.getInteger("custom_clanhall_id", 0);
		_AIParams = (StatsSet) set.getObject("aiParams", StatsSet.EMPTY);
		noInterractionDistance = set.getBool("noInterractionDistance", false);
	...
	/**
	* Creates a new NPC instance. For it, you should call (in that order):
	* <br> setSpawnedLoc (required)
	* <br> setReflection (if reflection is not basic)
	* <br> setChampion (optional)
	* <br> setCurrentHpMp (if setChampion was called)
	* <br> spawnMe (take getSpawnedLoc as a parameter)
	* @return
	*/
	...
	private void setType(String type)
	{
		...
		try
		{
			classType = (Class<NpcInstance>) Class.forName("l2r.gameserver.model.instances." + type + "Instance");
		}
		...
		// TODO [G1ta0] make the field in the respective classe
		isRaid = isInstanceOf(RaidBossInstance.class) && !isInstanceOf(ReflectionBossInstance.class);
		isBoss = isInstanceOf(BossInstance.class) || isInstanceOf(ReflectionBossInstance.class);
	}
	...
	private void setAI(String ai)
	{
		...
		try
		{
			classAI = (Class<CharacterAI>) Class.forName("l2r.gameserver.ai." + ai);
		}
		catch (ClassNotFoundException e)
		{
	...
	public int getCustomClanHallId()
	{
		return _CustomclanHallId;
	}
