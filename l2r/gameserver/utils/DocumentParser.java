...
{
	protected static final Logger _log = LoggerFactory.getLogger(DocumentParser.class);
	...
	protected boolean _isNamespaceAware = true;
	protected boolean _isIgnoringComments = true;
	...
	{
		parseFile(Config.findResource(path));
	}
	...
	protected void parseFile(File f)
	{
		...
		dbf.setNamespaceAware(_isNamespaceAware);
		dbf.setValidating(_isValidating);
		dbf.setIgnoringComments(_isIgnoringComments);
	...
	{
		return parseDirectory(Config.findResource(path), false);
	}
	...
	{
		return parseDirectory(Config.findResource(path), recursive);
	}
