...
import l2r.gameserver.network.clientpackets.RequestExSendPost;
...
public class Log
{
	...
	public static final String MailRecivePayment = "MailReceivePayment";
	...
	{
		LOG_ENTER_WORLD.info(player.toString() + " Entered Game. Ip: " + player.getIP() + " HWID: " + player.getHWID() + " System Version: "); // + player.getClient().getPatchVersion());
	}
	
	public static void logLeftGame(Player player, String leaveType)
	{
		final long minutesInGame = TimeUnit.MILLISECONDS.toMinutes(System.currentTimeMillis() - player.getOnlineTime());
	...
	/*
	 * public static void logStream(String msg) { if (ConfigHolder.getBool("AllowStreamLogs")) { LOG_STREAM.info(msg); } }
	 */
	...
	public static void logItemActions(String actionTitle, ItemActionLog... actions)
	{
		...
		if (containsItemId(actions, 57)) // < = Fix again
		{
	...
	public static void item(String logType, Object source, Object target, ItemInstance item, String log)
	{
		StringBuilder output = new StringBuilder().append(logType).append(" From: ").append(source == null ? "NULL" : source.toString()).append(" -> ").append(target == null ? "NULL" : target.toString()).append(" ").append(item).append(" Log: ").append(log);
		_logItems.info(output.toString());
		/*
		 * if (item != null && item.getItemId() == 57) _logAdena.info(output.toString());
		 */
	}
