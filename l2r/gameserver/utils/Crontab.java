...
/**
 * Клас для парсинга и анализа даты и времени, представленного в формате Crontab http://ru.wikipedia.org/wiki/Cron * * * * * выполняемая команда - - - - - | | | | | | | | | ----- День недели (0 - 7) (Воскресенье =0 или =7) | | | ------- Месяц (1 - 12) | | --------- День (1 - 31) | ----------- Час (0
 * - 23) ------------- Минута (0 - 59)
 */
public class Crontab
{
	...
	private GArray<Integer> _Minutes = new GArray<>();
	private GArray<Integer> _Hours = new GArray<>();
	private GArray<Integer> _DaysInMonth = new GArray<>();
	private GArray<Integer> _Month = new GArray<>();
	private GArray<Integer> _DaysInWeek = new GArray<>();
	...
	public Crontab(String line)
	{
		...
		_Month = parseRangeParam(params[3], MONTHSPERYEAR, MONTHSMIN); // 1 = jan
		_DaysInWeek = parseRangeParam(params[4], DAYSPERWEEK, DAYSPERWEEKMIN);
	}
	
	/**
	 * @param param a range
	 * @param timelength MINUTESPERHOUR, HOURESPERDAY, DAYSPERWEEK, MONTHSPERYEAR
	 * @return
	 */
	private static GArray<Integer> parseRangeParam(String param, int timelength, int minlength)
	{
		...
		for (String element : paramarray)
		{
			...
			if (element.indexOf("/") != -1)
			{
				...
				{
					if ((a % Integer.parseInt(element.substring(element.indexOf("/") + 1))) == 0)
					{
						if (a == timelength)
						{
			...
			{
				rangeitems.append(fillRange(element));
			}
		}
		
		String[] values = rangeitems.toString().split(",");
		GArray<Integer> result = new GArray<>();
	...
	public boolean canRunAt(Calendar cal)
	{
		...
		if (_Minutes.contains(minute) && _Hours.contains(hour) && _DaysInMonth.contains(day) && _Month.contains(month) && _DaysInWeek.contains(dayOfWeek))
		{
			return true;
		}
	 ...
	 * @param time время, относительно которого выполняется расчет
	 * @return время в мс
	 */
	public long timeNextUsage(long time)
	{
		...
		// System.out.println("Source date: " + cal.getTime().toString() + " day of week:" + cal.get(Calendar.DAY_OF_WEEK));
		
		// System.out.println("Minutes[]: " + _Minutes.toString());
		cal.set(Calendar.MINUTE, brute(_Minutes, cal.get(Calendar.MINUTE), MINUTESMIN, MINUTESPERHOUR));
		...
		cal.set(Calendar.HOUR_OF_DAY, brute(_Hours, cal.get(Calendar.HOUR_OF_DAY), HOURESMIN, HOURESPERDAY));
		...
		cal.set(Calendar.DAY_OF_MONTH, brute(_DaysInMonth, cal.get(Calendar.DAY_OF_MONTH), DAYSMIN, DAYSPERMONTH));
		...
		cal.set(Calendar.MONTH, brute(_Month, cal.get(Calendar.MONTH) + 1, MONTHSMIN, MONTHSPERYEAR) - 1);
		...
		cal.set(Calendar.DAY_OF_WEEK, brute(_DaysInWeek, cal.get(Calendar.DAY_OF_WEEK), DAYSPERWEEKMIN, DAYSPERWEEK));
		...
		// System.out.println("1Modified unixtime date: " + cal.getTimeInMillis());
		...
		// System.out.println("2Modified unixtime date: " + cal.getTimeInMillis());
