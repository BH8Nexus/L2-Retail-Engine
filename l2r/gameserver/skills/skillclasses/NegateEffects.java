...
{
	private final Map<EffectType, Integer> _negateEffects = new HashMap<EffectType, Integer>();
	private final Map<String, Integer> _negateStackType = new HashMap<String, Integer>();
	...
	public NegateEffects(StatsSet set)
	{
		...
		for (String element : negateEffectsString)
		{
			if (!element.isEmpty())
			{
		...
		for (String element : negateStackTypeString)
		{
			if (!element.isEmpty())
			{
				String[] entry = element.split(":");
	...
	private void negateEffectAtPower(Creature target, EffectType type, int power)
	{
		for (Effect e : target.getEffectList().getAllEffects())
		{
			...
			// If the buff is higher than the skill of Cancel, then there is a chance that this buff will not be removed
