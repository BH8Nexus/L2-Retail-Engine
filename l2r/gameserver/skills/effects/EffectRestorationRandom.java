...
public class EffectRestorationRandom extends Effect
{
	...
	public void onStart()
	{
		...
		/*
		 * if (i < chances.length) { List<Item> itemList = items.get(i); for (Item item : itemList) { ItemFunctions.addItem((Playable) getEffected(), item.itemId, item.count, true); } } else { getEffected().sendPacket(SystemMsg.THERE_WAS_NOTHING_FOUND_INSIDE); } // иначе ничего не выдаем }
		 */
		if (i < chances.length)
		{
			...
			for (Item item : itemList)
			{
				...
				{
					// getEffected().getPlayer().getInventory().addItem(item.itemId, item.count, true);
					ItemFunctions.addItem((Playable) getEffected(), item.itemId, item.count, true, "EffectRestorationRandom");
				}
				else
				{
					((PetInstance) getEffected()).getInventory().addItem(item.itemId, item.count, true);
				}
			}
		}
		else
