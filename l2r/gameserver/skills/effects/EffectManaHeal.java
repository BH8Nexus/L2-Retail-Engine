...
public class EffectManaHeal extends Effect
{
	...
	public void onStart()
	{
		...
		if(getEffector() != getEffected())
			getEffected().sendPacket(new SystemMessage2(SystemMsg.S2_MP_HAS_BEEN_RESTORED_BY_C1).addName(getEffector()).addInteger(Math.round(addToMp)));
		else
			getEffected().sendPacket(new SystemMessage2(SystemMsg.S1_MP_HAS_BEEN_RESTORED).addInteger(Math.round(addToMp)));
