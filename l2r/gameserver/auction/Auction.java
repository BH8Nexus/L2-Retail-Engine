...
public class Auction
{
	...
	public void removePlayerAuction(int page, boolean admin)
	{
		...
		{
			ThreadPoolManager.getInstance().schedule(new EndAuction(false, true, page, false), 1000);
		}
	}
	...
	public class EndAuction implements Runnable
	{
		...
		public void run()
		{
			...
			if (_cancel)
			{
				...
				letter1.setReceiverName(CharacterDAO.getNameByObjectId(_sellerId));
				...
				letter1.setType(Mail.SenderType.NONE);
			...
			else if (_buyout && !_cancel)
			{
				if (_sellerId != _lastBidder)
				{
					...
					letter1.setSenderId(1);
					letter1.setSenderName("Auction");
					letter1.setReceiverId(_lastBidder);
					CharacterDAO.getInstance();
					letter1.setReceiverName(CharacterDAO.getNameByObjectId(_lastBidder));
					letter1.setTopic("You buyout an item from auction!");
					CharacterDAO.getInstance();
					letter1.setBody("You buyout " + (_item.getCount() > 1 ? _item.getCount() + " " : "") + _item.getName() + " from " + CharacterDAO.getNameByObjectId(getSellerId()) + "'s auction for " + _highestBid + " adena.");
					letter1.addAttachment(_item);
					letter1.setType(Mail.SenderType.NONE);
					letter1.setUnread(true);
					letter1.setExpireTime((720 * 3600) + (int) (System.currentTimeMillis() / 1000L));
				...
				ItemInstance sellerItem = new ItemInstance(57);
				letter2.setSenderId(1);
				letter2.setSenderName("Auction");
				letter2.setReceiverId(_sellerId);
				CharacterDAO.getInstance();
				letter2.setReceiverName(CharacterDAO.getNameByObjectId(_sellerId));
				letter2.setTopic("You sold an item in auction!");
				CharacterDAO.getInstance();
				letter2.setBody("You sold " + (_item.getCount() > 1 ? _item.getCount() + " " : "") + _item.getName() + " to " + CharacterDAO.getNameByObjectId(getLastBidder()) + " through auction for " + _buyOutPrice + " adena.");
				letter2.setType(Mail.SenderType.NONE);
				...
				sellerItem.save();
				letter2.addAttachment(sellerItem);
				letter2.save();
			...
			else if ((_sellerId != _lastBidder) && !_cancel && !_buyout && !_admin)
			{
				...
				CharacterDAO.getInstance();
				letter1.setReceiverName(CharacterDAO.getNameByObjectId(_lastBidder));
				letter1.setTopic("You bought an item from auction!");
				CharacterDAO.getInstance();
				letter1.setBody("You bought " + (_item.getCount() > 1 ? _item.getCount() + " " : "") + _item.getName() + " from " + CharacterDAO.getNameByObjectId(getSellerId()) + "'s auction for " + _highestBid + " adena.");
				letter1.addAttachment(_item);
				letter1.setType(Mail.SenderType.NONE);
				letter1.setUnread(true);
				letter1.setExpireTime((720 * 3600) + (int) (System.currentTimeMillis() / 1000L));
				...
				letter2.setTopic("You sold an item in auction!");
				CharacterDAO.getInstance();
				letter2.setBody("You sold " + (_item.getCount() > 1 ? _item.getCount() + " " : "") + _item.getName() + " to " + CharacterDAO.getNameByObjectId(getLastBidder()) + " through auction for " + _highestBid + " adena.");
				letter2.setType(Mail.SenderType.NONE);
				letter2.setUnread(true);
				letter2.setExpireTime((720 * 3600) + (int) (System.currentTimeMillis() / 1000L));
			...
			else
			{
				...
				if (_admin && (getAllBidds() != 0) && (_lastBidder != _sellerId))
				{
					...
					ItemInstance overBidItem = new ItemInstance(57);
					letter.setSenderId(1);
					letter.setSenderName("Auction");
					letter.setReceiverId(_lastBidder);
					CharacterDAO.getInstance();
					letter.setReceiverName(CharacterDAO.getNameByObjectId(_lastBidder));
					letter.setTopic("Return of bid!");
					letter.setBody("Auction was canceled by Admin. Your adena is returned.");
				...
				CharacterDAO.getInstance();
				letter3.setReceiverName(CharacterDAO.getNameByObjectId(_sellerId));
				letter3.setTopic("Auction finished. Your item was not sold.");
				letter3.setBody("The auction for the " + (_item.getCount() > 1 ? _item.getCount() + " " : "") + _item.getName() + " has finished. No one was interested from your item, therefore it was not sold.");
				letter3.addAttachment(_item);
				letter3.setType(Mail.SenderType.NONE);
