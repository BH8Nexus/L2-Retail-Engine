...
public class AuctionManager
{
	...
	public static void usebypass(Player player, String bypass)
	{
		...
		/*
		 * else if(bypass.startsWith("_bbs_Auction:itemlink")) { String[] cm = bypass.split(" "); if(cm.length < 3) sendItemLink(player, Integer.parseInt(cm[1])); }
		 */
		else
		{
			getAllAuctions(player, 1);
	...
	public static void getAllAuctions(Player pl, int page)
	{
		...
		for (String s : new String[]
		{
			...
			"ItemName(Ascending);",
			"ItemName(Descending);",
			"TimeRemaning(Ascending);",
			"TimeRemaning(Descending);",
			"BidPrice(Ascending);",
			"BidPrice(Descending);"
		})
		{
		...
		for (Auction auc : getSorttedAuctions(_playerSortBy.get(pl.getObjectId()), auctions))
		{
			...
			if (auc.isGolden())
			{
				...
				htmltosend = htmltosend.replaceAll("%timeleft" + clansvisual + "%", "- <font color=\"0099FF\">" + Math.max(auc.getTimeLeft() / 60000, 0) + "</font> <font color=LEVEL>min.</font>");
				...
				htmltosend = htmltosend.replaceAll("%width" + clansvisual + "%", "275");
			...
			else
			{
				...
				htmltosend = htmltosend.replaceAll("%timeleft" + clansvisual + "%", "- <font color=\"0099FF\">" + Math.max(auc.getTimeLeft() / 60000, 0) + "</font> min.");
				htmltosend = htmltosend.replaceAll("%button" + clansvisual + "%", "<button value=\"\" action=\"bypass _bbs_Auction:showAuction " + auc.getAuctionId() + "\" width=32 height=32 back=\"L2UI_CT1.MiniMap_DF_PlusBtn_Blue_Over\" fore=\"L2UI_CT1.MiniMap_DF_PlusBtn_Blue\">");
				htmltosend = htmltosend.replaceAll("%width" + clansvisual + "%", "275");
				CharacterDAO.getInstance();
				htmltosend = htmltosend.replaceAll("%auctionerName" + clansvisual + "%", "<font color=\"FF293D\">[" + CharacterDAO.getNameByObjectId(auc.getSellerId()) + "]</font>");
				htmltosend = htmltosend.replaceAll("%itemInfo" + clansvisual + "%", "<button value=\"\" action=\"bypass _bbs_Auction:itemInfo " + item.getObjectId() + "\" width=16 height=16 back=\"L2UI_CH3.aboutotpicon\" fore=\"L2UI_CH3.aboutotpicon_over\">");
		...
		if (clansvisual < 12)
		{
			for (int d = clansvisual + 1; d != 13; d++)
			{
				...
				htmltosend = htmltosend.replaceAll("%width" + d + "%", "142");
	...
	public static void playerAuctionItems(Player player, int page)
	{
		...
		for (Auction auc : getPlayerListAuctions(player))
		{
			...
			html = html.replaceAll("%timeleft" + clansvisual + "%", "Time left: <font color=\"0099FF\">" + Math.max(auc.getTimeLeft() / 60000, 0) + "</font> min.");
			html = html.replaceAll("%maxbid" + clansvisual + "%", "Bid: <font color=\"6AA121\">" + maxBid + "</font>");
			html = html.replaceAll("%buyout" + clansvisual + "%", "Buyout: <font color=\"3EA99F\">" + buyoutprice + "</font>");
		...
		if (clansvisual < 12)
		{
			for (int d = clansvisual + 1; d != 13; d++)
			{
				...
				html = html.replaceAll("%width" + d + "%", "146");
	...
	public static synchronized void addItemToAuction(Player requester, int objid, long count, long startBid, long buyoutPrice, int hours)
	{
		...
		/*
		 * LETS DISABLE IT... double tax = hours / 24 / 100; // Divide by 24 and then divide by 100 to turn it into %. tax *= startBid; // Now get those % from the initial bid as tax. if (tax < 1) tax = 1; if(requester.getInventory().reduceAdena((long) tax)) { // No Idea if this work.... Castle
		 * nearestCastle = ResidenceHolder.getInstance().getResidenceByCoord(Castle.class, requester.getX(), requester.getY(), requester.getZ(), requester.getReflection()); if (nearestCastle != null && requester.getCastle() != null && requester.getCastle().getId() > 0)
		 * nearestCastle.addToTreasuryNoTax((long) tax, false, false); } else { requester.sendMessage(new CustomMessage("l2r.gameserver.auction.auctionmanager.tax", requester).addNumber(tax)); return; }
		 */
		...
		if (item.getCount() >= count)
		{
			...
			try
			{
				...
				items.setOwnerId(Config.AUCTION_DUMMY_CHARACTER);
	...
	public static void restoreAuctions()
	{
		...
		_log.info("Auction: Loaded " + count + " auctions. ");
	}
	...
	/*
	 * no idea why its not working... debug when i have more time... private static void sendItemLink(Player player, int itemobjectId) { player = player.getClient().getConnection().getClient().getActiveChar(); ItemInfo item = ItemInfoCache.getInstance().get(itemobjectId); if (item != null)
	...
	private static void sendItemInfo(Player player, int itemobjectId)
	{
		...
		CharacterDAO.getInstance();
		html = html.replaceAll("%seller%", String.valueOf(CharacterDAO.getNameByObjectId(auc.getSellerId())));
		html = html.replaceAll("%icon%", "icon." + item.getTemplate().getIcon());
