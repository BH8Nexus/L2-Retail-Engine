...
public class SkillTreeTable
{
	...
	public static Map<Integer, List<EnchantSkillLearn>> _enchant = new ConcurrentHashMap<>(530);
		...
		if (learn.getMinLevel() > (player.getLevel() + 10))
		{
			...
			for (int i = skill.getBaseLevel(); i != 0; i--)
			{
				...
				if (newSkill != null)
				{
					...
					// Update shortcuts for skills on delevel.
					if ((player.getAllShortCuts().size() > 0) && (newSkill.getLevel() > 1))
					{
						for (ShortCut sc : player.getAllShortCuts())
						{
							if ((sc.getId() == newSkill.getId()) && (sc.getType() == ShortCut.TYPE_SKILL))
							{
								ShortCut newsc = new ShortCut(sc.getSlot(), sc.getPage(), sc.getType(), sc.getId(), newSkill.getLevel(), 1);
	...
	public static int unconvertEnchantLevel(int skillId, int level)
	{
		EnchantSkillLearn sl = SkillTreeTable.getSkillEnchant(skillId, 101);
		if ((sl == null) || (level <= sl.getBaseLevel()))
		{
			return level;
		}
		
		level -= sl.getBaseLevel() - 1;
		int route = ((level) / sl.getMaxLevel());
		level %= sl.getMaxLevel();
		level += 1;
		return level + ((route + 1) * 100);
	}
