...
public class StatFunctions
{
	private static class FuncMultRegenResting extends Func
	{
		...
		public void calc(Env env)
		{
			...
			{
				env.value *= 6.; // TODO: Ð¿ÐµÑ€ÐµÐ´ÐµÐ»Ð°Ñ‚ÑŒ ÐºÑ€Ð°Ñ�Ð¸Ð²ÐµÐµ
			}
			else
			{
				env.value *= 1.5;
	...
	private static class FuncPAtkMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			//env.value *= BaseStats.STRbonus[env.character.getSTR()] * env.character.getLevelMod();
			env.value *= BaseStats.STR.calcBonus(env.character) * env.character.getLevelMod();
		}
	}
	...
	private static class FuncMAtkMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			//double ib = BaseStats.INTbonus[env.character.getINT()];
			...
			env.value *= lvlb * lvlb * ib * ib; //Math.pow(lvlb, 2) * Math.pow(ib, 2);
		}
	}
	...
	private static class FuncMDefMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			//env.value *= BaseStats.MENbonus[env.character.getMEN()] * env.character.getLevelMod();
			env.value *= BaseStats.MEN.calcBonus(env.character) * env.character.getLevelMod();
		}
	}
	...
	private static class FuncAccuracyAdd extends Func
	{
		...
		public void calc(Env env)
		{
			...
			env.value += (Math.sqrt(env.character.getDEX()) * 6) + BaseStats.ACCURACY_EVASION_LEVEL_BONUS[env.character.getLevel()];
	...
	private static class FuncMCriticalRateMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			//env.value *= BaseStats.WITbonus[env.character.getWIT()];
			env.value *= BaseStats.WIT.calcBonus(env.character);
		}
	}
	...
	private static class FuncPCriticalRateMul extends Func
	{
		...
		public void calc(Env env)
		{
			if (!(env.character instanceof Summon))
			{
				...
				//env.value *= BaseStats.DEXbonus[env.character.getDEX()];
				env.value *= BaseStats.DEX.calcBonus(env.character);
			}
...
//				arg = 100;
//			}
//			env.value *= BaseStats.WITbonus[arg];
			env.value *= BaseStats.WIT.calcBonus(env.character);
		}
	}
	...
	private static class FuncMaxHpAdd extends Func
	{
		...
		public void calc(Env env)
		{
			...
			// Synerge - Temporary fix for HP when players delevel from a third class to lvl 20 or close. HP were not decreasing from the base level, and from lower lvls they were going to 0
	...
	private static class FuncMaxHpMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			//env.value *= BaseStats.CONbonus[env.character.getCON()];
			env.value *= BaseStats.CON.calcBonus(env.character);
		}
	}
	...
	private static class FuncMaxCpAdd extends Func
	{
		...
		public void calc(Env env)
		{
			...
			// Synerge - Temporary fix for CP when players delevel from a third class to lvl 20 or close. HP were not decreasing from the base level, and from lower lvls they were going to 0
			final int lvl = env.character.getLevel() - t.classBaseLevel;
	...
	private static class FuncMaxMpAdd extends Func
	{
		...
		public void calc(Env env)
		{
			...
			// Synerge - Temporary fix for MP when players delevel from a third class to lvl 20 or close. HP were not decreasing from the base level, and from lower lvls they were going to 0
			final int lvl = env.character.getLevel() - t.classBaseLevel;
	...
	private static class FuncPDamageResists extends Func
	{
		...
		public void calc(Env env)
		{
			...
			// TODO convert to the same system as the effects
			WeaponTemplate weapon = env.character.getActiveWeaponItem();
			if (weapon == null)
	...
	private static class FuncSDefPlayers extends Func
	{
		...
		public void calc(Env env)
		{
			...
			// env.value *= BaseStats.DEX.calcBonus(env.character);
			//env.value *= BaseStats.DEXbonus[cha.getDEX()];
			env.value *= BaseStats.DEX.calcBonus(env.character);
		}
	}
	...
	private static class FuncRunSpdLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				if (env.character.getPlayer().isGM())
					env.value = Math.min(Config.LIMIT_MOVE, env.value);
				else
					env.value = Math.min(Config.LIMIT_MOVE, env.value);
				// env.value = Math.min(Config.getLimit(Config.LIMIT_MOVE, env.character), env.value);
				//env.value = Math.min(Config.LIMIT_MOVE, env.value);
			}
		}
	}
	...
	private static class FuncPDefLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_PDEF, env.character), env.value);
				env.value = Math.min(Config.LIMIT_PDEF, env.value);
			}
		}
	}
	...
	private static class FuncMDefLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_MDEF, env.character), env.value);
				env.value = Math.min(Config.LIMIT_MDEF, env.value);
			}
		}
	}
	...
	private static class FuncPAtkLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_PATK, env.character), env.value);
				env.value = Math.min(Config.LIMIT_PATK, env.value);
			}
		}
	}
	...
	private static class FuncMAtkLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_MATK, env.character), env.value);
				env.value = Math.min(Config.LIMIT_MATK, env.value);
			}
		}
	}
	...
	private static class FuncPAtkSpdLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_PATK_SPD, env.character), env.value);
				env.value = Math.min(Config.LIMIT_PATK_SPD, env.value);
			}
		}
	}
	...
	private static class FuncMAtkSpdLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_MATK_SPD, env.character), env.value);
				env.value = Math.min(Config.LIMIT_MATK_SPD, env.value);
			}
		}
	}
	...
	private static class FuncCAtkLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_CRIT_DAM, env.character), env.value);
				env.value = Math.min(Config.LIMIT_CRIT_DAM / 2., env.value);
			}
		}
	}
	...
	private static class FuncEvasionLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_EVASION, env.character), env.value);
				env.value = Math.min(Config.LIMIT_EVASION, env.value);
			}
		}
	}
	...
	private static class FuncAccuracyLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_ACCURACY, env.character), env.value);
				env.value = Math.min(Config.LIMIT_ACCURACY, env.value);
			}
		}
	}
	...
	private static class FuncCritLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_CRIT, env.character), env.value);
				env.value = Math.min(Config.LIMIT_CRIT, env.value);
			}
		}
	}
	...
	private static class FuncMCritLimit extends Func
	{
		...
		public void calc(Env env)
		{
			if (!env.character.canOverrideCond(PcCondOverride.MAX_STATS_VALUE))
			{
				// env.value = Math.min(Config.getLimit(Config.LIMIT_MCRIT, env.character), env.value);
				env.value = Math.min(Config.LIMIT_MCRIT, env.value);
			}
		}
	}
	...
	public static void addPredefinedFuncs(Creature cha)
	{
	    ...
	    if (cha.isNpc())
	    {
	        cha.addStatFunc((Func)FuncMaxHpMul.func);
	        cha.addStatFunc((Func)FuncMaxMpMul.func);
	        cha.addStatFunc((Func)FuncMAtkMul.func);
	        cha.addStatFunc((Func)FuncPAtkMul.func);
	        cha.addStatFunc((Func)FuncPDefMul.func);
	        cha.addStatFunc((Func)FuncMDefMul.func);
	      } 
		
		//if (cha.isPlayer() || cha.isBoss() || cha.isPet()) // QQ players :(
