...
//			info.damage = 1;
		...
		if (skill != null)
		{
			final boolean skillEvaded = !skill.hasEffects() && Rnd.chance(target.calcStat(Stats.PSKILL_EVASION, 0, attacker, skill));
			if (info.shld)
			{
				if (info.damage == 1)
				{
...
//		}
	...
	public static boolean calcMCrit(double mRate)
	{
		...
		return (Rnd.get() * 100) <= Math.min(Config.LIMIT_MCRIT, mRate);
	}
...
//		}
		...
		{
			if (et == null || et.chance(100) < 0) // Эффекты с индивидуальным шансом - не дебафы (сброс цели). TODO: отдельный флаг
			{
	...
	public static double calcDamageResists(Skill skill, Creature attacker, Creature defender, double value)
	{
		...
		if (attacker.isBoss())
		{
			...
			value *= NpcAndRaidMod.RaidPAtkMul(attacker.getLevel());
		}
		else if (attacker.isRaid() || (attacker instanceof ReflectionBossInstance))
		{
			//value *= Config.RATE_RAID_ATTACK;
			value *= NpcAndRaidMod.RaidMAtkMul(attacker.getLevel());
		}

		if (defender.isBoss())
		{
			//value /= Config.RATE_EPIC_DEFENSE;
			value /= NpcAndRaidMod.RaidPDefMul(defender.getLevel());
		}
