package l2r.gameserver.facebook;
...
public final class FacebookAutoAnnouncement extends RunnableImpl
{
	...
	private static void announceNotConnectedMsg()
	{
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
		{
			...
			{
				player.sendPacket(new Say2(0, ChatType.HERO_VOICE, "Facebook", StringHolder.getNotNull(player, NOT_CONNECTED_MSG_ADDRESS, player.getName())));
			}
		}
	}
	...
	private static void announceConnectedMsg()
	{
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
		{
			...
			{
				player.sendPacket(new Say2(0, ChatType.HERO_VOICE, "Facebook", StringHolder.getNotNull(player, CONNECTED_MSG_ADDRESS, player.getName())));
			}
		}
	}
	...
	private static void announceNegativePointsMsg()
	{
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
		{
			...
			{
				player.sendPacket(new Say2(0, ChatType.HERO_VOICE, "Facebook", StringHolder.getNotNull(player, NEGATIVE_POINTS_MSG_ADDRESS, player.getName())));
			}
		}
	}
