...
public class Achievements
{
	...
	private final List<IAchievementCategory> _achievementCategories = new LinkedList<>();
	...
	public void generatePage(Player player)
	{
		...
		String ac = "";
		for (IAchievementCategory cat : _achievementCategories)
		{
	...
	public void generatePage(Player player, int category, int page)
	{
		...
		IAchievementCategory cat = _achievementCategories.stream().filter(ctg -> ctg.getCategoryId() == category).findAny().orElse(null);
		if (cat == null)
		{
		...
		for (Entry<Integer, Integer> entry : playerAchievements.entrySet())
		{
			...
			iAchievement a = getAchievement(aId, Math.max(1, nextLevel));
	...
	public void checkAchievementRewards(Player player)
	{
		synchronized (player.getAchievements())
		{
			for (Entry<Integer, Integer> arco : player.getAchievements().entrySet())
			{
				...
				iAchievement nextLevelAchievement;
				do
				{
					achievementLevel++;
	...
	public int getPointsForThisLevel(int totalPoints, int achievementId, int achievementLevel)
	{
		...
		{
			iAchievement a = getAchievement(achievementId, i);
			if (a != null)
			{
	...
	public iAchievement getAchievement(int achievementId, int achievementLevel)
	{
		for (IAchievementCategory cat : _achievementCategories)
		{
			for (iAchievement ach : cat.getAchievements())
			{
	...
	public void load()
	{
		...
		try
		{
			File file = Config.findNonCustomResource("config/achievements.xml");
					...
					if (z.getNodeName().equals("categories"))
					{
						for (Node i = z.getFirstChild(); i != null; i = i.getNextSibling())
						{
							if ("cat".equalsIgnoreCase(i.getNodeName()))
							{
								...
								_achievementCategories.add(new IAchievementCategory(categoryId, categoryName, categoryIcon, categoryDesc));
							}
						}
					}
					else if (z.getNodeName().equals("achievement"))
					{
						...
						for (Node i = z.getFirstChild(); i != null; i = i.getNextSibling())
						{
							if ("level".equalsIgnoreCase(i.getNodeName()))
							{
								...
								iAchievement achievement = new iAchievement(achievementId, level, name, achievementCategory, icon, desc, pointsToComplete, fieldType, fame);
								...
								IAchievementCategory lastCategory = _achievementCategories.stream().filter(ctg -> ctg.getCategoryId() == achievementCategory).findAny().orElse(null);
