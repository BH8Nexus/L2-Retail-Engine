...
public abstract class Effect extends RunnableImpl implements Comparable<Effect>, FuncOwner
{
	...
	protected boolean _savable;
	protected boolean _isOffensive;
	private Func[] _statFuncsOverride;
	...
	protected Effect(Env env, EffectTemplate template)
	{
		...
		_hideStartMessage = _skill.isHideStartMessage();
		...
		_cancelable = _template.isCancelable(getSkill().isCancelable());
		_savable = _template.isSaveable(getSkill().isSaveable()) && (getTimeLeft() >= Config.ALT_SAVE_EFFECTS_REMAINING_TIME);
		_isOffensive = _template.isOffensive(getSkill().isOffensive());
	...
	public void setPeriod(long timeInMilis)
	{
		_period = timeInMilis;
		_duration = _period * _count;
	}
	...
	{
		@Override
		public boolean onMagicUse(Creature actor, Skill skill, Creature target, boolean alt)
		{
			exit();
			return false;
		}
	...
	protected void onStart()
	{
		...
		if (getTemplate()._abnormalEffect == AbnormalEffect.REAL_TARGET)
		{
			getEffected().setIsRealTargeted(true);
		}
	}
	...
	protected void onExit()
	{
		...
		if (getTemplate()._abnormalEffect == AbnormalEffect.REAL_TARGET)
		{
			getEffected().setIsRealTargeted(false);
		}
	}
	...
	{
		if (_statFuncsOverride == null)
		{
			return getTemplate().getStatFuncs(this);
		}
		return _statFuncsOverride;
	}
	
	/**
	 * @param funcs: Null will set default stat funcs. Else the given will override. <br>
	 *            Use getStatFuncs() and add more funcs or remove if you wanna add/remove funcs from default.
	 */
	public void setStatFuncs(Func... funcs)
	{
		_statFuncsOverride = funcs;
	}
	...
	/**
	 * @param savable: Null will set default value
	 */
	public void setSaveable(Boolean savable)
	{
		if (savable == null)
		{
			_savable = _template.isSaveable(getSkill().isSaveable()) && (getTimeLeft() >= Config.ALT_SAVE_EFFECTS_REMAINING_TIME);
	...
	/**
	 * @param displayId: Null will set default value
	 */
	public void setDisplayId(Integer displayId)
	{
		if (displayId == null)
		{
			_displayId = getTemplate()._displayId != 0 ? getTemplate()._displayId : _skill.getDisplayId();
		}
		else
		{
	...
	/**
	 * @param displayLevel: Null will set default value
	 */
	public void setDisplayLevel(Integer displayLevel)
	{
		if (displayLevel == null)
		{
			_displayLevel = getTemplate()._displayLevel != 0 ? getTemplate()._displayLevel : _skill.getDisplayLevel();
		}
		else
		{
	...
	/**
	 * @param cancelable: Null will set default value
	 */
	...
	/**
	 * @param isOffensive: Null will set default value
	 */
