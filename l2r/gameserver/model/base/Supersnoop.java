...
public class Supersnoop
{
	...
	private final List<Player> _snoopers; // List of players snooping this player
	
	public Supersnoop(Player player)
	{
		_player = player;
		_snoopers = new ArrayList<>();
	}
	...
	private boolean sendSnoopPackets(Player snooper, Player snoopedTarget)
	{
		try
		{
			...
			snooper.sendPacket(new ExBR_PremiumState(snoopedTarget, false));
			// Macro update?
			...
			snooper.sendPacket(ClientSetTime.STATIC, new ExSetCompassZoneCode(snoopedTarget));
