...
import l2r.gameserver.listener.actor.OnAttackHitListener;
import l2r.gameserver.listener.actor.OnAttackListener;
import l2r.gameserver.listener.actor.OnCharEnterLeaveZoneListener;
...
import l2r.gameserver.listener.actor.OnStatusUpdateBroadcastListener;
import l2r.gameserver.listener.actor.ai.OnAiEventListener;
import l2r.gameserver.listener.actor.ai.OnAiIntentionListener;
...
public class CharListenerList extends ListenerList<Creature>
{
	...
	public boolean onMagicUse(Skill skill, Creature target, boolean alt)
	{
		boolean stopCast = false;
		if (!global.getListeners().isEmpty())
		{
			for (Listener<Creature> listener : global.getListeners())
			{
				...
				{
					if (((OnMagicUseListener) listener).onMagicUse(getActor(), skill, target, alt))
					{
						stopCast = true;
					}
				}
