...
import l2r.gameserver.network.serverpackets.ExNavitAdventEffect;
...
public class NevitSystem
{
	...
	{
		return (int)(/*100.0D*/ 0.013888888888888888D/ MAX_POINTS * points);
	}
		...
		public void runImpl()
		{
			...
			else
			{
				...
				if((NevitSystem.this._time % 60) == 0)
					NevitSystem.this._player.sendPacket(new ExNavitAdventTimeChange(true, NevitSystem.this._time));
			}
		}
	}
	...
	private class NevitEffectEnd extends RunnableImpl
	{
		...
		public void runImpl()
		{
			NevitSystem.this._player.sendPacket(new ExNavitAdventEffect(0));
			NevitSystem.this._player.sendPacket(new ExNavitAdventPointInfo(NevitSystem.this._points));
			NevitSystem.this._player.sendPacket(SystemMsg.NEVITS_BLESSING_HAS_ENDED_CONTINUE_YOUR_JOURNEY_AND_YOU_WILL_SURELY_MEET_HIS_FAVOR_AGAIN_SOMETIME_SOON);
			NevitSystem.this._player.stopAbnormalEffect(AbnormalEffect.S_NAVIT);
