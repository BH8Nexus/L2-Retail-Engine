...
public class BookMarkList
{
	...
	public boolean tryTeleport(int slot)
	{
		...
		if (!owner.canOverrideCond(PcCondOverride.TELEPORT_BOOKMARK_CONDITIONS) && (Functions.removeItem(owner, 13016, 1, "BookMarkTeleport") != 1))
		{
	...
	public boolean add(Location loc, String aname, String aacronym, int aiconId, boolean takeFlag)
	{
		...
		{
			if (owner.canOverrideCond(PcCondOverride.TELEPORT_BOOKMARK_CONDITIONS))
			{
				capacity++;
			}
			else
		...
		if (takeFlag && !owner.canOverrideCond(PcCondOverride.TELEPORT_BOOKMARK_CONDITIONS))
		{
	...
	public static boolean checkFirstConditions(Player player)
	{
		...
		if (player.canOverrideCond(PcCondOverride.TELEPORT_BOOKMARK_CONDITIONS))
		{
			return true;
		}
