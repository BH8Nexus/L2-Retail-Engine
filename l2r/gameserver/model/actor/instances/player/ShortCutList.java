...
public class ShortCutList
{
	...
	private synchronized void registerShortCutInDb(ShortCut shortcut, ShortCut oldShortCut)
	{
		...
		{
			_log.error("Could not store shortcuts (slot=" + shortcut.getSlot() + ", page=" + shortcut.getPage() + ", type=" + shortcut.getType() + ", id=" + shortcut.getId() + ", level=" + shortcut.getLevel() + ", characterType=" + shortcut.getCharacterType() + "):", e);
		}
		finally
		{
	...
	/**
	 * @return A map representing all the shortcuts before being cleaned and the new added.
	 */
	public Map<Integer, ShortCut> restore()
	{
		Map<Integer, ShortCut> oldShortcuts = new ConcurrentHashMap<>();
		oldShortcuts.putAll(_shortCuts);
