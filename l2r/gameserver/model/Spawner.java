...
import l2r.gameserver.taskmanager.SpawnTaskManager;
...
public abstract class Spawner extends EventOwner implements Cloneable
{
	...
	{
		final List<NpcInstance> spawnedNpcs = new ArrayList<>();
		while ((_currentCount + _scheduledCount) < _maximumCount)
		{
			try
			{
	...
	protected NpcInstance initNpc0(NpcInstance mob, Location newLoc, boolean spawn, MultiValueSet<String> set)
	{
		...
		// Synchronize Minion Spawn + Template
		...
		// Ð¯Ð²Ð»Ñ�ÐµÑ‚Ñ�Ñ� Ð»Ð¸ Ð¼Ð¾Ð± "Ð¿Ð¾Ð´Ð·ÐµÐ¼Ð½Ñ‹Ð¼" Ð¼Ð¾Ð±Ð¾Ð¼?
		...
		if (spawn)
		{
			// Ð¡Ð¿Ð°Ð²Ð½Ð¸Ñ‚Ñ�Ñ� Ð² ÑƒÐºÐ°Ð·Ð°Ð½Ð½Ð¾Ð¼ Ð¾Ñ‚Ñ€Ð°Ð¶ÐµÐ½Ð¸Ð¸
			...
			if (mob.isMonster())
			{
				if (mob.getChampionTemplate() != null)
				{
					...
					mob.setTitle(mob.getTemplate().title); // Restore title to default.
				}
				
				if (ChampionTemplateTable.ENABLE_EXT_CHAMPION_MODE)
				{
					if (!mob.isRaid())
					{
						int rnd = Rnd.get(ChampionTemplateTable.EXT_CHAMPION_MODE_MAX_ROLL_VALUE);
						
						for (ChampionTemplate ct : (ChampionTemplateTable.getSpecificChampionTemplates(mob.getNpcId()) != null ? ChampionTemplateTable.getSpecificChampionTemplates(mob.getNpcId()) : ChampionTemplateTable.getChampionTemplates()))
						{
							if ((ct.spawnsInInstances || (mob.getReflection().getId() == 0)) // if template allows to be spawned here
								&& (rnd >= ct.minChance) && (rnd <= ct.maxChance // If this template is whitin the dice roll
								) && (mob.getLevel() >= ct.minLevel) && (mob.getLevel() <= ct.maxLevel)) // If this template is whitin the mob level range
							{
								mob.setChampionTemplate(ct);
								mob.setTitle(mob.getChampionTemplate().title);
								break;
							}
						}
					}
