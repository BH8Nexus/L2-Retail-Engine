...
import l2r.gameserver.model.items.listeners.StatsListener;
...
public abstract class Inventory extends ItemContainer
{
	...
	protected void onAddItem(ItemInstance item)
	{
		...
		if (getActor() != null)
		{
			if (getActor().isPlayer() && (getActor().getPlayer() != null) && (getActor().getPlayer().getPet() == null))
			{
				Log.item(Log.Create, this, getActor().getPlayer(), item, Thread.currentThread().getStackTrace()[3].toString());
			}
			else
			{
				Log.item(Log.Create, this, getActor(), item, Thread.currentThread().getStackTrace()[3].toString());
			}
		}
	...
	protected void onModifyItem(ItemInstance item)
	{
		...
		if (getActor() != null)
		{
			if (getActor().isPlayer() && (getActor().getPlayer() != null) && (getActor().getPlayer().getPet() == null))
			{
				Log.item(Log.Modifiy, this, getActor().getPlayer(), item, Thread.currentThread().getStackTrace()[3].toString());
			}
			else
			{
				Log.item(Log.Modifiy, this, getActor(), item, Thread.currentThread().getStackTrace()[3].toString());
			}
		}
	...
	protected void onDestroyItem(ItemInstance item)
	{
		if (getActor() != null)
		{
			if (getActor().isPlayer() && (getActor().getPlayer() != null) && (getActor().getPlayer().getPet() == null))
			{
				Log.item(Log.Delete, this, getActor().getPlayer(), item, Thread.currentThread().getStackTrace()[3].toString());
			}
			else
			{
				Log.item(Log.Delete, this, getActor(), item, Thread.currentThread().getStackTrace()[3].toString());
			}
		}
	...
	/**
	 * Π�Π°Ρ…ΠΎΠ΄ΠΈΡ‚ ΠΈ Π²ΠΎΠ·Π²Ρ€Π°Ρ‰Π°ΠµΡ‚ ΠΏΡƒΡ�Ρ‚ΠΎΠΉ Ρ�Π»ΠΎΡ‚ Π² ΠΈΠ½Π²ΠµΠ½Ρ‚Π°Ρ€Πµ.
	 */
	private int findSlot()
	{
		...
		return slot; // Ρ�Π»ΠΎΡ‚ Π½Πµ Π·Π°Π½Ρ�Ρ‚, Π²ΠΎΠ·Π²Ρ€Π°Ρ‰Π°ΠµΠΌ
	}
	...
	/*
	 * public int getPaperdollItemDisplayId(int slot) { ItemInstance item = getPaperdollItem(slot); if (item != null) { if (item.getVisualItemId() > 0) { return item.getVisualItemId(); } if (item.getdisplayId() > 0) { return item.getdisplayId(); } return item.getItemId(); } else if (slot ==
	 * PAPERDOLL_HAIR) { item = _paperdoll[PAPERDOLL_DHAIR]; if (item != null) { if (item.getVisualItemId() > 0) { return item.getVisualItemId(); } if (item.getdisplayId() > 0) { return item.getdisplayId(); } return item.getItemId(); } } return 0; }
	 */
	...
	private void unEquipItemInBodySlot(int bodySlot, ItemInstance item)
	{
		...
		switch (bodySlot)
		{
			...
			case ItemTemplate.SLOT_R_BRACELET:
				...
				// Π�Ρ€ΠΈ Ρ�Π½Ρ�Ρ‚ΠΈΠΈ ΠΏΡ€Π°Π²ΠΎΠ³ΠΎ Π±Ρ€Π°Ρ�Π»ΠµΡ‚Π°, Ρ�Π½ΠΈΠΌΠ°ΠµΠΌ ΠΈ Ρ‚Π°Π»ΠΈΡ�ΠΌΠ°Π½Ρ‹ Ρ‚ΠΎΠ¶Πµ
	...
	// Test Code is need to test
	public void restoreCursedWeapon()
	{
		Playable owner = getActor();
		if ((owner == null) || !owner.isPlayer())
		{
			return;
		}
		
		for (ItemInstance i : getItems())
		{
			if (i.isCursed())
			{
	...
	{
		return quest ? super.getSize() : (super.getSize() - getQuestSize());
	}
