...
import l2r.gameserver.data.xml.holder.ItemChanceHolder;
...
public class ItemsHolder
{
	...
	{
		if ((_count < 1) || (ItemHolder.getInstance().getTemplate(getId()) == null))
		{
			return null;
		}
		
		ItemInstance item = new ItemInstance((long) IdFactory.getInstance().getNextId(), getId());
	...
	public void giveItem(Playable playable, boolean notify)
	{
		...
		{
			// playable.getPlayer().getInventory().addItem(_id, _count, notify);
			ItemFunctions.addItem(playable.getPlayer(), _id, getCount(), notify, "ItemHold");
		}
		else if (playable.isPet())
		{
			((PetInstance) playable).getInventory().addItem(_id, _count, notify);
		}
	}
