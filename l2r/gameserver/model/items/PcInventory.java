...
import l2r.commons.collections.CollectionUtils;
...
public class PcInventory extends Inventory
{
	...
	public long getDC()
	{
		ItemInstance _dc = getItemByItemId(57); // Config.DONATEID // is need to add
		if (_dc == null)
		{
			return 0;
		}
	...
	public long getItemCountByID(int id)
	{
		ItemInstance _item = getItemByItemId(id);
		if (_item == null)
		{
			return 0;
		}
	...
	public ItemInstance addCustomItem(int id, long amount, String log)
	{
		return addItem(id, amount, log);
	}
	...
	public boolean reduceAuctionCurrency(int coinId, long quantity, String log)
	{
		return destroyItemByItemId(coinId, quantity, log);
	}
	...
	protected void onDestroyItem(ItemInstance item)
	{
		...
		if ((item.getItemId() >= 31653) && (item.getItemId() <= 31655))
		{
			System.out.println("REEEEE --- EEE --");
			getActor().getPlayer().unsetVar("vipticket");
		}
	...
	protected class LifeTimeTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			synchronized (item)
			{
				...
				if ((item.getItemId() >= 31653) && (item.getItemId() <= 31655) && !getActor().getPlayer().getVarB("vipticket", false))
				{
					System.out.println("REEEEE --- EEE --");
					getActor().sendMessage("VIP Ticket is active");
					getActor().getPlayer().setVar("vipticket", "true");
				}
				if (left <= 0)
				{
