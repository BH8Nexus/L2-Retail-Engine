...
public final class WorldRegion implements Iterable<GameObject>
{
	...
	void addToPlayers(GameObject object)
	{
		if (object == null)
		{
			return;
		...
		for (GameObject obj : this)
		{
			if (((obj.getObjectId() == oid) && !(object instanceof FenceInstance)) || (obj.getReflectionId() != rid))
			{
				continue;
			}
			...
			if (player != null)
			{
				player.sendPacket(player.addVisibleObject(obj));
			}
			...
			{
				p = (Player) obj;
				p.sendPacket(p.addVisibleObject(object));
			}
		}
	}
