package l2r.gameserver.model.entity.SevenSignsFestival;
...
public class DarknessFestival extends Reflection
{
	...
	public DarknessFestival(Party party, int cabal, int level)
	{
		...
		for (Player p : party)
		{
			...
			//p.teleToLocation(_startLocation.loc.setR(this).findPointToStay(20, 100), this);
	...
	public void collapse()
	{
		...
		if ((SevenSigns.getInstance().getCurrentPeriod() == SevenSigns.PERIOD_COMPETITION) && (getParty() != null))
		{
			...
			if (player.getInventory().destroyItem(bloodOfferings, ""))
			{
				...
				sendCustomMessageToParticipants("l2r.gameserver.model.entity.SevenSignsFestival.Ended");
				if (isHighestScore)
				{
			...
			{
				player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2FestivalGuideInstance.BloodOfferings", player));
			}
		}
