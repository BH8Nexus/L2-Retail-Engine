...
import l2r.gameserver.model.entity.events.objects.SpawnableObject;
...
public class FortressSiegeEvent extends SiegeEvent<Fortress, SiegeClanObject>
{
	...
	private class SpawnCommanderTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			 ...
			 * убит кеп if(spawner.getAllSpawned().isEmpty()) spawner.doSpawn(true); else { NpcInstance npc = spawner.getAllSpawned().get(0); npc.setCurrentHpMp(npc.getMaxHp(), npc.getMaxMp(), true); } }
			 */
			...
			broadcastTo(SystemMsg.THE_BARRACKS_FUNCTION_HAS_BEEN_RESTORED, SiegeEvent.ATTACKERS, SiegeEvent.DEFENDERS);
	...
	public synchronized void reCalcNextTime(boolean onStart)
	{
		...
		if (attackersSize > 0)
		{
			...
			/*
			 * if (startSiegeDate.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY) { if (startSiegeDate.get(Calendar.HOUR_OF_DAY) >= 19 && startSiegeDate.get(Calendar.HOUR_OF_DAY) < 22) { startSiegeDate.add(Calendar.HOUR_OF_DAY, 22-startSiegeDate.get(Calendar.HOUR_OF_DAY)); } }
			 */
	...
	public SystemMsg checkForAttack(Creature target, Creature attacker, Skill skill, boolean force)
	{
		try
		{
			...
			if (attacker.isPlayer() && target.isPlayer() && (attacker.getPlayer().getPlayerGroup() == target.getPlayer().getPlayerGroup()))
			{
				return SystemMsg.INVALID_TARGET;
			}
