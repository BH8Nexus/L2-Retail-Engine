package l2r.gameserver.model.entity.events.fightclubmanager;
...
import l2r.gameserver.data.xml.holder.FightClubMapHolder;
...
import l2r.gameserver.model.entity.events.impl.AbstractFightClub.EventState;
import l2r.gameserver.model.entity.events.impl.DuelEvent;
import l2r.gameserver.model.entity.events.impl.fightclub.KoreanStyleEvent;
...
public class FightClubEventManager
{
	...
	public void unsignFromEvent(Player player)
	{
		...
		for (FightClubGameRoom room : _rooms)
		{
			...
			{
				// Dont allow to unregister from korean event, or it fucks up the event
				if ((room.getGame() instanceof KoreanStyleEvent) && (room.getGame().getState() != EventState.NOT_ACTIVE) && (room.getGame().getState() != EventState.OVER))
				{
					continue;
				}
	...
	public boolean isPlayerRegistered(Player player, boolean checkSameHWID)
	{
		...
		for (FightClubGameRoom room : _rooms)
		{
			...
			{
				if (iPlayer.getObjectId() == player.getObjectId())
				{
					return true;
				}
				
				if (checkSameHWID && Config.FIGHT_CLUB_HWID_CHECK && iPlayer.getHWID().equals(player.getHWID()))
				{
					return true;
				}
			}
	...
	public void startEventCountdown(AbstractFightClub event, boolean isAutoRun)
	{
		...
		ThreadPoolManager.getInstance().schedule(() ->
		{
			...
			if (!Config.ALLOW_FIGHT_CLUB || (Config.FIGHT_CLUB_DISALLOW_EVENT == duplicatedEvent.getEventId()))
			{
				return;
			}
	...
	/**
	 * Checking {@link #canPlayerParticipate(l2r.gameserver.model.Player, boolean, boolean)} for every player participated in event. Sending Message, checking all condtions
	...
	{
		final List<FightClubGameRoom> eventRooms = getEventRooms(event);
		...
		int playerCount = 0;
		final List<FightClubGameRoom> finalEventRooms = new ArrayList<>();
		for (FightClubGameRoom room : eventRooms)
		{
			...
			finalEventRooms.add(room);
		}
		
		// Synerge - Chequeamos si el evento tiene la minima cantidad de jugadores para comenzar. Si no la tiene, lo cancelamos
		if (playerCount < FightClubMapHolder.getInstance().getMinPlayersForEvent(Util.getChangedEventName(event)))
		{
			LOG.info(event.getName() + ": Cancelled. Not enough players");
			return;
		}
		
		for (FightClubGameRoom room : finalEventRooms)
		{
