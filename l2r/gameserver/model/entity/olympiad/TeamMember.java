...
public class TeamMember
{
	...
	public void takePointsForCrash()
	{
		if (!checkPlayer())
		{
			try
			{
				...
				// TODO: Ð¡Ð½ÐµÑ�Ñ‚Ð¸ Ð¿Ð¾Ð´Ñ€Ð¾Ð±Ð½Ñ‹Ð¹ Ð»Ð¾Ð³ Ð¿Ð¾Ñ�Ð»Ðµ Ð¸Ñ�Ð¿Ñ€Ð°Ð²Ð»ÐµÐ½Ð¸Ñ� Ð±ÐµÑ�Ð¿Ñ€Ð¸Ñ‡Ð¸Ð½Ð½Ð¾Ð³Ð¾ Ð¾Ñ‚ÑŠÑ‘Ð¼Ð° Ð¾Ñ‡ÐºÐ¾Ð².
	...
	public void portPlayerBack()
	{
		...
		try
		{
			...
			// Ð’Ð¾Ð·Ð²Ñ€Ð°Ñ‰Ð°ÐµÐ¼ ÐºÐ»Ð°Ð½Ð¾Ð²Ñ‹Ðµ Ñ�ÐºÐ¸Ð»Ð»Ñ‹ ÐµÑ�Ð»Ð¸ Ñ€ÐµÐ¿ÑƒÑ‚Ð°Ñ†Ð¸Ñ� Ð¿Ð¾Ð»Ð¾Ð¶Ð¸Ñ‚ÐµÐ»ÑŒÐ½Ð°Ñ�.
			if (player.getClan() != null && player.getClan().getReputationScore() >= 0)
				player.getClan().enableSkills(player);
			// Add Hero Skills
			if (player.isHero())
				Hero.addSkills(player);
			// ÐžÐ±Ð½Ð¾Ð²Ð»Ñ�ÐµÐ¼ Ñ�ÐºÐ¸Ð»Ð» Ð»Ð¸Ñ�Ñ‚, Ð¿Ð¾Ñ�Ð»Ðµ Ð´Ð¾Ð±Ð°Ð²Ð»ÐµÐ½Ð¸Ñ� Ñ�ÐºÐ¸Ð»Ð¾Ð²
			player.sendPacket(new SkillList(player));
			player.sendPacket(new ExOlympiadMode(0));
			player.sendPacket(new ExOlympiadMatchEnd());
			// Ð’Ð¾Ñ�Ñ�Ñ‚Ð°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÐµÐ¼ Ñ‚Ð¾Ñ‡ÐºÑƒ Ð¸Ñ‚ÐµÐ¼Ð¾Ð²
	...
	public void preparePlayer()
	{
		...
		try
		{
			...
			// Ð£Ð´Ð°Ð»Ñ�ÐµÐ¼ Ð±Ð°Ñ„Ñ„Ñ‹ Ð¸ Ñ‡ÑƒÐ¶Ð¸Ðµ ÐºÑƒÐ±Ð¸ÐºÐ¸
			removeBuffs(true);
			// unsummon agathion
			if (player.getAgathionId() > 0)
				player.setAgathion(0);
			// Ð¡Ð±Ñ€Ð¾Ñ� ÐºÑƒÐ»Ð´Ð°ÑƒÐ½Ð° Ð²Ñ�ÐµÑ… Ñ�ÐºÐ¸Ð»Ð¾Ð², Ð²Ñ€ÐµÐ¼Ñ� Ð¾Ñ‚ÐºÐ°Ñ‚Ð° ÐºÐ¾Ñ‚Ð¾Ñ€Ñ‹Ñ… Ð¼ÐµÐ½ÑŒÑˆÐµ 15 Ð¼Ð¸Ð½ÑƒÑ‚
			...
			// ÐžÐ±Ð½Ð¾Ð²Ð»Ñ�ÐµÐ¼ Ñ�ÐºÐ¸Ð»Ð» Ð»Ð¸Ñ�Ñ‚, Ð¿Ð¾Ñ�Ð»Ðµ ÑƒÐ´Ð°Ð»ÐµÐ½Ð¸Ñ� Ñ�ÐºÐ¸Ð»Ð¾Ð²
			player.sendPacket(new SkillList(player));
			// ÐžÐ±Ð½Ð¾Ð²Ð»Ñ�ÐµÐ¼ ÐºÑƒÐ»Ð»Ð´Ð°ÑƒÐ½, Ð¿Ð¾Ñ�Ð»Ðµ Ñ�Ð±Ñ€Ð¾Ñ�Ð°
			...
			// Ð Ð°Ð·Ñ€Ñ�Ð¶Ð°ÐµÐ¼ Ð·Ð°Ñ€Ñ�Ð¶ÐµÐ½Ð½Ñ‹Ðµ Ñ�Ð¾ÑƒÐ» Ð¸ Ñ�Ð¿Ð¸Ñ€Ð¸Ñ‚ ÑˆÐ¾Ñ‚Ñ‹
			...
			// ÐŸÑ€Ð¾Ð²ÐµÑ€Ñ�ÐµÐ¼ Ñ‚Ð¾Ñ‡ÐºÑƒ Ð¸Ñ‚ÐµÐ¼Ð¾Ð²
