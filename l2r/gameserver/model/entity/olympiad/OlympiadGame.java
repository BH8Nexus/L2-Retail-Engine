...
public class OlympiadGame
{
	...
	public void validateWinner(boolean aborted, boolean team1, boolean isEndByTime)
	{
		...
		// Ð•Ñ�Ð»Ð¸ Ð¸Ð³Ñ€Ð° Ð·Ð°ÐºÐ¾Ð½Ñ‡Ð¸Ð»Ð°Ñ�ÑŒ Ð´Ð¾ Ñ‚ÐµÐ»ÐµÐ¿Ð¾Ñ€Ñ‚Ð°Ñ†Ð¸Ð¸ Ð½Ð° Ñ�Ñ‚Ð°Ð´Ð¸Ð¾Ð½, Ñ‚Ð¾ Ð·Ð°Ð±Ð¸Ñ€Ð°ÐµÐ¼ Ð¾Ñ‡ÐºÐ¸ Ñƒ Ð²Ñ‹ÑˆÐµÐ´ÑˆÐ¸Ñ… Ð¸Ð· Ð¸Ð³Ñ€Ñ‹, Ð½Ðµ Ð·Ð°Ñ�Ñ‡Ð¸Ñ‚Ñ‹Ð²Ð°Ñ� Ð½Ð¸ÐºÐ¾Ð¼Ñƒ Ð¿Ð¾Ð±ÐµÐ´Ñƒ
		if (state < 1 && aborted)
		{
		...
		if (_winner <= 0)
		{
			if (!teamOneCheck && !teamTwoCheck)
				...
				_winner = 1; // Ð’Ñ‹Ð¸Ð³Ñ€Ð°Ð»Ð° Ð¿ÐµÑ€Ð²Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð°
			else if (!teamOneCheck)
				_winner = 2; // Ð’Ñ‹Ð¸Ð³Ñ€Ð°Ð»Ð° Ð²Ñ‚Ð¾Ñ€Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð°
			else if (_team1.getDamage() < _team2.getDamage()) // Ð’Ñ‚Ð¾Ñ€Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð° Ð½Ð°Ð½ÐµÑ�Ð»Ð° Ð²Ñ€ÐµÐ´Ð° Ð¼ÐµÐ½ÑŒÑˆÐµ, Ñ‡ÐµÐ¼ Ð¿ÐµÑ€Ð²Ð°Ñ�
				_winner = 1; // Ð’Ñ‹Ð¸Ð³Ñ€Ð°Ð»Ð° Ð¿ÐµÑ€Ð²Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð°
			else if (_team1.getDamage() > _team2.getDamage()) // Ð’Ñ‚Ð¾Ñ€Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð° Ð½Ð°Ð½ÐµÑ�Ð»Ð° Ð²Ñ€ÐµÐ´Ð° Ð±Ð¾Ð»ÑŒÑˆÐµ, Ñ‡ÐµÐ¼ Ð¿ÐµÑ€Ð²Ð°Ñ�
				_winner = 2; // Ð’Ñ‹Ð¸Ð³Ñ€Ð°Ð»Ð° Ð²Ñ‚Ð¾Ñ€Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð°
		}

		try
		{
			if (_winner == 1) // Ð’Ñ‹Ð¸Ð³Ñ€Ð°Ð»Ð° Ð¿ÐµÑ€Ð²Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð°
				winGame(_team1, _team2, isEndByTime);
			else if (_winner == 2) // Ð’Ñ‹Ð¸Ð³Ñ€Ð°Ð»Ð° Ð²Ñ‚Ð¾Ñ€Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð°
	...
	public void winGame(OlympiadTeam winnerTeam, OlympiadTeam looseTeam, boolean isEndByTime)
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
		{
			...
			if (player.isNotShowOlympiadAnnouncements())
				continue;
	...
	public void broadcastInfo(Player sender, Player receiver, boolean onlyToSpectators)
	{
		...
		{
			// Ð Ð°Ñ�Ñ�Ñ‹Ð»Ð°ÐµÐ¼ Ð¸Ð½Ñ„Ð¾Ñ€Ð¼Ð°Ñ†Ð¸ÑŽ Ð¾ Ð¿ÐµÑ€Ð²Ð¾Ð¹ ÐºÐ¾Ð¼Ð°Ð½Ð´Ðµ
			...
			// Ð Ð°Ñ�Ñ�Ñ‹Ð»Ð°ÐµÐ¼ Ð¸Ð½Ñ„Ð¾Ñ€Ð¼Ð°Ñ†Ð¸ÑŽ Ð¾ Ð²Ñ‚Ð¾Ñ€Ð¾Ð¹ ÐºÐ¾Ð¼Ð°Ð½Ð´Ðµ
