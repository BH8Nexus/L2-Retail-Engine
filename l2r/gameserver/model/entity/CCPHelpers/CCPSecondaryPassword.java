...
public class CCPSecondaryPassword
{
	...
	public static void startSecondaryPasswordSetup(Player player, String text)
	{
		...
		if (pageIndex.equals("F"))
		{
			...
			{
				if (player.getVar("secondarypasslang", "en").equalsIgnoreCase("en"))
				{
					sendHtml(player, HtmCache.getInstance().getNotNull("command/cfgSPSecondarySet.htm", player));
				}
				else if (player.getVar("secondarypasslang", "en").equalsIgnoreCase("ru"))
				{
					sendHtml(player, HtmCache.getInstance().getNotNull("command/cfgSPSecondarySet.htm", player));
				}
			}
	...
	private static boolean checkConditions(String password, int mustChars)
	{
		...
		if (((mustChars & CHAR_UPPERCASE) != 0) && !containsUpperCase)
		{
			return false;
		}
		if (((mustChars & CHAR_LOWERCASE) != 0) && !containsLowerCase)
		{
			return false;
		}
		if (((mustChars & CHAR_NUMBER) != 0) && !containsDigit)
		{
			return false;
		}
		if (((mustChars & CHAR_SYMBOL) != 0) && !hasSpecialChar)
		{
			return false;
		}
