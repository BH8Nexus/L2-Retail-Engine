...
import l2r.gameserver.dao.ClanDataDAO;
import l2r.gameserver.dao.ClanHallDAO;
	...
	public void chanceCycle()
	{
		...
		if (getPaidCycle() >= REWARD_CYCLE)
		{
			if (_owner.getWarehouse().getCountOf(ItemTemplate.ITEM_ID_ADENA) > _rentalFee)
			{
				...
				setPaidCycle(0);
				Log.add("Clanhall " + getName() + " lease " + _rentalFee + " adena from clan " + _owner.getName() + "(id:" + _owner.getClanId() + ")", "residence");
			}
			else
			{
				...
				Log.add("Remove " + getName() + "  clanhall from clan " + _owner.getName() + "(id:" + _owner.getClanId() + "), because thay have only " + _owner.getWarehouse().getCountOfAdena() + " when lease is " + _rentalFee, "residence");
				changeOwner(null);
			}
		}
	}
