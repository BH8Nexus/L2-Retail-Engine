...
public class Individual
{
	public static void giveReward(Player player)
	{
		...
		// continue;
		//
		// if(_achievement.getConditions().get(0).getName().equals("Min Votes"))
		// {
	...
	public static synchronized boolean hasVotedTopzone(Player player)
	{
		try
		{
			final URL obj = new URL(String.format("https://api.l2topzone.com/v1/vote?token=%s&ip=%s", Config.VOTE_TOPZONE_APIKEY, player.getIP()));// player.getClient().getConnection().getInetAddress().getHostAddress()));
	...
	public static synchronized boolean hasVotedHopzone(Player player)
	{
		try
		{
			final URL obj = new URL(String.format("http://api.hopzone.net/lineage2/vote?token=%s&ip_address=%s", Config.VOTE_HOPZONE_APIKEY, player.getIP()));// player.getClient().getConnection().getInetAddress().getHostAddress()));
			final HttpURLConnection con = (HttpURLConnection) obj.openConnection();
	...
	public static final boolean votedNet(Player player)
	{
		try
		{
			final URL obj = new URL(String.format("https://l2network.eu/index.php?a=in&u=%s&ipc=%s", Config.VOTE_L2JBRASIL_NAME, player.getIP()));// player.getClient().getConnection().getInetAddress().getHostAddress()));
			final HttpURLConnection con = (HttpURLConnection) obj.openConnection();
	...
	public static boolean isAccountTookTheReward(Player activeChar)
	{
		// Time restriction
		long lastVoteReward = getLastVoteReward(activeChar);
		...
		getLastVoteRewardsOfPlayerCharacters(activeChar, _accountCharacters);
		...
		getLastVoteRewardsOfPlayerIP(activeChar, _IPCharacters);
		...
		getLastVoteRewardsOfPlayerHWID(activeChar, _HWIDCharacters);
