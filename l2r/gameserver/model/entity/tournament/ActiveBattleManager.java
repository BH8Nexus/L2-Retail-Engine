...
import l2r.gameserver.utils.ChatUtil;
...
public final class ActiveBattleManager
{
	...
	private static Team getWalkoverWinner(BattleInstance battleInstance)
	{
		...
		for (Team team : teams)
		{
			if (battleInstance.getFightersForIterate(team).isEmpty())
			{
				return team;
			}
		}
	...
	{
		if (!actor.isPlayable() || battleInstance.isBattleOver())
		{
			return;
		}
	...
	public static boolean canUseBuffer(Player player, boolean premiumBuff)
	{
		...
		if (battleInstance.isFightTime() || !ConfigHolder.getBool("TournamentAllowBuffer"))
		{
			return false;
		}
		if (premiumBuff && ConfigHolder.getBool("TournamentPremiumBuffsDisabled"))
		{
			return false;
		}
		return true;
