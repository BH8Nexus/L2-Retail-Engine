...
public class TournamentTeamsManager
{
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("REPLACE INTO tournament_teams VALUES(?,?,?,?,?,?,?,?,?,?,?)"))
		{
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("DELETE FROM tournament_teams WHERE team_id = ?"))
		{
