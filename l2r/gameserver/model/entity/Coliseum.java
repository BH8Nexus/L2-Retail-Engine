...
import l2r.gameserver.instancemanager.UnderGroundColliseumManager;
...
public class Coliseum
{
			...
			else
			{
				if (_party1.getLeader().getTeam() == _winner)
				{
					...
					else
					{
						...
						for (Player member : _party1)
						{
							member.setFame(member.getFame() + 800 + (getWinCount() * 5), "Coliseum"); // TODO: getWinCount() * 5 тоже нужно поднять, инфы нет
						}
					}
	...
	public class StartBattle implements Runnable
	{
		...
		public void run()
		{
			...
			if (!isParty1Ready)
			{
				...
				// Original Message: Opponents party is offline, wait for next opponent.
				_party2.sendMessage(new CustomMessage("l2r.gameserver.model.entity.Coliseum.message1", null)); // default: english.
				return;
			}
			else if (!isParty2Ready)
			{
				...
				// Original Message: Opponents party is offline, wait for next opponent.
				_party1.sendMessage(new CustomMessage("l2r.gameserver.model.entity.Coliseum.message1", null)); // default: english.
				return;
			}
			else
			{
				...
				if (party_waiting_list.size() > 0)
				{
					...
					for (Party party : party_waiting_list)
					{
						if (party.size() > 6)
						{
							// Original Message: Free room at coliseum append. //FIXME: SM? CM? not broadcast?
							party.sendMessage(new CustomMessage("l2r.gameserver.model.entity.Coliseum.message2", null)); // Default english
						}
						else
						{
							toDel.add(party);
					...
					{
						// Original Message: Conditions are not right for Underground battle, try to register later. //FIXME: SM? CM? not broadcast?
						party.sendMessage(new CustomMessage("l2r.gameserver.model.entity.Coliseum.message3", null));
	...
	public static void register(Player player, int minLevel, int maxLevel)
	{
		...
		if (coli == null)
		{
			// Original Message: This does not work now, if you have any information about it, contact us.
			player.sendMessage(new CustomMessage("l2r.gameserver.model.entity.Coliseum.message4", player));
			return;
		}
	...
	public void teleportToWaitingRoom(Party party, Location teleloc)
	{
		...
		{
			// Original Message: Rooms are not free. You are already registered, so, try to use teleport function later.
			party.getLeader().sendMessage(new CustomMessage("l2r.gameserver.model.entity.Coliseum.message5", party.getLeader()));
			return;
		}
	...
	public final Zone getZone()
	{
		if (_zone == null)
		{
			_zone = ZoneHolder.getZone("UnderGroundColiseum");
		}
