	...
	public static void inviteInAcademy(Player activeChar, Player academyChar)
	{
		...
		try
		{
			if (academyChar.getPledgePrice() != 0)
			{
				...
				// if (activeChar.getInventory().removeItem(itemId, price, true)) // == 0)
			...
			if (giveReward)
			{
				...
				while (rs.next())
				{
					try
					{
						...
						String charName = CharacterDAO.getNameByObjectId(charId);
			...
			if (kick)
			{
				...
				while (rs1.next())
				{
					try
					{
						...
						CharacterDAO.getInstance();
						String acadCharName = CharacterDAO.getNameByObjectId(charId);
