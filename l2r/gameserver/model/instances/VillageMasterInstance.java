...
public final class VillageMasterInstance extends NpcInstance
{
		...
		else if (command.startsWith("Subclass"))
		{
			...
			switch (intVal)
			{
				case 1: // Возвращает список сабов, которые можно взять (см case 4)
					...
					{
						player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", player));
						return;
					}
					break;
				...
				case 4: // Добавление сабкласса - обработка выбора из case 1
					...
					{
						player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.NoSubBeforeLevel", player).addNumber(Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS));
						allowAddition = false;
					}
					...
					/*
					 * Если требуется квест - проверка прохождения Mimir's Elixir (Path to Subclass) Для камаэлей квест 236_SeedsOfChaos Если саб первый, то проверить начилие предмета, если не первый, то даём сабкласс. Если сабов нету, то проверяем наличие предмета.
					 */
					if (!Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS && !playerClassList.isEmpty() && (playerClassList.size() < (2 + Config.ALT_GAME_SUB_ADD)))
					{
						if (player.isQuestCompleted("_234_FatesWhisper"))
						{
							if (player.getRace() == Race.kamael)
							{
								...
								if (!allowAddition)
								{
									player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.QuestSeedsOfChaos", player));
								}
							}
							else
							{
								...
								if (!allowAddition)
								{
									player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.QuestMimirsElixir", player));
								}
							}
						}
						else
						{
							player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.QuestFatesWhisper", player));
							allowAddition = false;
						}
					}
					...
					if (allowAddition)
					{
						...
						{
							player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", player));
							return;
						}
				...
				case 5: // Смена саба на другой из уже взятых - обработка выбора из case 2
					 ...
					 */
					/*
					 * for(L2SubClass<?> sub : playerClassList.values()) if(sub.isBase() && sub.getLevel() < Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS) { player.sendMessage("You may not change to your subclass before you are level " + Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS,
					 * "Вы не можете добавить еще сабкласс пока у вас уровень " + Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS + " на Вашем предыдущем сабклассе."); return; }
					 */
	...
	public void setLeader(Player leader, String newLeader)
	{
		...
		if (member == null)
		{
			// FIXME зачем 2-ве мессаги(VISTALL)
			leader.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.S1IsNotMemberOfTheClan", leader).addString(newLeader));
		...
		{
			leader.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.CannotAssignUnitLeader", leader));
			return;
		}
	...
	{
		player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.ClanLeaderWillBeChangedFromS1ToS2", player).addString(clan.getLeaderName()).addString(newLeader.getName()));
		...
		/*
		 * if(clan.getLevel() >= CastleSiegeManager.getSiegeClanMinLevel()) { if(clan.getLeader() != null) { L2Player oldLeaderPlayer = clan.getLeader().getPlayer(); if(oldLeaderPlayer != null) SiegeUtils.removeSiegeSkills(oldLeaderPlayer); } L2Player newLeaderPlayer = newLeader.getPlayer();
		 * if(newLeaderPlayer != null) SiegeUtils.addSiegeSkills(newLeaderPlayer); }
		 */
	...
	public void createSubPledge(Player player, String clanName, int pledgeType, int minClanLvl, String leaderName)
	{
		...
		if (pledgeType != Clan.SUBUNIT_ACADEMY)
		{
			...
			if (subLeader == null)
			{
				player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.PlayerCantBeAssignedAsSubUnitLeader", player));
				return;
			}
			else if (subLeader.getLeaderOf() != Clan.SUBUNIT_NONE)
			{
				player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.ItCantBeSubUnitLeader", player));
				return;
			}
		}
	...
	public void assignSubPledgeLeader(Player player, String clanName, String leaderName)
	{
		...
		if (clan == null)
		{
			player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.ClanDoesntExist", player));
			return;
		}
		...
		if (targetUnit == null)
		{
			player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.SubUnitNotFound", player));
			return;
		}
		SubUnit mainUnit = clan.getSubUnit(Clan.SUBUNIT_MAIN_CLAN);
		...
		player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.L2VillageMasterInstance.NewSubUnitLeaderHasBeenAssigned", player));
	}
		...
		switch (clan.getLevel())
		{
			...
			case 10:
				...
				if ((clan.getReputationScore() >= Config.CLAN_LEVEL_11_COST) && (clan.getAllSize() >= Config.CLAN_LEVEL_11_REQUIREMEN))
				{
					...
					if (castle == null)
					{
						player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.VillageMasterInstance.message1", player));
	...
	public void createAlly(Player player, String allyName)
	{
		...
		player.broadcastCharInfo();
		player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.VillageMasterInstance.message2", player).addString(allyName));
	}
	...
	private Set<PlayerClass> getAvailableSubClasses(Player player, boolean isNew)
	{
		 ...
		 * Spellhowler Kamael могут брать только сабы Kamael Другие классы не могут брать сабы Kamael
		 */
