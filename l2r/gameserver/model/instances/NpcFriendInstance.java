...
public final class NpcFriendInstance extends MerchantInstance
{
	...
	public void onAction(Player player, boolean shift)
	{
		...
		if (!Config.ALT_GAME_KARMA_PLAYER_CAN_SHOP && (player.getKarma() > 0) && !player.canOverrideCond(PcCondOverride.ITEM_SHOP_CONDITIONS))
		{
			player.sendActionFailed();
			return;
		}
		
		else if (!Config.ALT_GAME_KARMA_PLAYER_CAN_USE_GK && (player.getKarma() > 0) && !player.isGM())
		{
		...
		// Π΅ NPC Π½ΠµΠ»Ρ�Π·Ρ� Ρ€Π°Π·Π³ΠΎΠ²Π°Ρ€ΠΈΠ²Π°Ρ‚Ρ� ΠΌΠµΡ€Ρ‚Π²Ρ‹ΠΌ ΠΈ Ρ�ΠΈΠ΄Ρ�
...
//	}
	 ...
	 * @return boolean
	 */
	private boolean showPkDenyChatWindow(Player player)
	{
		String html = HtmCache.getInstance().getNotNull("teleport-pk.htm", player);
