...
public final class PetBabyInstance extends PetInstance
{
	...
	public Skill onActionTask()
	{
		try
		{
			...
			if ((!owner.isDead() && !owner.isInvul() && !this.isCastingNow()) || !this.isDead())
			{
				...
				if (!Config.ALT_PET_HEAL_BATTLE_ONLY || owner.isInCombat())
				{
					...
					{
						if (curHp < 33) // Ρ�ΠΊΡ�Ρ‚Ρ€ΠµΠ½Π½Π°Ρ� Ρ�ΠΈΡ‚ΡƒΠ°Ρ†ΠΈΡ�, Ρ�ΠΈΠ»Ρ�Π½Ρ‹ΠΉ Ρ…ΠΈΠ»
						{
							skill = SkillTable.getInstance().getInfo(improved ? BattleHeal : GreaterHealTrick, getHealLevel());
						}
						else if ((getNpcId() != PetDataTable.IMPROVED_BABY_KOOKABURRA_ID) || (getNpcId() != PetDataTable.FAIRY_PRINCESS_ID) || (getNpcId() != PetDataTable.SPIRIT_SHAMAN_ID))
						{
					...
					if (((skill == null) && (getNpcId() == PetDataTable.IMPROVED_BABY_KOOKABURRA_ID)) || (getNpcId() == PetDataTable.FAIRY_PRINCESS_ID) || (getNpcId() == PetDataTable.SPIRIT_SHAMAN_ID)) // Π ΠµΡ‡Π°Ρ€Π΄Π¶ Ρ‚ΠΎΠ»Ρ�ΠΊΠΎ Ρƒ Kookaburra ΠΈ Π² ΠΊΠΎΠΌΠ±Π°Ρ‚ ΠΌΠΎΠ΄Πµ
					{
				...
				if (!improved || /* owner.isInZonePeace() || */ owner.isInOfflineMode() || (owner.getEffectList().getEffectsCountForSkill(5771) > 0))
				{
					return null;
				}
				...
				{
					if ((buff == null) || (getCurrentMp() < buff.getMpConsume2()))
					{
						continue;
					}
	...
	/**
	 * Π’ΠΎΠ·Π²Ρ€Π°Ρ‰Π°ΠµΡ‚ true ΠµΡ�Π»ΠΈ Ρ�Ρ„Ρ„ΠµΠΊΡ‚ Π΄Π»Ρ� Ρ�ΠΊΠΈΠ»Π»Π° ΡƒΠ¶Πµ ΠµΡ�Ρ‚Ρ� ΠΈ Π·Π°Π½ΠΎΠ²ΠΎ Π½Π°ΠΊΠ»Π°Π΄Ρ‹Π²Π°Ρ‚Ρ� Π½Πµ Π½Π°Π΄ΠΎ
	 */
	private boolean checkEffect(Effect ef, Skill skill)
	{
		if ((ef == null) || !ef.isInUse() || !EffectList.checkStackType(ef.getTemplate(), skill.getEffectTemplates()[0])) // Ρ‚Π°ΠΊΠΎΠ³ΠΎ Ρ�ΠΊΠΈΠ»Π»Π° Π½ΠµΡ‚
		{
			return false;
		}
		if (ef.getStackOrder() < skill.getEffectTemplates()[0]._stackOrder) // Ρ�Ρ‚Π°Ρ€Ρ‹ΠΉ Ρ�Π»Π°Π±ΠµΠµ
		{
			return false;
		}
		if (ef.getTimeLeft() > 10) // Ρ�Ρ‚Π°Ρ€Ρ‹ΠΉ Π½Πµ Ρ�Π»Π°Π±ΠµΠµ ΠΈ ΠµΡ‰Πµ Π½Πµ ΠΊΠΎΠ½Ρ‡Π°ΠµΡ‚Ρ�Ρ� - Π¶Π΄ΠµΠΌ
		{
			return true;
		}
		if (ef.getNext() != null) // Ρ�Ρ‚Π°Ρ€Ρ‹ΠΉ Π½Πµ Ρ�Π»Π°Π±ΠµΠµ Π½ΠΎ ΡƒΠ¶Πµ ΠΊΠΎΠ½Ρ‡Π°ΠµΡ‚Ρ�Ρ� - ΠΏΡ€ΠΎΠ²ΠµΡ€ΠΈΡ‚Ρ� Ρ€ΠµΠΊΡƒΡ€Ρ�ΠΈΠµΠΉ Ρ‡Ρ‚ΠΎ Ρ‚Π°ΠΌ Π·Π°Ρ�ΠµΠ΄ΡƒΠ»ΠµΠ½ΠΎ
		{
	...
	{
		if ((getNpcId() == PetDataTable.WHITE_WEASEL_ID) || (getNpcId() == PetDataTable.FAIRY_PRINCESS_ID) || ((getNpcId() >= 1562) && (getNpcId() <= 1573)))
		{
