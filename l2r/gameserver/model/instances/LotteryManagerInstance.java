...
public class LotteryManagerInstance extends NpcInstance
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4869872964100345087L;
	...
	public void showLotoWindow(Player player, int val)
	{
		...
		if (val == 22)
		{
			...
			int price = Config.SERVICES_LOTTERY_TICKET_PRICE;
			...
			ItemInstance item = new ItemInstance(4442);
			item.setCustomType1(lotonumber);
			item.setEnchantLevel(enchant);
			item.setCustomType2(type2);
			player.getInventory().addItem(item, "");
			
			Log.item(Log.NpcCreateItem, this, player, item, "Payed Price: " + price);
			
			// If a player buy a ticket it will increase the mass reward
		...
		html.replace("%prize5%", "" + Util.formatAdena((long) (LotteryManager.getInstance().getPrize() / (Config.SERVICES_LOTTERY_5_NUMBER_RATE))));
		html.replace("%prize4%", "" + Util.formatAdena((long) (LotteryManager.getInstance().getPrize() / (Config.SERVICES_LOTTERY_4_NUMBER_RATE))));
		html.replace("%prize3%", "" + Util.formatAdena((long) (LotteryManager.getInstance().getPrize() / (Config.SERVICES_LOTTERY_3_NUMBER_RATE))));
		html.replace("%prize2%", "" + Util.formatAdena((long) (LotteryManager.getInstance().getPrize() / (Config.SERVICES_LOTTERY_2_AND_1_NUMBER_PRIZE))));
