...
public class SummonInstance extends Summon
{
	...
	{
		double init = getActiveWeaponInstance() == null ? _template.getBasePAtk() : 0; //basePAtk
		double mod = BaseStats.STR.calcBonus(this) * getLevelMod();
		return (int) calcStat(Stats.POWER_ATTACK, init * mod, target, null);
	}
	...
	public int getMAtk(final Creature target, final Skill skill)
	{
		...
		final double init = getActiveWeaponInstance() == null ? _template.getBaseMAtk() : 0; //baseMAtk
		double ib = BaseStats.INT.calcBonus(this);
		double lvlb = getLevelMod();
		double mod = lvlb * lvlb * ib * ib;
		
		return (int) calcStat(Stats.MAGIC_ATTACK, init * mod, target, skill);
	}
	...
	public int getPDef(Creature target)
	{
		...
		return (int) calcStat(Stats.POWER_DEFENCE, _template.getBasePDef() * mod, target, null);
	}
	...
	public int getMDef(Creature target, Skill skill)
	{
		...
		return Math.max((int) calcStat(Stats.MAGIC_DEFENCE, _template.getBaseMDef() * mod, target, skill), 1);
	}
	
	@Override
	public int getMaxHp()
	{
		double mod = BaseStats.CON.calcBonus(this);
		return (int) calcStat(Stats.MAX_HP, _template.getBaseHpMax() * mod, null, null);
	}
	
	@Override
	public int getMaxMp()
	{
		double mod = BaseStats.MEN.calcBonus(this);
		return (int) calcStat(Stats.MAX_MP, _template.getBaseMpMax() * mod, null, null);
	}
	...
	public void onAction(Player player, boolean shift)
	{
		...
		{
			/*
			 * if(!player.getAccessLevel().canViewCharacter()) TODO FIXXX return;
			 */
	...
	{
		return WeaponType.SWORD.mask(); // TODO: Ñ‡Ð¸Ñ‚Ð°Ñ‚ÑŒ Ð¿Ð°Ñ�Ñ�Ð¸Ð²ÐºÐ¸ Ð¸ Ñ�Ð¼Ð¾Ñ‚Ñ€ÐµÑ‚ÑŒ Ñ‚Ð¸Ð¿ Ð¾Ñ€ÑƒÐ¶Ð¸Ñ� Ð¸ Ð±Ñ€Ð¾Ð½Ð¸ Ñ‚Ð°Ð¼
	}
}
