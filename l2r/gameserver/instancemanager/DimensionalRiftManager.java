...
import l2r.commons.geometry.Rectangle;
...
public class DimensionalRiftManager
{
	...
	public void load()
	{
		...
		{
			_log.error("DimensionalRiftManager: Error on loading DimensionalRift spawns!", e);
		}
		...
		_log.info("DimensionalRiftManager: Loaded " + countGood + " DimensionalRift spawns, " + countBad + " errors.");
	}
	...
	{
		//teleToLocation(player, getRoom(0, 0).getTeleportCoords().findPointToStay(250), null);
		...
		if (!player.canOverrideCond(PcCondOverride.INSTANCE_CONDITIONS))
		{
			...
			for (Player p : player.getParty())
			{
				...
				if ((i == null) || (i.getCount() < getNeededItems(type)))
				{
					...
					player.sendMessage("Required items: " + getNeededItems(type) + " " + ItemHolder.getInstance().getTemplateName(DIMENSIONAL_FRAGMENT_ITEM_ID)); // Custom message for noobs.
					return;
				}
			}
	...
	public static void teleToLocation(Player player, Location loc, Reflection ref)
	{
		...
		// Ð�ÑƒÐ¶Ð½Ð¾ Ð¿Ñ€Ð¸ Ñ‚ÐµÐ»ÐµÐ¿Ð¾Ñ€Ñ‚Ðµ Ñ� Ð±Ð¾Ð»ÐµÐµ Ð²Ñ‹Ñ�Ð¾ÐºÐ¾Ð¹ Ñ‚Ð¾Ñ‡ÐºÐ¸ Ð½Ð° Ð±Ð¾Ð»ÐµÐµ Ð½Ð¸Ð·ÐºÑƒÑŽ, Ð¸Ð½Ð°Ñ‡Ðµ Ð½Ð°Ð½Ð¾Ñ�Ð¸Ñ‚Ñ�Ñ� Ð²Ñ€ÐµÐ´ Ð¾Ñ‚ "Ð¿Ð°Ð´ÐµÐ½Ð¸Ñ�"
