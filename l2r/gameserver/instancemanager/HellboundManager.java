...
public class HellboundManager
{
			...
			if (ArrayUtils.contains(hbsi.getStages(), getHellboundLevel()))
			{
				try
				{
					...
					for (int i = 0; i < hbsi.getAmount(); i++)
					{
						...
						spawnDat.getLastSpawn().setChampionTemplate(null);
	...
	private void getHellboundSpawn()
	{
		...
		try
		{
			File file = new File(Config.DATAPACK_ROOT, "data/xml/hellbound_spawnlist.xml");
	...
	private static void doorHandler()
	{
		...
		{
			ZoneHolder.getDoor(_door).closeMe();
		}
		...
		switch (getHellboundLevel())
		{
			...
			case 4:
				break;
			case 5:
				ZoneHolder.getDoor(NativeHell_native0131).openMe();
				ZoneHolder.getDoor(NativeHell_native0132).openMe();
				break;
			case 6:
				ZoneHolder.getDoor(NativeHell_native0131).openMe();
				ZoneHolder.getDoor(NativeHell_native0132).openMe();
			...
			case 7:
				ZoneHolder.getDoor(NativeHell_native0131).openMe();
				ZoneHolder.getDoor(NativeHell_native0132).openMe();
				ZoneHolder.getDoor(sdoor_trans_mesh00).openMe();
				break;
			case 8:
				ZoneHolder.getDoor(NativeHell_native0131).openMe();
				ZoneHolder.getDoor(NativeHell_native0132).openMe();
			...
			case 9:
				...
				ZoneHolder.getDoor(NativeHell_native0132).openMe();
				ZoneHolder.getDoor(sdoor_trans_mesh00).openMe();
				ZoneHolder.getDoor(Hell_gate_door).openMe();
				break;
			case 10:
				ZoneHolder.getDoor(NativeHell_native0131).openMe();
				ZoneHolder.getDoor(NativeHell_native0132).openMe();
