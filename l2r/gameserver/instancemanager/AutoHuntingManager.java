...
public class AutoHuntingManager
{
	 ...
	 */
	public synchronized void reportBot(Player reported, Player reporter, String typeOfReport)
	{
		if (!reportedIsOnline(reported) && (reported.getPrivateStoreType() == Player.STORE_PRIVATE_NONE))
		{
			reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message1", reporter));
			return;
		}
		...
		try
		{
			...
			{
				FastTable<Player> p = new FastTable<>();
				p.add(reported);
			...
			else
			{
				...
				{
					reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message2", reporter));
					return;
				}
			...
			statement = con.prepareStatement("INSERT INTO `bot_report`(`reported_name`, `reported_objectId`, `reporter_name`, `reporter_objectId`, `date`, `reportType`) VALUES (?,?,?,?,?,?)", Statement.RETURN_GENERATED_KEYS);
		...
		String msgContent = reporter.getName() + " has reported player: " + reported.getName() + " for " + typeOfReport + " ";
		AdminTable.broadcastToGMs(new Say2(reporter.getObjectId(), ChatType.HERO_VOICE, "REPORT", msgContent));
		
		String htmlreportsuccess = HtmCache.getInstance().getNotNull("command/report-success.htm", reporter);
	...
	private void loadUnread()
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("SELECT `report_id`, `reported_name`, `reporter_name`, `date`, `reportType` FROM `bot_report` WHERE `read` = ?");
	...
	public boolean validateBot(Player reported, Player reporter)
	{
		...
		{
			reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message3", reporter));
			return false;
		}
		
		/*
		 * why not? // Cannot report if reported and reporter are in war if (reported.getClan() != null && reporter.getClan() != null) { if (reported.getClan().isAtWarWith(reporter.getClanId())) { reporter.sendPacket(new SystemMessage2(SystemMsg.CANNOT_REPORT_TARGET_IN_CLAN_WAR)); return false; } }
		 */
		// Cannot report itself
		if (reporter.getName().equalsIgnoreCase(reported.getName()))
		{
			reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message4", reporter));
			return false;
		}
