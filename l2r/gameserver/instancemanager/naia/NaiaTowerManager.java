package l2r.gameserver.instancemanager.naia;
...
public final class NaiaTowerManager
{
	...
	public static void startNaiaTower(Player leader)
	{
		...
		for (Player member : leader.getParty())
		{
		...
		ZoneHolder.getDoor(18250001).openMe();
	}
	...
	private static void addGroupToTower(Player leader)
	{
		...
		leader.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.naia.naiatowermanager.message1", leader));
	}
	...
	public static void updateGroupTimer(Player player)
	{
		for (int i : _groupList.keySet())
		{
			if (_groupList.get(i).contains(player))
			{
				...
				player.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.naia.naiatowermanager.message2", player));
				break;
			}
		}
	}
				...
				for (int i : _groupTimer.keySet())
				{
					if (_groupTimer.get(i) < System.currentTimeMillis())
					{
						for (Player kicked : _groupList.get(i))
						{
							...
							kicked.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.naia.naiatowermanager.message3", kicked));
						}
						_groupList.remove(i);
						_groupTimer.remove(i);
					}
