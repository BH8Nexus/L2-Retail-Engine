...
public class ItemAuctionManager
{
	...
	private void load()
	{
		...
		File file = new File(Config.DATAPACK_ROOT, "data/xml/item_auctions.xml");
		if (!file.exists())
		{
						...
						if ("instance".equalsIgnoreCase(nb.getNodeName()))
						{
							...
							for (Node nc = nb.getFirstChild(); nc != null; nc = nc.getNextSibling())
							{
								if ("item".equalsIgnoreCase(nc.getNodeName()))
								{
									...
									int auctionLength = Integer.parseInt(nac.getNamedItem("auctionLenght").getNodeValue());
									...
									if (auctionLength < 1)
									{
										throw new IllegalArgumentException("auctionLength < 1 for instanceId: " + instanceId + ", itemId " + itemId);
									}
									...
									AuctionItem item = new AuctionItem(auctionItemId, auctionLength, auctionInitBid, itemId, itemCount, altByItem, itemExtra);
									items.add(item);
								}
							}
