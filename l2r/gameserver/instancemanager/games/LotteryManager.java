...
public class LotteryManager
{
	...
	private void scheduleEndOfLottery()
	{
		...
		/**
		 * Calendar finishtime = Calendar.getInstance(); finishtime.setTimeInMillis(_enddate); finishtime.set(Calendar.MINUTE, 0); finishtime.set(Calendar.SECOND, 0); finishtime.add(Calendar.DAY_OF_MONTH, 7); finishtime.set(Calendar.DAY_OF_WEEK, 6); finishtime.set(Calendar.HOUR_OF_DAY, 7); _enddate
		 * = finishtime.getTimeInMillis(); ThreadPoolManager.getInstance().scheduleGeneral(new stopSellingTickets(), _enddate - System.currentTimeMillis() - 10 * MINUTE); ThreadPoolManager.getInstance().scheduleGeneral(new finishLottery(), _enddate - System.currentTimeMillis());
		 **/
		...
		finishtime.add(Calendar.DAY_OF_MONTH, Config.SERVICES_LOTTERY_STARTING_DATE[0]);
		finishtime.set(Calendar.HOUR_OF_DAY, Config.SERVICES_LOTTERY_STARTING_DATE[1]);
	...
	private class finishLottery extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			long prize2 = 0;
			long prize3 = 0;
			long prize4 = (long) (count4 * Config.SERVICES_LOTTERY_2_AND_1_NUMBER_PRIZE);
			long newprize;
			
			if (count1 > 0)
			{
				prize1 = (long) (getPrize() - (prize4 / Config.SERVICES_LOTTERY_5_NUMBER_RATE));
			}
			if (count2 > 0)
			{
				prize2 = (long) (getPrize() - (prize4 / Config.SERVICES_LOTTERY_4_NUMBER_RATE));
			}
			if (count3 > 0)
			{
				prize3 = (long) (getPrize() - (prize4 / Config.SERVICES_LOTTERY_3_NUMBER_RATE));
			}
			...
			{
				newprize = getPrize() + prize2 + prize3;
			}
		...
		try
		{
			...
			if (rset.next())
			{
				...
				switch (count)
				{
					...
					default:
						res[0] = 4;
						res[1] = (int) (getPrize() / Config.SERVICES_LOTTERY_2_AND_1_NUMBER_PRIZE);
				}
			}
		}
		catch (SQLException e)
