...
import l2r.gameserver.model.entity.events.impl.KrateisCubeRunnerEvent;
...
public final class HandysBlockCheckerManager
{
	...
	public boolean addPlayerToArena(Player player, int arenaId)
	{
		...
		synchronized (holder)
		{
			...
			/*
			 * if(UnderGroundColiseum.getInstance().isRegisteredPlayer(player)) { UngerGroundColiseum.getInstance().removeParticipant(player); player.sendPacket(new SystemMessage(SystemMessageId.COLISEUM_OLYMPIAD_KRATEIS_APPLICANTS_CANNOT_PARTICIPATE)); }
			 */
	...
	public void changePlayerToTeam(Player player, int arena, int team)
	{
		...
		synchronized (holder)
		{
			...
			{
				player.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.games.handysblockcheckermanager.message2", player));
				return;
			}
			else if (!isFromRed && (holder.getRedTeamSize() == 6))
			{
				player.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.games.handysblockcheckermanager.message3", player));
				return;
			}
	...
	public static boolean isRegistered(Player player)
	{
		for (int i = 0; i < 4; i++)
		{
			if ((_arenaPlayers != null) && (_arenaPlayers[i] != null) && (_arenaPlayers[i].getAllPlayers() != null) && _arenaPlayers[i].getAllPlayers().contains(player))
			{
				return true;
			}
		}
