...
public class ActivityReward implements OnDeathListener
{
	...
	private void start()
	{
		...
		ThreadPoolManager.getInstance().scheduleAtFixedRate(() ->
		{
			...
			{
				if ((n3.getValue() + 1800000) < System.currentTimeMillis())
				{
					continue;
				}
				
				Player player = World.getPlayer(n3.getKey());
				// if (rewardedHWIDs.contains(player.getHWID()))
				// continue;
				...
				for (Integer[] reward : _rewards)
				{
					...
					player.getInventory().addItem(reward[0], Rnd.get(reward[1], reward[2]), ""); // 0 = ItemId, 1 = minCount, 2 = maxCount
	...
	{
		if (Config.ACTIVITY_REWARD_ENABLED && actor.isMonster() && (killer != null) && (killer != actor) && killer.isPlayer() && !killer.getPlayer().isPhantom())
		{
