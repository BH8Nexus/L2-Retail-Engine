...
import l2r.gameserver.model.AutoAnnounces;
...
public class AutoAnnounce implements Runnable
{
	...
	{
		load();
		_log.info("AutoAnnounce: Reloading autoannounce...");
	...
	private void load()
	{
		try
		{
			...
			File file = Config.findNonCustomResource("config/autoannounce.xml");
			if (!file.exists())
			{
				_log.warn("AutoAnnounce: NO FILE at " + Config.findNonCustomResource("config/autoannounce.xml").getAbsolutePath());
				return;
			}
