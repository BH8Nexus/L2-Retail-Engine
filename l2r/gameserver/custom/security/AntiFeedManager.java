 ...
 */
package l2r.gameserver.custom.security;
...
public class AntiFeedManager
{
	...
	public final boolean check(Creature attacker, Creature target)
	{
		...
		// - Chequeamos diferencias de level maximo
		if ((Config.ANTIFEED_MAX_LVL_DIFFERENCE > 0) && ((attackerPlayer.getLevel() - target.getLevel()) > Config.ANTIFEED_MAX_LVL_DIFFERENCE))
		{
			return false;
		}
		
		// - Chequeamos si esta en el mismo clan o ally
		if ((targetPlayer.getClanId() > 0) && (targetPlayer.getClanId() == attackerPlayer.getClanId()))
		{
			return false;
		}
