...
public abstract class AbstractDailyQuest extends AbstractDPScript
{
	...
	protected void showInfoTemplate(Player player, int index)
	{
		...
		if (st == null)
		{
			html = html.replace("%startAbort%", "<button action=\"bypass _bbs_daily_quests;start;" + getName() + "\" value=\"Start\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\">"); // empty place holder
		}
		else if (st.getState() == STARTED)
		{
			html = html.replace("%startAbort%", "<button action=\"bypass _bbs_daily_quests;abort;" + getName() + "\" value=\"Abort\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\">"); // empty place holder
		}
