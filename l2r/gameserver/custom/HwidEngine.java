...
import l2r.gameserver.custom.HwidGamer.PLAYER_THREAT;
import l2r.gameserver.custom.HwidLogging.SimpleLog;
import l2r.gameserver.database.DatabaseFactory;
import l2r.gameserver.model.Player;
import l2r.gameserver.model.entity.ChangeLogManager;
import l2r.gameserver.utils.BatchStatement;
...
public class HwidEngine
{
	...
	public String canILog(String hwid)
	{
		...
		final long banTime = getBanLeftTime(hwid);
		if (banTime == -100)
			return "You are banned!";
		if (banTime > 0)
			return "You are banned for "+(int)Math.ceil(banTime/60000)+" more minutes!";
		return null;
	}
	...
	{
		HwidGamer newGamer = new HwidGamer(hwid, System.currentTimeMillis()/1000L, 0, -1, 0, ChangeLogManager.getInstance().getLatestChangeId(), PLAYER_THREAT.NONE, 0);
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("UPDATE hwid SET banned=? WHERE HWID=?"))
		{
