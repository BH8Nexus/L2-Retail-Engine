...
public class AdminEffects implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch (command)
		{
				...
				if (wordList.length > 1)
				{
					...
					if (player != null)
					{
						...
						else
						{
							...
							player.broadcastCharInfo(); // Broadcast so GMs can see hidden.
							World.removeObjectFromPlayers(player);
						}
					}
					else
					{
						PetInstance pet = (PetInstance) GameObjectsStorage.getAllPlayersStream().map(plr -> plr.getPet()).filter(p -> p.getName().equalsIgnoreCase(wordList[1])).findAny().orElse(null);
						if (pet.isInvisible())
						{
							pet.setInvisibleType(InvisibleType.NONE);
							for (GameObject obj : World.getAroundObjects(pet))
							{
								if ((obj == null) || !obj.isPlayer() || (obj.getReflectionId() != pet.getReflectionId()) || (pet.getPlayer() == obj))
								{
									continue;
								}
								
								((Player) obj).sendPacket(((Player) obj).addVisibleObject(pet));
							}
							pet.broadcastCharInfo();
						}
						else
						{
							pet.setInvisibleType(InvisibleType.NORMAL);
							pet.broadcastCharInfo(); // Broadcast so GMs can see hidden.
							for (GameObject obj : World.getAroundObjects(pet))
							{
								if ((obj == null) || !obj.isPlayer() || (obj.getReflectionId() != pet.getReflectionId()) || (pet.getPlayer() == obj))
								{
								...
								((Player) obj).sendPacket(((Player) obj).removeVisibleObject(pet, pet.deletePacketList()));
							}
						}
					}
				}
			...
			case admin_gmspeed:
				...
				if (val == 0)
				{
					if (sh_level != 0)
					{
						activeChar.doCast(SkillTable.getInstance().getInfo(7029, sh_level), activeChar, true); // Ρ�Π½ΠΈΠΌΠ°ΠµΠΌ ΠµΡ„Ρ„ΠµΠΊΡ‚
					}
		...
		switch (command)
		{
			...
			case admin_changename:
				...
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message7", activeChar).addString(oldName).addString(newName));
				break;
			case admin_setinvul:
			...
			case admin_getinvul:
				if ((target != null) && target.isCreature())
				{
					...
					{
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message8", activeChar).addString(target.getName()).addNumber(target.getObjectId()));
					}
					else
					{
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message9", activeChar).addString(target.getName()).addNumber(target.getObjectId()));
					}
				}
				break;
				...
				try
				{
					...
					if (radius > 0)
					{
						for (Creature trgt : World.getAroundCharacters(activeChar, radius, 500))
						{
							if (ae == AbnormalEffect.NULL)
							{
								...
								// Original Message: Abnormal effects cleared by admin.
								trgt.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message11"));
								if (trgt != activeChar)
								{
									// Original Message: Abnormal effects cleared.
									trgt.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message12"));
								}
							}
							else
							{
								trgt.startAbnormalEffect(ae);
								// Original Message: Admin added abnormal effect: " + ae.getName()
								trgt.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message13").addString(ae.getName()));
								if (trgt != activeChar)
								{
									// Oriiginal Message: Added abnormal effect: " + ae.getName()
									trgt.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message14").addString(ae.getName()));
								}
							}
						}
					}
					else
					{
						...
						if (ae == AbnormalEffect.NULL)
						{
							...
							// Original Message: Abnormal effects cleared by admin.
							effectTarget.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message15"));
							if (effectTarget != activeChar)
							{
								// Original Message: Abnormal effects clearned.
								effectTarget.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message16"));
							}
						}
						else
						{
							...
							// Original Message: Admin added abnormal effect: " + ae.getName()
							effectTarget.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message17").addString(ae.getName()));
							if (effectTarget != activeChar)
							{
								// Original Message: Added abnormal effect: " + ae.getName()
								effectTarget.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message18").addString(ae.getName()));
							}
						}
					}
				}
				...
				try
				{
					...
					for (Creature plr : activeChar.getAroundCharacters(id, 1000))
					{
						if ((plr != null) && plr.isPlayer() && !plr.isParalyzed())
						{
							...
							// Original Message: You have been paralyzed by a GM.
							plr.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message25"));
						}
					}
					
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message26", activeChar).addNumber(id));
				}
				catch (final NumberFormatException aiooe)
				{
				...
				try
				{
					...
					for (Creature plr : activeChar.getAroundCharacters(id, 1000))
					{
						if ((plr != null) && plr.isPlayer() && plr.isParalyzed())
						{
							...
							// Original Message: Paralyze has been realeased.
							plr.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message28"));
						}
					}
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message29", activeChar).addNumber(id));
				}
				catch (final NumberFormatException aiooe)
				{
			...
			case admin_effect:
				try
				{
					...
					else
					{
						...
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message32", activeChar).addString(obj.getName()).addNumber(skill).addNumber(level));
					}
			...
			case admin_play_sounds:
				try
				{
					...
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message34", activeChar).addString(wordList[1]));
				}
	...
	private void handleInvul(Player activeChar, Player target)
	{
		if (target.isInvul())
		{
			...
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message40", activeChar).addString(target.getName()));
		}
		else
		{
			...
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message41", activeChar).addString(target.getName()));
		}
	}
	...
	private void handleTransform(Player activeChar, int transformation, int radius)
	{
		...
		if (radius > 0)
		{
			...
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineffects.message42", activeChar).addNumber(radius));
			return;
		}
