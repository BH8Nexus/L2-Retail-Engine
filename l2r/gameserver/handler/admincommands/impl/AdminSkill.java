...
public class AdminSkill implements IAdminCommandHandler
{
		...
		switch (command)
		{
				...
				try
				{
					if (wordList.length > 1)
					{
						...
						if (radius > 0)
						{
							...
							activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message3", activeChar).addNumber(radius));
						}
						else
						{
							...
							if (player != null)
							{
								...
								activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message5", activeChar).addString(player.getName()));
							}
						}
					}
					...
					if (player != null)
					{
						...
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message7", activeChar).addString(player.getName()));
					}
					else
					{
			...
			case admin_buff:
				if ((activeChar.getTarget() != null) && activeChar.getTarget().isCreature())
				{
					try
					{
						...
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message10", activeChar).addString(activeChar.getTarget().getName()).addString(skill.getName()).addNumber(level));
					}
					catch (Exception e)
					{
			...
			case admin_areabuff:
				try
				{
					...
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message12", activeChar).addNumber(radius).addString(skill.getName()).addNumber(level));
				}
				catch (Exception e)
				{
			...
			case admin_serverbuff:
				try
				{
					...
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message14", activeChar).addString(skill.getName()).addNumber(level));
				}
				catch (Exception e)
				{
			...
			case admin_cancel:
				if ((activeChar.getTarget() != null) && activeChar.getTarget().isCreature())
				{
					...
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message16", activeChar).addString(activeChar.getTarget().getName()));
				}
				else
				{
			...
			case admin_areacancel:
				try
				{
					...
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message18", activeChar).addNumber(radius));
				}
				catch (Exception e)
				{
					...
					if (objId > 0)
					{
						...
						if (target != null)
						{
							...
							else
							{
								...
								activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message20", activeChar).addString(target.getName()));
							}
	...
	private void adminGiveAllSkills(Player activeChar)
	{
		...
		player.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message22", player).addNumber(skillCounter));
		player.sendPacket(new SkillList(player));
		activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message23", activeChar).addNumber(skillCounter).addString(player.getName()));
	}
	...
	private void removeSkillsPage(Player activeChar, int page)
	{
		...
		int maxfpages = (int) Math.round((skills.size() / 18.0) + 1);
		int maxbpages = (int) Math.round((skills.size() / 12.0) + 1);
	...
	private void showEffects(Player activeChar, GameObject targetObject, int page)
	{
		...
		for (int x = 0; x < max; x++)
		{
			...
			{
				html.append("<td><a action=\"bypass admin_getbuffs ");
	...
	private void adminGetSkills(Player activeChar)
	{
		...
		else
		{
			...
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message25", activeChar).addString(player.getName()));
		}
	...
	private void adminResetSkills(Player activeChar)
	{
		...
		player.sendPacket(new SkillList(player));
		player.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message26", player).addString(activeChar.getName()));
		activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message27", activeChar).addNumber(counter));
	...
	private void adminAddSkill(Player activeChar, String[] wordList)
	{
		...
		if (wordList.length < 2)
		{
			activeChar.sendMessage("Usage: //add_skill skillId [level]");
			return;
		}
		
		int id = Integer.parseInt(wordList[1]);
		int level = wordList.length >= 3 ? Integer.parseInt(wordList[2]) : 1;
		...
		{
			level = SkillTable.getInstance().getBaseLevel(id);
		}
		
		Skill skill = SkillTable.getInstance().getInfo(id, level);
		if (skill != null)
		{
			player.sendMessage("You have received the skill " + skill.getName() + "(" + skill.getLevel() + ")");
			player.addSkill(skill, true);
			player.sendPacket(new SkillList(player));
			activeChar.sendMessage("You have given " + skill.getName() + "(" + skill.getLevel() + ") to " + player.getName());
		}
		else
		{
	...
	private void adminRemoveSkill(Player activeChar, String[] wordList)
	{
		...
		if (wordList.length >= 2)
		{
			...
			if (skill != null)
			{
				player.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message31", player).addString(skill.getName()));
				player.removeSkill(skill, true);
				player.sendPacket(new SkillList(player));
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminskill.message32", activeChar).addString(skill.getName()).addString(player.getName()));
			}
			else
			{
