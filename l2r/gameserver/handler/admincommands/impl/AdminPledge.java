		...
		if (fullString.startsWith("admin_pledge"))
		{
			...
			else if (action.equals("setlevel"))
			{
				...
				try
				{
					...
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminpledge.message1", activeChar).addNumber(level).addString(clan.getName()));
					clan.setLevel(level);
					clan.updateClanInDB();
					
					/*
					 * if(level < CastleSiegeManager.getSiegeClanMinLevel()) SiegeUtils.removeSiegeSkills(target); else SiegeUtils.addSiegeSkills(target);
					 */
			...
			else if (action.equals("resetcreate"))
			{
				...
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminpledge.message2", activeChar).addString(target.getName()));
				activeChar.sendHtml("admin/pledgemanage.htm");
			}
			else if (action.equals("resetwait"))
			{
				target.setLeaveClanTime(0);
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminpledge.message3", activeChar).addString(target.getName()));
				activeChar.sendHtml("admin/pledgemanage.htm");
			}
			else if (action.equals("addrep"))
			{
				try
				{
					...
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminpledge.message4", activeChar).addNumber(rep).addString(target.getClan().getName()));
					activeChar.sendHtml("admin/pledgemanage.htm");
				}
			...
			else if (action.equals("setname"))
			{
				...
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminpledge.message9", activeChar).addString(target.getName()).addString(target.getClan().getName()).addString(newClanName));
