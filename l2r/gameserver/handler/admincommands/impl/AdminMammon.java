			...
			if(npcIdList.contains(npcId))
			{
				...
				if(npcInst.isVisible())
				{
					...
					if(teleportIndex > -1)
					{
						...
						// Original Message: Index: " + index + " - " + npcInst.getName() + " (" + npcInst.getObjectId() + "): " + npcInst.getX() + " " + npcInst.getY() + " " + npcInst.getZ()
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminmammon.message3", activeChar).addNumber(index).addString(npcInst.getName()).addNumber(npcInst.getObjectId()).addNumber(npcInst.getX()).addNumber(npcInst.getY()).addNumber(npcInst.getZ()));
				}
			}
		}
	}
