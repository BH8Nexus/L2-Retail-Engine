...
public class AdminAttribute implements IAdminCommandHandler
{
	...
	private void setEnchant(Player target, int value, byte element, int armorType)
	{
		...
		if (item != null)
		{
			...
			target.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminattribute.message6", target).addString(ElementName).addNumber(value).addString(item.getName()).addNumber(curEnchant));
			player.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminattribute.message7", player).addString(ElementName).addNumber(value).addString(item.getName()).addNumber(curEnchant));
		}
	}
