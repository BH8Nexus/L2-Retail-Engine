		...
		switch (command)
		{
			case admin_setpremium:
			{
				...
				try (Connection con = DatabaseFactory.getInstance().getConnection();
					PreparedStatement statement = con.prepareStatement("REPLACE INTO account_bonus(account, bonus, bonus_expire) VALUES (?,?,?)"))
				{
				...
				target.getClient().setBonus(premiumId);
				target.getClient().setBonusExpire(newBonusTime);
			...
			case admin_removepremium:
			{
				...
				// Remove the bonus
				try (Connection con = DatabaseFactory.getInstance().getConnection();
					PreparedStatement statement = con.prepareStatement("DELETE FROM account_bonus WHERE account=?");)
				{
					statement.setString(1, target.getAccountName());
				...
				target.getClient().setBonus(0);
				target.getClient().setBonusExpire(0);
			...
			case admin_premiumall:
			{
				...
				for (Player player : GameObjectsStorage.getAllPlayers())
				{
					...
					if (player.getClient().getBonusExpire() > current)
					{
						continue;
					}
					...
					player.getClient().setBonus(premiumId);
					player.getClient().setBonusExpire(newBonusTime);
