...
public class AdminQuests implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_quest:
				...
				{
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message1", activeChar));
					return true;
				}
				Quest _quest = QuestManager.getQuest2(wordList[1]);
				if(_quest == null)
				{
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message2", activeChar).addString(wordList[1]));
					return true;
				}
	...
	private boolean cmd_Clear(Quest _quest, String[] wordList, Player activeChar)
	{
		...
		if(qs == null)
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message3", activeChar).addString(targetChar.getName()).addString(_quest.getName()));
			return false;
		}
	...
	private boolean cmd_Show(Quest _quest, String[] wordList, Player activeChar)
	{
		...
		if(qs == null)
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message4", activeChar).addString(targetChar.getName()).addString(_quest.getName()));
			return false;
		}
	...
	private boolean cmd_Var(Quest _quest, String[] wordList, Player activeChar)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message5", activeChar));
			return false;
		}
		...
		if(qs == null)
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message6", activeChar).addString(targetChar.getName()).addString(_quest.getName()));
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message7", activeChar));
			return false;
		}
	...
	private boolean cmd_State(Quest _quest, String[] wordList, Player activeChar)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message8", activeChar));
			return false;
		}
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message9", activeChar).addString(wordList[3]));
			return false;
		}
		...
		if(qs == null)
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message10", activeChar).addString(_quest.getName()).addString(targetChar.getName()));
	...
	{
		// Ρ†ΠµΠ»Ρ� Π·Π°Π΄Π°Π½Π° Π°Ρ€Π³ΡƒΠΌΠµΠ½Ρ‚ΠΎΠΌ
		if(wordListIndex >= 0 && wordList.length > wordListIndex)
		{
			Player player = World.getPlayer(wordList[wordListIndex]);
			if(player == null)
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminquests.message11", activeChar).addString(wordList[wordListIndex]));
			return player;
		}
		// Ρ†ΠµΠ»Ρ� Π·Π°Π΄Π°Π½Π° Ρ‚ΠµΠΊΡƒΡ‰ΠΈΠΌ Ρ‚Π°Ρ€Π³ΠµΡ‚ΠΎΠΌ
		GameObject my_target = activeChar.getTarget();
		if(my_target != null && my_target.isPlayer())
			return (Player) my_target;
		// Π² ΠΊΠ°Ρ‡ΠµΡ�Ρ‚Π²Πµ Ρ†ΠµΠ»ΠΈ Ρ�Π°ΠΌ Π°Π΄ΠΌΠΈΠ½
		return activeChar;
	}
