...
public class AdminKill implements IAdminCommandHandler
{
	...
	private void handleKill(Player activeChar, String player, boolean everything)
	{
		...
		if(player != null)
		{
			...
			else
			{
				...
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminkill.message1", activeChar).addNumber(radius));
				return;
			}
		}
	...
	private void handleDamage(Player activeChar, int damage)
	{
		...
		activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminkill.message2", activeChar).addNumber(damage).addString(cha.getName()));
	}
}