	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch (command)
		{
			case admin_cw_info:
				...
				{
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincursedweapons.message4", activeChar).addString(c.getName()).addNumber(c.getItemId()));
					if (c.isActivated())
					{
						Player pl = c.getPlayer();
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincursedweapons.message5", activeChar).addString(pl.getName()));
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincursedweapons.message6", activeChar).addNumber(c.getPlayerKarma()));
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincursedweapons.message7", activeChar).addNumber(c.getTimeLeft() / 60000));
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincursedweapons.message8", activeChar).addNumber(c.getNbKills()));
					}
					else if (c.isDropped())
					{
						...
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincursedweapons.message10", activeChar).addNumber(c.getTimeLeft() / 60000));
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincursedweapons.message11", activeChar).addNumber(c.getNbKills()));
					}
					else
					{
			...
			case admin_cw_add:
				...
				else
				{
					...
					if ((target != null) && target.isPlayer() && !((Player) target).isInOlympiadMode())
					{
						...
						ItemInstance item = new ItemInstance(cw.getItemId());
						
						// Π�Π·ΠΌΠµΠ½ΠΈΠ» ΠΊΠΎΠΌΠ°Π½Π΄Ρƒ Π΄Π»Ρ� Π²Ρ‹Π΄Π°Ρ‡ΠΈ ΠΏΡ€ΠΎΠΊΠ»Ρ�Ρ‚ΠΎΠ³ΠΎ ΠΎΡ€ΡƒΠ¶ΠΈΡ�, Ρ‚ΠµΠΏΠµΡ€Ρ� Ρ€Π°Π±ΠΎΡ‚Π°ΠµΡ‚ Ρ…ΠΎΡ€ΠΎΡ�ΠΎ.
				...
				break;
			case admin_cw_increase:
				// Π£Π²ΠµΠ»ΠΈΡ‡ΠΈΠ²Π°ΠµΡ‚ ΠΊΠΎΠ»-Π²ΠΎ ΡƒΠ±ΠΈΠΉΡ�Ρ‚Π² Ρƒ Ρ†ΠµΠ»ΠΈ-Π²Π»Π°Π΄ΠµΠ»Ρ�Ρ†Π° ΠΏΡ€ΠΎΠΊΠ»Ρ�Ρ‚ΠΎΠ³ΠΎ ΠΎΡ€ΡƒΠ¶ΠΈΡ�.
				if (cw == null)
				{
					return false;
				}
