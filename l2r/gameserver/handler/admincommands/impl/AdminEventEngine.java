...
import l2r.gameserver.model.entity.L2Event.EventState;
...
public class AdminEventEngine implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		try
		{
			...
			else if (actualCommand.startsWith("admin_add"))
			{
				...
				tempBuffer += fullString.substring(10);
				showNewEventPage(activeChar);
			...
			else if (actualCommand.startsWith("admin_event_see"))
			{
				...
				String eventName = fullString.substring(16);
				try
				{
					NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
					
					DataInputStream in = new DataInputStream(new BufferedInputStream(new FileInputStream(Config.DATAPACK_ROOT + "/data/gmevents/" + eventName)));
					BufferedReader inbr = new BufferedReader(new InputStreamReader(in));
					
					String html = HtmCache.getInstance().getNotNull("EventEngine/Participation.htm", activeChar);
			...
			else if (actualCommand.startsWith("admin_event_del"))
			{
				// There is an exception here for not using the ST. We use spaces (ST delim) for the event name.
				String eventName = fullString.substring(16);
				File file = new File(Config.DATAPACK_ROOT + "/data/gmevents/" + eventName);
				file.delete();
				showMainPage(activeChar);
			}
			...
			{
				// There is an exception here for not using the ST. We use spaces (ST delim) for the event name.
				tempName += fullString.substring(17);
				showNewEventPage(activeChar);
			}
			...
			else if (actualCommand.startsWith("admin_event_store"))
			{
				try
				{
					FileOutputStream file = new FileOutputStream(new File(Config.DATAPACK_ROOT, "data/gmevents/" + tempName));
			...
			{
				// There is an exception here for not using the ST. We use spaces (ST delim) for the event name.
				L2Event._eventName = fullString.substring(16);
			...
			else if (actualCommand.startsWith("admin_event_announce"))
			{
				...
				PlaySound _snd = new PlaySound("B03_F");
			...
			else if (actualCommand.startsWith("admin_event_control_teleport"))
			{
				while (st.hasMoreElements()) // Every next ST should be a team number
				{
					...
					for (Player player : L2Event._teams.get(teamId))
					{
						...
						player.teleToLocation(activeChar.getLoc(), activeChar.getReflectionId());
					}
				}
				showEventControl(activeChar);
			}
			...
			else if (actualCommand.startsWith("admin_event_control_kill"))
			{
				while (st.hasMoreElements()) // Every next ST should be a team number
				{
					for (Player player : L2Event._teams.get(Integer.parseInt(st.nextToken())))
					{
						player.reduceCurrentHp(player.getMaxHp() + player.getMaxCp() + 1, activeChar, null, true, true, true, false, false, false, true);
					}
				}
				showEventControl(activeChar);
			}
			...
			else if (actualCommand.startsWith("admin_event_control_transform"))
			{
				...
				for (Player player : L2Event._teams.get(teamId))
				{
					...
					player.setTransformation(transId);
				}
				showEventControl(activeChar);
			}
			...
			else if (actualCommand.startsWith("admin_event_control_kick"))
			{
				if (st.hasMoreElements()) // If has next token, it should be player name.
				{
					while (st.hasMoreElements())
					{
						Player player = World.getPlayer(st.nextToken());
						if (player != null)
						{
		...
		{
			e.printStackTrace();
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admineventengine.message1", activeChar));
		}
		return true;
	}
	...
	{
		final File dir = new File(Config.DATAPACK_ROOT, "/data/gmevents");
		if (dir.isFile())
		{
	...
	private void rewardTeam(Player activeChar, int team, int n, int id, String type)
	{
		...
		for (Player player : L2Event._teams.get(team))
		{
			...
			Functions.addItem(player, id, num, "");
			NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
