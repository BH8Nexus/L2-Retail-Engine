		...
		switch (command)
		{
			case admin_wipe:
			{
				...
				try
				{
					...
					{
						// Original Message: Usage: //wipe itemId; //wipe itemId count; //wipe playerName itemId; //wipe playerName itemId count. If player's name is only numbers, use the last example, because command treats name as ItemId. Error: {0}.
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminwipe.message7", activeChar).addString(e.getMessage()));
						return false;
					}
					
					if (target == null)
					{
						// Original Message: Target not found.
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminwipe.message8", activeChar));
						return false;
					}
					...
					if (item != null)
					{
						target.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminwipe.message3", target).addString(activeChar.getName()).addNumber(itemCount).addString(item.getName()));
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminwipe.message4", activeChar).addNumber(itemCount).addString(item.getName()).addString(target.getName()));
					}
			...
			case admin_wipewh:
			{
				...
				try
				{
					...
					{
						// Original Message: Usage: //wipewh itemId; //wipe itemId count; //wipewh playerName itemId; //wipewh playerName itemId count. If player's name is only numbers, use the last example, because command treats name as ItemId. Error: {0}.
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminwipe.message9", activeChar).addString(e.getMessage()));
						return false;
					}
