...
import l2r.gameserver.model.entity.tournament.BattleRecord;
import l2r.gameserver.model.entity.tournament.BattleScheduleManager;
import l2r.gameserver.model.entity.tournament.TournamentStatus;
...
public class AdminTournament implements IAdminCommandHandler
{
	...
	private static int deleteMails(String mailTitle) throws SQLException
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT message_id FROM mail WHERE sender_id = ? AND topic LIKE ?"))
		{
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("DELETE FROM tournament_battles WHERE id = ?"))
		{
