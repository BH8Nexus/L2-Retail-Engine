	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_s_new:
				...
				int time1 = 100 + text1.length() * 100; 
				ExShowScreenMessage sm1 = new ExShowScreenMessage(text1 + " icon.skill0000", time1, ScreenMessageAlign.TOP_CENTER, text1.length() < 64);
				for(Player player : GameObjectsStorage.getAllPlayersForIterate())
			...
			case admin_reload_announcements:
				...
				listAnnouncements(activeChar);
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminannouncements.reloaded", activeChar));
				break;
		}
