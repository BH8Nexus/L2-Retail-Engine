...
public class AdminMenu implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		else if(fullString.equals("admin_kill_menu"))
		{
			...
			if(st.countTokens() > 1)
			{
				...
				if(plyr == null)
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminmenu.message1", activeChar).addString(player));
				obj = plyr;
			}
	...
	private void teleportToCharacter(Player activeChar, GameObject target)
	{
		...
		else
		{
			...
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminmenu.message6", activeChar).addString(player.getName()));
		}
	}
}