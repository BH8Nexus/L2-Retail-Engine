		...
		switch(command)
		{
			...
			case admin_script_load:
				...
				if(param.equalsIgnoreCase("all"))
				{
					...
					if(!Scripts.getInstance().reload())
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminscripts.message2", activeChar).addNumber(Scripts.getInstance().getClasses().size()));
					else
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminscripts.message3", activeChar).addNumber(Scripts.getInstance().getClasses().size()));
				}
			...
			case admin_quest_load:
				...
				if(!Scripts.getInstance().reload("quests/" + quest))
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminscripts.message6", activeChar).addString(quest));
				else
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminscripts.message7", activeChar).addString(quest));
