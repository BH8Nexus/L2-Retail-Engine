	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_searchitem:
				if (wordList.length > 1)
				{
					...
					for (ItemTemplate tmpl : ItemHolder.getInstance().getAllTemplates())
					{
						...
						if (tmpl != null && tmpl.getName().toLowerCase().contains(search.toLowerCase()))
						{
							...
							sb.append("</td><td>");
							sb.append("<button value=" + tmpl.getItemId() + " action=\"bypass -h admin_create_item " + tmpl.getItemId() + " " + 1 + "\" width=32 height=23 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">");
							sb.append("</td></tr>");
						}
					}
				...
				break;
			case admin_tax:
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminshop.message2", activeChar).addNumber(GameStats.getTaxSum()));
				break;
			case admin_taxclear:
				GameStats.addTax(-GameStats.getTaxSum());
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.adminshop.message3", activeChar).addNumber(GameStats.getTaxSum()));
				break;
		}
