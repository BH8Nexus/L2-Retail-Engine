	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_create_instance:
				...
				refl.init(iz);
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admininstance.message1", activeChar).addNumber(refl.getId()).addNumber(iz.getId()));
				break;
				...
				if (wordList.length > 2)
				{
					...
					if ("party".equalsIgnoreCase(wordList[2]))
					{
						if (activeChar.isInParty())
							...
							// Original Message: You are not in a party.
							activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admininstance.message3", activeChar));
					}
					else if ("cc".equalsIgnoreCase(wordList[2]))
					{
						if (activeChar.isInParty() && activeChar.getParty().isInCommandChannel())
							...
							// Original Message: You are not in a command channel.
							activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admininstance.message4", activeChar));
					}
					...
					{
						// Original Message: Usage: //instance_reset <instaceId> [party|cc|rangeAroundYou]
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admininstance.message5", activeChar));
					}
					
					int instanceId = NumberUtils.toInt(wordList[1]);
					int count = 0;
					...
					// Original Message: Instance reuse for " + instanceId + " has been removed for " + count + " players.
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admininstance.message6", activeChar).addNumber(instanceId).addNumber(count));
				}
			...
			case admin_instance_all_reset:
				if (wordList.length > 1)
				{
					...
					// Original Message: Instance reuse for all instances has been removed for " + count + " players.
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admininstance.message7", activeChar, count));
				}
