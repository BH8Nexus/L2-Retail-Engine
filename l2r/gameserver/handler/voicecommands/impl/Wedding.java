...
public class Wedding implements IVoicedCommandHandler
{
	private static class CoupleAnswerListener implements OnAnswerListener
	{
		...
		public void sayYes()
		{
			...
			player1.sendMessage(new CustomMessage("l2r.gameserver.model.L2Player.EngageAnswerYes", player2));
		}
		...
		public void sayNo()
		{
			...
			player1.sendMessage(new CustomMessage("l2r.gameserver.model.L2Player.EngageAnswerNo", player2));
		}
	}
	...
	public boolean divorce(Player activeChar)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.Divorced", activeChar));
			AdenaAmount = Math.abs(((activeChar.getAdena() / 100) * Config.WEDDING_DIVORCE_COSTS) - 10);
			activeChar.reduceAdena(AdenaAmount, true, "");
		}
		else
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.Disengaged", activeChar));
		}
		...
		if (partner != null)
		{
			...
			if (partner.isMaried())
			{
				partner.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.PartnerDivorce", partner));
			}
			else
			{
				partner.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.PartnerDisengage", partner));
			}
	...
	public boolean engage(final Player activeChar)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.NoneTargeted", activeChar));
			return false;
		}
		// check if target is a L2Player
		if (!activeChar.getTarget().isPlayer())
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.OnlyAnotherPlayer", activeChar));
			return false;
		}
		// check if player is already engaged
		if (activeChar.getPartnerId() != 0)
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.AlreadyEngaged", activeChar));
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.EngagingYourself", activeChar));
			return false;
		}
		
		if (ptarget.isMaried())
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.PlayerAlreadyMarried", activeChar));
			return false;
		}
		
		if (ptarget.getPartnerId() != 0)
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.PlayerAlreadyEngaged", activeChar));
			return false;
		}
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.PlayerAlreadyAsked", activeChar));
			return false;
		}
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.SameSex", activeChar));
			return false;
		}
		
		// TODO [G1ta0] Π ΠµΠ°Π»ΠΈΠ·ΠΎΠ²Π°Ρ‚Ρ� Π½ΠΎΡ€ΠΌΠ°Π»Ρ�Π½Ρ‹ΠΉ Ρ�ΠΏΠΈΡ�ΠΎΠΊ Π΄Ρ€ΡƒΠ·ΠµΠΉ
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.NotInFriendlist", activeChar));
			return false;
		}
	...
	public boolean goToLove(Player activeChar)
	{
		if (!activeChar.isMaried())
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.YoureNotMarried", activeChar));
			return false;
		}
		
		if (activeChar.getPartnerId() == 0)
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.PartnerNotInDB", activeChar));
			return false;
		}
		...
		if (partner == null)
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.PartnerOffline", activeChar));
			return false;
		}
		
		if (partner.isInJail() || activeChar.isInJail() || partner.isInOlympiadMode() || partner.isFestivalParticipant() || activeChar.isMovementDisabled() || activeChar.isMuted(null) || activeChar.isInOlympiadMode() || activeChar.isInDuel() || activeChar.isFestivalParticipant() || activeChar.getPlayer().isTerritoryFlagEquipped() || partner.isInZone(no_summon))
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.TryLater", activeChar));
			return false;
		}
		...
		// "Π�ΠµΠ»Ρ�Π·Ρ� Π²Ρ‹Π·Ρ‹Π²Π°Ρ‚Ρ� ΠΏΠµΡ€Ρ�ΠΎΠ½Π°Π¶ΠµΠΉ Π²/ΠΈΠ· Π·ΠΎΠ½Ρ‹ Ρ�Π²ΠΎΠ±ΠΎΠ΄Π½ΠΎΠ³ΠΎ PvP"
		// "Π² Π·ΠΎΠ½Ρ‹ ΠΎΡ�Π°Π΄"
		// "Π½Π° Π�Π»ΠΈΠΌΠΏΠΈΠΉΡ�ΠΊΠΈΠΉ Ρ�Ρ‚Π°Π΄ΠΈΠΎΠ½"
		// "Π² Π·ΠΎΠ½Ρ‹ ΠΎΠΏΡ€ΠµΠ΄ΠµΠ»ΠµΠ½Π½Ρ‹Ρ… Ρ€ΠµΠΉΠ΄-Π±ΠΎΡ�Ρ�ΠΎΠ² ΠΈ Ρ�ΠΏΠΈΠΊ-Π±ΠΎΡ�Ρ�ΠΎΠ²"
		// Π² Ρ€ΠµΠ¶ΠΈΠΌΠµ ΠΎΠ±Ρ�ΠµΡ€Π²ΠµΡ€Π° ΠΈΠ»ΠΈ ΠΊ ΠΎΠ±Ρ�ΠµΡ€Π²ΠµΡ€Ρƒ
		...
		activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.Wedding.Teleport", activeChar).addNumber(teleportTimer / 60));
