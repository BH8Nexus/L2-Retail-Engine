			...
			if(args != null)
			{
				...
				if(param.length > 0)
					if(param[0].equalsIgnoreCase("allowwh") && param.length > 1)
					{
						...
						if(cm != null && cm.getPlayer() != null) // Ρ†ΠµΠ»Ρ� ΠΎΠ½Π»Π°ΠΉΠ½
						{
							if(cm.getPlayer().getVarB("canWhWithdraw"))
							{
								cm.getPlayer().unsetVar("canWhWithdraw");
								activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.cwhprivileges.message1", activeChar));
							}
							else
							{
								cm.getPlayer().setVar("canWhWithdraw", "1", -1);
								activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.cwhprivileges.message2", activeChar));
							}
						}
						else if(cm != null) // Ρ†ΠµΠ»Ρ� ΠΎΡ„Ρ„Π»Π°ΠΉΠ½
						{
							...
							if(state > 0)
							{
								...
								activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.cwhprivileges.message3", activeChar));
							}
							else
							{
								...
								activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.cwhprivileges.message4", activeChar));
							}
						}
						else
							activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.voicecommands.impl.cwhprivileges.message5", activeChar));
					}
