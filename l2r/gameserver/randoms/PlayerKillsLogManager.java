...
{
	TIntObjectHashMap<PlayerKillsLog> _logs = new TIntObjectHashMap<>();
	...
	public String toString(SortBy sort)
	{
		...
		{
			TreeMap<String, PlayerKillsLog> sortedMap = new TreeMap<>();
	...
	private final class PlayerKillsLog extends FastTable<KillLog>
	{
		...
		{
			StringBuilder sb = new StringBuilder();
			List<KillLog> tmp = new FastTable<>();
			tmp.addAll(this);
	...
	public final class KillLog
	{
		...
		{
			String diff = (killDatesDiff.get(Calendar.MINUTE) > 0 ? (killDatesDiff.get(Calendar.MINUTE) + " minutes") : killDatesDiff.get(Calendar.SECOND) > 0 ? (killDatesDiff.get(Calendar.SECOND) + " seconds") : killDatesDiff.get(Calendar.MILLISECOND) > 0 ? (killDatesDiff.get(Calendar.MILLISECOND) + " miliseconds") : "0");
			StringBuilder sb = new StringBuilder();
			sb.append("[");
			sb.append(killer);
			sb.append(" killed ");
