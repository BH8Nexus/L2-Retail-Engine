...
public class CaptchaImage implements OnDeathListener
{
	...
	public static void increaseCaptchaRequests(Player player)
	{
		...
		fullVal &= ~(0x1F << CAPTCHA_REQUESTS); // creates 5 bits all set to 1, then shifts them to the left, then reverts them to 0s and all 0s to 1s, so you get a long value consisting only of 1s except for the place of the 5 bits. Wherever the location of those 5 bits is, all data there will be
												// erased (set to 0s).
	...
	private static class CaptchaTimer implements Runnable
	{
		...
		public void run()
		{
			...
			if (isCaptchaCodeWrong(_activeChar))
			{
				...
				if (_fromReport)
				{
					AdminTable.broadcastToGMs(new Say2(0, ChatType.HERO_VOICE, "REPORT", _activeChar.getName() + "  has FAILED on captcha test!"));
				}
