...
public class TradesHandler
{
	...
	private static Map<Integer, Trade> _trades = new HashMap<>();
	...
	private static List<Trade> getList()
	{
		...
		return _trades.values().stream().sorted((o1, o2) -> o1.timeAddedInSecs - o2.timeAddedInSecs).collect(Collectors.toList());
	}
	...
	private static List<Trade> getList(String search)
	{
		...
		Map<Integer, Trade> result = new TreeMap<>(); // OrderPoints, Trade
	...
	protected static class TradeItemListener implements OnAnswerListener
	{
		...
		public void sayNo()
		{
			...
			{
				// Original Message: You can disable this message in .cfg -> DisableTradelistAutoask.
				player.sendMessage(new CustomMessage("l2r.gameserver.randoms.TradesHandler.message1", player));
			}
		}
	}
}
