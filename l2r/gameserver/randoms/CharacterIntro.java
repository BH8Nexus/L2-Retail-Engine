...
{
	private static final Logger LOG = LoggerFactory.getLogger(CharacterIntro.class);
	...
	public static String getAccountValue(String accountName, String var)
	{
		...
		{
			LOG.warn("Could not get value data:", e);
		}
		finally
		{
	...
	public static String getAccountVar(String accountName)
	{
		...
		{
			LOG.warn("Could not get var data:", e);
		}
		finally
		{
			DbUtils.closeQuietly(con, statement, rset);
	...
	public static void setSecurity(Player activeChar, String bypass)
	{
		...
		{
			activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "Password cannot be empty!" : "Password cannot be empty!"));
			showTutorialHTML("security", activeChar);
			return;
		}
		...
		if (activeChar.getSecurityPassword() != null)
		{
			activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "You already have a password!" : "You already have a password!"));
			showTutorialHTML("security", activeChar);
			return;
		}
		...
		try
		{
			...
			if (!((newpass == null) || (repeatnewpass == null)))
			{
				...
				{
					activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "The new password doesn't match with the repeated one!" : "The new password doesn't match with the repeated one!"));
					showTutorialHTML("security", activeChar);
					return;
				}
				...
				{
					activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "Your password is too easy to guess. Please use more difficult password!" : "Your password is too easy to guess. Please use more difficult password!"));
					showTutorialHTML("security", activeChar);
					return;
				}
				...
				{
					activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "The security password cannot be the same as your account ID." : "The security password cannot be the same as your account ID."));
					showTutorialHTML("security", activeChar);
					return;
				}
				...
				{
					activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "The security password cannot be the same as your character name." : "The security password cannot be the same as your character name."));
					showTutorialHTML("security", activeChar);
					return;
				}
				...
				{
					activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "The new password is shorter than 5 chars! Please try with a longer one" : "The new password is shorter than 5 chars! Please try with a longer one."));
					showTutorialHTML("security", activeChar);
					return;
				}
				...
				{
					activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "The new password is longer than 30 chars! Please try with a shorter one." : "The new password is longer than 30 chars! Please try with a shorter one."));
					showTutorialHTML("security", activeChar);
					return;
				}
				...
				raw = md.digest(raw);
				String newpassEnc = Base64.getEncoder().encodeToString(raw);// .encodeBytes(raw);
				...
				{
					activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "The security password cannot match your login password!" : "The security password cannot match your login password!"));
					return;
				}
				...
				activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "You have successfully set your password!" : "You have successfully set your password!"));
				showTutorialHTML("emailvalidation", activeChar);
			...
			{
				activeChar.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (activeChar.isLangRus() ? "Invalid password data! You have to fill all boxes." : "Invalid password data! You have to fill all boxes."));
				showTutorialHTML("security", activeChar);
				return;
			}
		}
	...
	public static void sendEmailValidation(Player player, String email, String email2)
	{
		...
		if (!email.isEmpty() && email.contains("@") && email.contains(".") && (email.length() <= 50) && (email.length() >= 5))
		{
			if (email.equalsIgnoreCase(email2))
			{
				CharacterEmails.setEmail(player, email);
				player.sendChatMessage(0, ChatType.TELL.ordinal(), "Email", (player.isLangRus() ? "Π�Π΄Ρ€ΠµΡ� Ρ�Π»ΠµΠΊΡ‚Ρ€ΠΎΠ½Π½ΠΎΠΉ ΠΏΠΎΡ‡Ρ‚Ρ‹ Π²Π°Ρ�ΠµΠ³ΠΎ Π°ΠΊΠΊΠ°ΡƒΠ½Ρ‚Π° Π² Π½Π°Ρ�Ρ‚ΠΎΡ�Ρ‰ΠµΠµ Π²Ρ€ΠµΠΌΡ�:" : "Your account's e-mail is now: ") + email);
			...
			else
			{
				...
				player.sendChatMessage(0, ChatType.TELL.ordinal(), "Email", (player.isLangRus() ? "Π’Π²ΠµΠ»ΠΈ Ρ�Π»ΠµΠΊΡ‚Ρ€ΠΎΠ½Π½ΡƒΡ� ΠΏΠΎΡ‡Ρ‚Ρƒ Π² Ρ�Ρ‰ΠΈΠΊΠ°Ρ… Π½Πµ Ρ�ΠΎΠ²ΠΏΠ°Π΄Π°Ρ�Ρ‚." : "The typed e-mails in the boxes do not match."));
			}
		...
		{
			showTutorialHTML("emailvalidation", player);
			player.sendChatMessage(0, ChatType.TELL.ordinal(), "Email", (player.isLangRus() ? "ΠΆΠΈΠΏΠΈΠ·ΠΈΡ€ΠΎΠ²Π°Π½Π½Ρ‹Ρ… Ρ�Π»ΠµΠΊΡ‚Ρ€ΠΎΠ½Π½ΠΎΠΉ ΠΏΠΎΡ‡Ρ‚Ρ‹ Ρ�Π²Π»Ρ�ΠµΡ‚Ρ�Ρ� Π½ΠµΠ΄ΠΎΠΏΡƒΡ�Ρ‚ΠΈΠΌΡ‹ΠΌ." : "The typed e-mail is invalid."));
		}
	}
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (updated > 0)
				{
					...
					LOG.warn("Referral System: Player " + activeChar.getName() + " has reached level " + levelPassed + " . And recived " + amount + " coins.");
				}
