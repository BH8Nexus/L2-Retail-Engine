...
public class LocationStorage
{
	...
	{
		/*
		 * if (loc != null && loc.r == ReflectionManager.DEFAULT_ID) { loc.setH(delayBeforeMove); // Ub3r h4x0r drunkard hack. Use heading for move delay HAAHAHAHAH! List<Location> locs = _storage.get(objId); if (locs == null) _storage.put(objId, locs = new CopyOnWriteArrayList<Location>()); if
		 * (locs.isEmpty()) locs.add(loc); else { Location prevLoc = locs.get(locs.size() - 1); // Last location if (Location.checkIfInRange(25, prevLoc.x, prevLoc.y, loc.x, loc.y)) // Check if there is already a location nearby return false; locs.add(loc); } return true; }
		 */
