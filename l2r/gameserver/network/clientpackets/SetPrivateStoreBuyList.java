...
public class SetPrivateStoreBuyList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(System.currentTimeMillis() - buyer.getLastSetPrivateStoreBuyListPacket() < Config.SETPRIVATESTOREBUYLISTPACKETDELAY)
		{
			buyer.sendActionFailed();
			return;
		}
		buyer.setLastSetPrivateStoreBuyListPacket();
		...
		try
		{
			loop: for (int i = 0; i < _count; i++)
			{
				...
				{
					buyer.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.SetPrivateStoreBuyList.TooLowPrice", buyer).addItemName(item).addNumber(item.getReferencePrice() / 2));
					continue;
				}
