	...
	private static void doPutOld(Player activeChar, ItemInstance itemToEnchant, ItemInstance scroll)
	{
		...
		switch (itemToEnchant.getItemId())
		{
			...
			default:
			{
				...
				{
					if ((Config.ENCHANT_MAX_WEAPON > 0) && (itemToEnchant.getEnchantLevel() >= Config.ENCHANT_MAX_WEAPON))
					{
						fail = true;
					}
				}
				...
				{
					if ((Config.ENCHANT_MAX_ARMOR > 0) && (itemToEnchant.getEnchantLevel() >= Config.ENCHANT_MAX_ARMOR))
					{
						fail = true;
					}
				}
				...
				{
					if ((Config.ENCHANT_MAX_JEWELRY > 0) && (itemToEnchant.getEnchantLevel() >= Config.ENCHANT_MAX_JEWELRY))
					{
						fail = true;
					}
				}
