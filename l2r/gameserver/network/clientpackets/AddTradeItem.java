...
public class AddTradeItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (tradeItem == null)
		{
			// Π΄ΠΎΠ±Π°Π²Π»Ρ�ΠµΠΌ Π½ΠΎΠ²ΡƒΡ� Π²ΠµΡ‰Ρ� Π² Ρ�ΠΏΠΈΡ�ΠΎΠΊ
			tradeItem = new TradeItem(item);
			tradeItem.setCount(count);
			tradeList.add(tradeItem);
		}
		
		//FIXME IS NEED TO TEST MY
...
//		parthner2.sendPacket(new TradeOtherAdd(tradeItem, tradeItem.getCount()));
		parthner1.sendPacket(new TradeOwnAdd(tradeItem, tradeItem.getCount()), new TradeOwnAdd(tradeItem, tradeItem.getCount()));
		parthner1.sendPacket(new TradeUpdate(tradeItem, item.getCount() - tradeItem.getCount()), new TradeUpdate(tradeItem, item.getCount() - tradeItem.getCount()));
		parthner2.sendPacket(new TradeOtherAdd(tradeItem, tradeItem.getCount()));
		parthner2.sendPacket(new TradeOtherAdd(tradeItem, tradeItem.getCount()));
	}
}