...
public class RequestJoinCastleSiege extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		/*
		 * else if (residence.getType() == ResidenceType.Fortress) { registerAtFortress(player, (Fortress) residence, _isAttacker, _isJoining); }
		 */
	...
	/*
	 * private static void showChatWindow(Player player, String htmlfile) { String htmltosend = HtmCache.getInstance().getNotNull("residence2/fortress/" + htmlfile, player); NpcHtmlMessage html = new NpcHtmlMessage(0); html.setHtml(htmltosend); player.sendPacket(html); }
	 */
}
