...
public class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if ((item == null) && activeChar.canOverrideCond(PcCondOverride.ITEM_USE_CONDITIONS)) // Support for GMs deleting items from alt+g inventory.
		{
		...
		if (!activeChar.canOverrideCond(PcCondOverride.ITEM_USE_CONDITIONS))
		{
			...
			{
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.UseItem.NotUseIsEvents", activeChar));
				return;
			}
			...
			{
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.UseItem.NotUseIsFlag", activeChar));
				return;
			}
			...
			{
				activeChar.sendMessage("Use at your state is not possible.");
				return;
			}
			...
			// FIXME System protection test bow in olympiad true or false
			if (Config.BOWTAN_PENALTY)
			{
				...
				if (!activeChar.isInOlympiadMode() && ((classid == 88) || (classid == 89) || (classid == 6) || (classid == 90) || (classid == 91) || (classid == 100) || (classid == 99) || (classid == 113) || (classid == 114)))
				{
					...
					{
						SystemMessage sm = new SystemMessage(SystemMessage.S1_CANNOT_BE_FOUND);
			...
			if (!Config.ALT_GAME_KARMA_PLAYER_CAN_TELEPORT && (activeChar.getKarma() > 0))
			{
				...
				if (skills.length > 0)
				{
					...
					{
						if ((skill != null) && ((skill.getSkillType() == SkillType.TELEPORT_NPC) || (skill.getSkillType() == SkillType.RECALL)))
						{
							return;
						}
					}
		...
		if (item.isEquipable())
		{
			...
			if ((Config.MAX_ITEM_ENCHANT_KICK > 0) && !activeChar.isGM() && (item.getEnchantLevel() > Config.MAX_ITEM_ENCHANT_KICK))
			{
				...
				// Util.handleIllegalPlayerAction(activeChar, "Player " + activeChar.getName() + " has item Overenchanted! Kicked ", null, Config.DEFAULT_PUNISH);
				Util.handleIllegalPlayerAction(activeChar, "EnterWordl [Overenchanted]", "Player \" + activeChar.getName() + \" has item Overenchanted! Kicked  ", 2);
