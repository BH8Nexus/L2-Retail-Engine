...
public class SetPrivateStoreSellList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(System.currentTimeMillis() - seller.getLastSetPrivateStoreListPacket() < Config.SETPRIVATESTORELISTPACKETDELAY)
		{
			seller.sendActionFailed();
			return;
		}
		seller.setLastSetPrivateStoreListPacket();
		...
		try
		{
			for (int i = 0; i < _count; i++)
			{
				...
				if ((seller.getPet() != null) && (seller.getPet().getControlItemObjId() == item.getObjectId()))
				{
					...
					seller.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.SetPrivateStoreSellList.CannotSellYourPet", seller));
					continue;
				}
