...
public class RequestMultiSellChoose extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			// Original Message: You cannot use this while in Event.
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestMultiSellChoose.message1", activeChar));
			activeChar.sendActionFailed();
			activeChar.setMultisell(null);
			return;
		}
		...
		{
			// Original Message: You cannot use this while on Olympiad.
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestMultiSellChoose.message2", activeChar));
			activeChar.sendActionFailed();
			activeChar.setMultisell(null);
			return;
		}
		...
		final List<ItemData> _items = new ArrayList<>();
		...
		try
		{
			...
			for (MultiSellIngredient ingridient : entry.getIngredients())
			{
				...
				if (ingridientItemId == ItemTemplate.ITEM_ID_CLAN_REPUTATION_SCORE)
				{
					...
					{
						// Original Message: You need at least clan level 5.
						activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestMultiSellChoose.message3", activeChar));
						return;
					}
				...
				else if (ingridientItemId == ItemTemplate.ITEM_ID_DIVISION_POINTS)
				{
					if (activeChar.getDivisionPoints() < totalAmount)
					{
						activeChar.sendPacket(SystemMsg.YOU_ARE_SHORT_OF_ACCUMULATED_POINTS);
						return;
					}
			...
			if ((tax > 0) && !notax)
			{
				if (castle != null)
				{
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestMultiSellChoose.HavePaidTax", activeChar).addNumber(tax));
			...
			for (MultiSellIngredient in : entry.getProduction())
			{
				...
				else if (ItemHolder.getInstance().getTemplate(in.getItemId()).isStackable() && !bindtochar) // stupid hack...
				{
					...
					// activeChar.getInventory().addItem(in.getItemId(), total, true);
				...
				else
				{
					...
					{
						ItemInstance product = new ItemInstance(in.getItemId());
