...
import l2r.gameserver.network.serverpackets.ExReplyRegisterDominion;
...
public class RequestExJoinDominionWar extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		registerAtDominion(player, dominion, _isRegistration, _clanRegistration);
	}
	
	public static void registerAtDominion(Player player, Dominion dominion, boolean isReg, boolean isclanReg)
	{
		if (player == null)
		{
			return;
		...
		if (isReg)
		{
			...
			if (!isclanReg && ((clanReg > 0) || (playerReg > 0)))
			{
				player.sendPacket(SystemMsg.YOUVE_ALREADY_REQUESTED_A_TERRITORY_WAR_IN_ANOTHER_TERRITORY_ELSEWHERE);
				return;
			}
		...
		{
			if (isclanReg && (clanReg != dominion.getId()))
			{
				return;
			}
			if (!isclanReg && (playerReg != dominion.getId()))
			{
				return;
			}
		...
		player.sendPacket(new ExReplyRegisterDominion(dominion, true, isReg, isclanReg));
	}
}