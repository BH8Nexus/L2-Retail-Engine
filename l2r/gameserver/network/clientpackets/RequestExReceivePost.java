		...
		if (mail != null)
		{
			...
			try
			{
				...
				synchronized (attachments)
				{
					...
					if (mail.getPrice() > 0)
					{
						...
						else
						{
							...
							ItemInstance item = new ItemInstance(ItemTemplate.ITEM_ID_ADENA);
							item.setOwnerId(reply.getReceiverId());
							item.setCount(mail.getPrice());
							item.setLocation(ItemLocation.MAIL);
							item.save();
							
							Log.item(Log.MailRecivePayment, sender, activeChar, item, "Packet Hash: " + hashCode());
				...
				for (ItemInstance item : items)
				{
					...
					Log.item(Log.MailRecive, sender, activeChar, item, "Packet Hash: " + hashCode());
