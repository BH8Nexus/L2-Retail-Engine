...
public class RequestEnchantItemAttribute extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (!activeChar.getPermissions().canAttributeItem(itemToEnchant, stone, true))
		{
			activeChar.sendActionFailed();
			return;
		}
		...
		Log.add(activeChar.getName() + "|Trying to attribute enchant|" + itemToEnchant.getItemId() + "|attribute:" + element + "|" + itemToEnchant.getObjectId(), "enchants");
		...
		for (int i = 0; i < attributeTries; i++)
		{
			...
			if (activeChar.getClient().getBonusExpire() > (System.currentTimeMillis() / 1000L))
			{
			...
			// if (Rnd.chance(stone.getTemplate().isAttributeCrystal() ? Config.ENCHANT_ATTRIBUTE_CRYSTAL_CHANCE : Config.ENCHANT_ATTRIBUTE_STONE_CHANCE)) + premiumBonusChance))
			if (success)
			{
				...
				Log.add(activeChar.getName() + "|Successful to attribute enchant|" + itemToEnchant.getItemId() + "|attribute:" + element + "|" + itemToEnchant.getObjectId(), "enchants");
			...
			{
				Log.add(activeChar.getName() + "|Failed to attribute enchant|" + itemToEnchant.getItemId() + "|attribute:" + element + "|" + itemToEnchant.getObjectId(), "enchants");
