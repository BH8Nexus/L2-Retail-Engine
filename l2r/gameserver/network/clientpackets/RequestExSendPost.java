...
import l2r.gameserver.model.actor.Custom.Mail.MailManager;
...
public class RequestExSendPost extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestExSendPost.message5", activeChar).addNumber(Config.LEVEL_REQUIRED_TO_SEND_MAIL));
			return;
		}
		...
		{
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestExSendPost.MailDisabled", activeChar));
			activeChar.sendActionFailed();
			return;
		}
		...
		{
			// Original Message: Mail is allowed once per " + (activeChar.getLevel() >= 76 ? "minute." : "10 minutes."));
			if (activeChar.getLevel() >= 76)
			{
				// Original Message: Mail is allowed once per minute.
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestExSendPost.message2", activeChar));
			}
			else
			{
				// Original Message: Mail is allowed once per 10 minutes.
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestExSendPost.message3", activeChar));
			}
			return;
		}
		...
		if (_price > 0)
		{
			...
			if ((tradeBan != null) && (tradeBan.equals("-1") || (Long.parseLong(tradeBan) >= System.currentTimeMillis())))
			{
				...
				{
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestExSendPost.TradeBannedPermanently", activeChar));
				}
				else
				{
					activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestExSendPost.TradeBanned", activeChar).addString(Util.formatTime((int) ((Long.parseLong(tradeBan) / 1000L) - (System.currentTimeMillis() / 1000L)))));
				}
				return;
			}
		}
		...
		{
			// Original Message: Your mail contains prohibited words. Correct it and try again.
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.network.clientpackets.RequestExSendPost.message4", activeChar));
			return;
		}
		...
		try
		{
			...
			if (_count > 0)
			{
				for (int i = 0; i < _count; i++)
				{
					...
					Log.item(Log.MailSend, activeChar, target != null ? target : recieverId, item, "Packet Hash: " + hashCode());
		...
		Log.logMail(mail, activeChar.getAccessLevel().getLevel() > 0);
