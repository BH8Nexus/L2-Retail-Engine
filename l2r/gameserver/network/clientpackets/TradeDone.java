...
import l2r.gameserver.network.serverpackets.TradePressOtherOk;
...
public class TradeDone extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		final boolean p1Override = parthner1.canOverrideCond(PcCondOverride.ITEM_TRADE_CONDITIONS);
		
		if (!p1Override && parthner1.isOutOfControl())
		{
			request.cancel();
			parthner1.sendPacket(SendTradeDone.FAIL);
			parthner1.sendActionFailed();
		...
		if (_response == 0)
		{
			request.cancel();
			parthner1.sendPacket(SendTradeDone.FAIL);
			parthner2.sendPacket(SendTradeDone.FAIL, new SystemMessage2(SystemMsg.C1_HAS_CANCELLED_THE_TRADE).addString(parthner1.getVisibleName(parthner2)));
			return;
		}
		
		final boolean p2Override = parthner2.canOverrideCond(PcCondOverride.ITEM_TRADE_CONDITIONS);
		final boolean override = p1Override || p2Override;
		if (!override && !parthner1.isInRangeZ(parthner2, Creature.INTERACTION_DISTANCE))
		{
		...
		parthner2.sendPacket(new SystemMessage2(SystemMsg.C1_HAS_CONFIRMED_THE_TRADE).addString(parthner1.getVisibleName(parthner2)), TradePressOtherOk.STATIC);
		...
		try
		{
			...
			if (!override && !parthner2.getInventory().validateWeight(weight))
			{
				parthner2.sendPacket(SystemMsg.YOU_HAVE_EXCEEDED_THE_WEIGHT_LIMIT);
				return;
			}
			
			if (!override && !parthner2.getInventory().validateCapacity(slots))
			{
			...
			if (!override && !parthner1.getInventory().validateWeight(weight))
			{
				parthner1.sendPacket(SystemMsg.YOU_HAVE_EXCEEDED_THE_WEIGHT_LIMIT);
				return;
			}
			
			if (!override && !parthner1.getInventory().validateCapacity(slots))
			{
			...
			// I need to look againt
			// ItemLogHandler.getInstance().addLog(parthner1, parthner2, tradeList2, tradeList1, ItemActionType.TRADE);
			
			success = true;
			/*
			 * for(TradeItem ti : tradeList1) { ItemInstance item = parthner1.getInventory().removeItemByObjectId(ti.getObjectId(), ti.getCount()); Log.item(Log.TradeSell, parthner1, parthner2, item, "Trade by Hand, ID: " + request.getId()); parthner2.getInventory().addItem(item); } for(TradeItem ti
			 * : tradeList2) { ItemInstance item = parthner2.getInventory().removeItemByObjectId(ti.getObjectId(), ti.getCount()); Log.item(Log.TradeSell, parthner2, parthner1, item, "Trade by Hand, ID: " + request.getId()); parthner1.getInventory().addItem(item); }
			 * parthner1.sendPacket(SystemMsg.YOUR_TRADE_WAS_SUCCESSFUL); parthner2.sendPacket(SystemMsg.YOUR_TRADE_WAS_SUCCESSFUL); success = true;
			 */
		}
		finally
		{
