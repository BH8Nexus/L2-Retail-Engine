...
public class OlympiadNobleDAO
{
	...
	//public static final String GET_EACH_CURRENT_CLASS_LEADER = "SELECT characters.char_name, olympiad_points FROM `olympiad_nobles` LEFT JOIN characters ON char_id=characters.obj_Id WHERE `class_id` = ? AND `competitions_done` >= ? AND `competitions_win` > 0 ORDER BY `olympiad_points` DESC, `competitions_win` DESC, `competitions_done` DESC LIMIT 10";
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement(SELECT_BY_CHAR_ID_SQL_QUERY))
		{
