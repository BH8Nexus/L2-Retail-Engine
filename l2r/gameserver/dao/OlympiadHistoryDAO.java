...
public class OlympiadHistoryDAO
{
	...
	public static final String DELETE_CHARACTER_HISTORY_SQL_QUERY = "DELETE FROM `olympiad_history` WHERE `object_id_1` = ? OR `object_id_2` = ?";
	...
	public boolean clearHistory(int nobleId)
	{
		Connection con = null;
		PreparedStatement statement = null;
		try
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement(DELETE_CHARACTER_HISTORY_SQL_QUERY);
			statement.setInt(1, nobleId);
			statement.setInt(2, nobleId);
			return statement.execute();
		}
		catch (Exception e)
		{
			_log.error("Can't delete noble history " + nobleId + " from `olympiad_history`!", e);
		}
		return false;
	}
}
