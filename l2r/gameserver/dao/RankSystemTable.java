...
// {
// private final Long MIN_MAX_CLEAN_RATE = (long) (Config.RANK_NPC_RELOAD * 60000);
...
//
// private final String QRY_SELECT_ITEMS = "SELECT c.obj_Id, c.char_name, Sum(i.count) AS total " + "FROM items AS i Inner Join characters AS c ON i.owner_id = c.obj_Id " + "WHERE c.accesslevel = 0 AND i.item_id = ? " + "GROUP BY c.obj_Id " + "ORDER BY total DESC LIMIT 0, ?";
//
// private final String QRY_SELECT_OLYMPIAD = "SELECT c.obj_Id, c.char_name, o.competitions_win AS total " + "FROM olympiad_nobles AS o Inner Join characters AS c ON o.char_id = c.obj_Id " + "WHERE c.accesslevel = 0 AND o.class_id = ? AND o.competitions_done >= 1 " + "ORDER BY o.competitions_win
// DESC LIMIT 0, ?";
//
// private final String QRY_SELECT_PVPPK = "SELECT c.obj_Id, c.char_name, c.%column% AS total " + "FROM characters AS c " + "WHERE c.accesslevel = 0 AND c.%column% > 0 " + "ORDER BY c.%column% DESC LIMIT 0, ?";
//
...
// {
// int count = 0;
// Map<Integer, TreeSet<RankInformation>> valuesResult = new FastMap<>();
...
// ResultSet rset = null;
// TreeSet<RankInformation> list = new TreeSet<>(new AmountComparator());
// try
// {
