...
public class ChampionTemplateTable
{
	...
	public static final String FILE_DIR = "config/npc/ExtChampionMode.xml";
	...
	private static Map<Integer, List<ChampionTemplate>> _SpecificTemplates = new ConcurrentHashMap<>();
	...
	public static List<ChampionTemplate> getSpecificChampionTemplates(int monsterId)
	{
		return _SpecificTemplates.containsKey(monsterId) ? _SpecificTemplates.get(monsterId) : null;
	}
	...
	public void loadConfig()
	{
		try
		{
			...
			File file = Config.findNonCustomResource(FILE_DIR);
			if (!file.exists())
			{
				_log.warn("[ChampionTemplateTable] File is missing " + Config.findNonCustomResource(FILE_DIR).getAbsolutePath());
				return;
			}
			...
			if ((first != null) && "list".equalsIgnoreCase(first.getNodeName()))
			{
				...
				for (Node n = first.getFirstChild(); n != null; n = n.getNextSibling())
				{
					if ("champion".equalsIgnoreCase(n.getNodeName()))
					{
						...
						if ((att = attrs.getNamedItem("monsterId")) == null)
						{
							monsterId = -1;
						}
						else
						{
							monsterId = Integer.parseInt(att.getNodeValue());
						}
						...
						if (monsterId != -1)
						{
							if (!_SpecificTemplates.containsKey(monsterId))
							{
								_SpecificTemplates.put(monsterId, new ArrayList<>());
							}
							_SpecificTemplates.get(monsterId).add(ct);
						}
						else
						{
